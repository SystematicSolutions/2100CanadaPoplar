...
function IData_Inputs(db)
  @info "Inside IData_Inputs"
  data = Data(; db)
  ...
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  #
  fuel = Select(Fuel, "Thermal")
  tech = Select(Tech, "Storage")
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  ...
  venduse = Select(vEnduse, "ProcessHeat")
  vEUMap[venduse,enduse] = 1.0

  enduse = Select(Enduse, "Motors")
  venduse = Select(vEnduse, "Motors")
  vEUMap[venduse,enduse] = 1.0
  ...
  venduse = Select(vEnduse, "Misc")
  vEUMap[venduse,enduse] = 1.0

  enduse = Select(Enduse, "OffRoad")
  venduse = Select(vEnduse, "OffRoad")
  vEUMap[venduse,enduse] = 1.0

  enduse = Select(Enduse, "Steam")
  venduse = Select(vEnduse, "Steam")
  vEUMap[venduse,enduse] = 1.0
  ...
  tech = Select(Tech, "Electric")
  fuels = Select(Fuel, ["Electric","Geothermal","Hydro","Wind"])
  for fuel in fuels
  ...
  tech = Select(Tech, "Steam")
  fuels = Select(Fuel, ["Geothermal","Steam"])
  for fuel in fuels   
    vFlMap[fuel,tech] = 1.0
  end
  
  tech = Select(Tech, "OffRoad")
  fuels = Select(Fuel, ["Asphalt","Asphaltines","AviationGasoline","Biodiesel","Biojet","Biogas",
      "Diesel","Electric","Ethanol","Gasoline","HFO","JetFuel","Kerosene","LFO","LPG","Lubricants",
      "Naphtha","NaturalGas","NonEnergy","PetroFeed","RNG"])
  for fuel in fuels   
    vFlMap[fuel,tech] = 1.0
  end
  ...
  #
  @. ElecMap=0.0
  techs=Select(Tech,["Electric","HeatPump"])
  ElecMap[techs] .= 1.0
  ...
  tech = Select(Tech, "Oil")
  fuels = Select(Fuel, ["Asphalt","Asphaltines","AviationGasoline","Biojet",
      "HFO","JetFuel","Kerosene","LFO","Lubricants",
      "Naphtha","NonEnergy","PetroFeed"])
  for fuel in fuels
    FTMap[fuel,tech] = 1.0
  end
  ...
  tech = Select(Tech, "OffRoad")
  fuels = Select(Fuel, ["Biodiesel","Diesel","Ethanol","Gasoline"])
  for fuel in fuels
    FTMap[fuel,tech] = 1.0
  end
  ...
  # TODO - debug these values - Jeff Amlin 03/30/25
  ...
  #0.894   #Chemicals                     
  #0.894   #Chemicals                     
  #0.894   #Petroleum Products            
  #0.894   #Rubber                        
  #0.894   #Cement                        
  #0.894   #Glass                         
  #0.894   #Lime & Gypsum                 
  #0.894   #Other Non-Metallic            
  #0.894   #Iron & Steel                  
  #0.894   #Aluminum                      
  #0.894   #Other Nonferrous              
  #0.894   #Transport Equipment           
  #0.894   #Other Manufacturing           
  #0.560   #Metal Mining                  
  #0.560   #Metal Mining                  
  #0.560   #Non-metal Mining              
  #0.560   #Light Oil Mining              
  #0.560   #Heavy Oil Mining              
  #0.560   #Frontier Oil Mining           
  #0.560   #Oil Sands In-Situ             
  #0.560   #Oil Sands In-Situ             
  #0.560   #Oil Sands In-Situ             
  #0.560   #Oil Sands Mining              
  #0.560   #Oil Sands Upgraders           
  #0.560   #Conv. Gas Production          
  #0.560   #Sweet Gas Processing          
  #0.560   #UnConv Gas Production         
  #0.560   #Sour Gas Processing           
  #0.560   #LNG Production                
  #0.560   #Coal Mining                   
  #0.560   #Construction                  
  #0.538   #Forestry                      
  #0.538   #On Farm Fuel Use              
  #0.538   #Crop Production               
  #0.538   #Animal Production             
  #]
  ...
  ecs = Select(EC,["PrimaryOilSands","SAGDOilSands","CSSOilSands",
  ...
  #                               Peak  Ave   Min
  enduse=Select(Enduse,"Heat")
  xLSF[enduse,1,1,Days,Summer,1]=[1.100 1.020 0.800]
  xLSF[enduse,1,1,Days,Winter,1]=[1.100 0.980 0.800]
  ...
  # *******************************************************************************
  # Define Variable
  # xPolute(Enduse,FuelEP,EC,Poll,Area,Year)     'Exogenous Pollution Adjustment (Tonnes/Yr)',
  #  Type=Real(14,0), Disk(Input,xPolute(Enduse,FuelEP,EC,Poll,Area,Year))
  # End Define Variable
  # *
  # * Exogenous pollution adjustment is initialized at 0 (No exogenous adjustment)
  # *
  @. xPolute = 0
  WriteDisk(db,"$Input/xPolute",xPolute)
  ...
  # * 
  @. RCPL = 20
  ecs = Select(EC,(from = "Cement", to = "OtherManufacturing"))
  for ec in ecs
    RCPL[ec,:] .= 33
  end
...
end # end IData_Inputs

function Control(db)
  @info "IData.jl - Control"
  IData_Inputs(db)
end
