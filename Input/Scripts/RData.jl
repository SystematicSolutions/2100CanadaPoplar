...
function RData_Inputs(db)
  @info "Inside RData_Inputs"
  data = Data(; db)
  ...
  #                               Peak  Ave   Min
  enduse=Select(Enduse,"Heat")
  xLSF[enduse,1,1,Days,Summer,1]=[0.000 0.000 0.000]
  xLSF[enduse,1,1,Days,Winter,1]=[5.911 2.255 1.860]

  enduse=Select(Enduse,"HW")
  xLSF[enduse,1,1,Days,Summer,1]=[0.488 0.936 0.603]
  xLSF[enduse,1,1,Days,Winter,1]=[0.467 1.064 1.034]

  enduse=Select(Enduse,"OthSub")
  xLSF[enduse,1,1,Days,Summer,1]=[0.901 0.897 0.265]
  xLSF[enduse,1,1,Days,Winter,1]=[2.252 1.075 0.166]

  enduse=Select(Enduse,"Refrig")
  xLSF[enduse,1,1,Days,Summer,1]=[1.178 1.139 0.895]
  xLSF[enduse,1,1,Days,Winter,1]=[0.995 0.899 0.715]

  enduse=Select(Enduse,"Light")
  xLSF[enduse,1,1,Days,Summer,1]=[0.414 0.728 0.358]
  xLSF[enduse,1,1,Days,Winter,1]=[3.459 1.172 0.358]

  enduse=Select(Enduse,"AC")
  xLSF[enduse,1,1,Days,Summer,1]=[4.000 2.000 1.000]
  xLSF[enduse,1,1,Days,Winter,1]=[0.000 0.000 0.000]

  enduse=Select(Enduse,"OthNSub")
  xLSF[enduse,1,1,Days,Summer,1]=[0.914 0.893 0.465]
  xLSF[enduse,1,1,Days,Winter,1]=[2.128 1.142 0.484]
  ...
  @. PDif=1.0
  enduse=Select(Enduse,"Heat")
  techs=Select(Tech,["Electric","HeatPump","DualHPump"])
  PDif[enduse,techs,:,:] .= 1.64
  tech=Select(Tech,"Oil")
  PDif[enduse,tech,:,:] .= 1.06
  ...
  #   .134 .134 
  #]
  # TODO - assign tax rates - Jeff Amlin 3/31/25
  #
  #TxRt[:,:,years] .= TxRt[1,1,years]
  #
  #years = collect(Yr(2032):Final)
  #for year in years
  #  TxRt[:,:,year] .= TxRt[:,:,Yr(2031)]
  #end
  
  for year in Years, area in Areas, ec in ECs
    TxRt[ec,area,year] = 0.134
  end
...
end # end RData_Inputs

function Control(db)
  @info "RData.jl - Control"
  RData_Inputs(db)
end
