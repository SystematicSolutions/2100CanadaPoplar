#
# GHG_VB_Ind.jl - Read in Industrial GHG emissions coefficients from VBInput
#
using EnergyModel
...
function ICalibration(db)
  data = IControl(; db)
  (;Input) = data
  (;Area,EC,ECCMap,ECC,ECs,Enduse) = data
  (;Enduses,FuelEP,FuelEPs,Fuels) = data
  (;Poll,Years,vArea) = data
  (;CgPOCX,FsPOCX,POCX,vFsPOCX,vPOCX) = data
  
  
  println("Reading Industrial GHG POCX")  
  ...
  IndECCs = Select(ECC, (from = "Food", to = "AnimalProduction"))
    ...
    if ec != []
      ...
      for area in AreasCanada
        ...
        for eu in Enduses,fuel in FuelEPs,poll in GHGs,year in Years
          POCX[eu,fuel,ec,poll,area,year]=vPOCX[fuel,ecc,poll,varea,year]
        end
      end
    end
  end
  ...
  println("Reading Industrial GHG CgPOCX")  
...
#   *
# * Exception: POCX stores a non-zero value for Coke, CO2, IronSteel
# *       for cogen emissions. However, once that value is saved
# *       to CgPOCX, we do not want the CO2, Coke emissions to
# *       be calculated for energy Dmd per R.White. 5/15/12 RBL
# *
Coke = Select(FuelEP,"Coke")
CO2 = Select(Poll,"CO2")
IronSteel = Select(EC,"IronSteel")
for area in AreasCanada, year in years
  POCX[firstEU,Coke,IronSteel,CO2,area,year]=0
end


  println("Reading Industrial GHG FsPOCX")  
...
function CalibrationControl(db)
  @info "GHG_VB_Ind.jl - CalibrationControl"
