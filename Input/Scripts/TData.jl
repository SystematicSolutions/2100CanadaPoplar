...
const Outpt = "TOutput"
const CalDB = "TCalDB"
const SectorName::String = "Transport"
...
Base.@kwdef struct Data
  ...
  FTMap::VariableArray{3} = ReadDisk(db,"$Input/FTMap") # [Fuel,EC,Tech] Map between Fuel and Tech
  ...
  POCAM::VariableArray{6} = ReadDisk(db,"$Input/POCAM") # [FuelEP,Tech,EC,Poll,Area,Year] Average Pollution Coefficients Multiplier (Fraction)
  ...
  POCS::VariableArray{7} = ReadDisk(db,"$Input/POCS") # [Enduse,FuelEP,Tech,EC,Poll,Area,Year] Pollution Standards (Tonnes/TBtu)
  ...
  TrPCovMap::VariableArray{5} = ReadDisk(db,"$Input/TrPCovMap") # [Tech,EC,PCov,Area,Year] 'Transportation Technology Pollution Coverage Map (1=Mapped)'
...
function TData_Inputs(db)
  @info "Inside TData_Inputs"
  data = Data(; db)
  (; Areas,Day,Days,EC,ECs,ECC,Enduse,Enduses,ES,Fuel,FuelEP) = data
  (; Hour,Hours,Month,Months,PCov,PI,PIs,Poll,Polls,Tech,Techs,Years) = data
  (; CgDmSw,CgVCSw,CurTime,DActV,DDHourly,DDHourlyCoefficient,DEPM,DmdSw) = data
  (; DEEThermalMax,xDmFrac,xFsFrac,xDCMM,xDEMM) = data
  (; DUCFSw,ECCMap,EEImpact,EESat,EESw,EEUCosts,xEE) = data
  (; DDSmoothingTime,ECESMap,ElecMap,FTMap,PEMMSw,PEPM,PInvMinFrac,RetroSw) = data
  (; RetroSwExo,xProcSw,xXProcSw,CnvrtEU,BAT,BCMultTr,BE,BMM,CgAT,CgHRtM,CgIVTC) = data
  (; CgLoad,CgMSMM,CgRisk,CgSCM,CgPL,CgPotMult,CgPotSw,CgResI,CgTL) = data
  (; CgBL,CgCUFP,CgOF,xCgVF,CHRM,CMSFSwitch,CROIN,DCCLimit,DEEAM,DERReduction,DOCF) = data
  ...
  (; RDCCM,ROIN,RDVF,RPVF,RHCM,RPCCM,RPEEFr,RPMSLimit,TrPCovMap,TSLoad,TxRt) = data
  (; xRM,CalibTime,YCERSM,YCgCUF,YCgMSM,YCUF,YDCMM,YDEMM,YDSt,YEndTime) = data
  (; InitialDemandYear,YFsPEE,YMMSM,YPEMM,ROCF,VRP,VRRT) = data
  ...
  tech = Select(Tech, ["LDVFuelCell","LDTFuelCell","BusFuelCell","TrainFuelCell","PlaneFuelCell","HDV2B3FuelCell",
                      "HDV45FuelCell","HDV67FuelCell","HDV8FuelCell","MarineFuelCell"])
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  #
  fuel = Select(Fuel, "Ethanol")
  tech = Select(Tech, ["LDVEthanol","LDTEthanol"])
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  #
  fuel = Select(Fuel, "NaturalGas")
  tech = Select(Tech, ["LDVNaturalGas","LDTNaturalGas","HDV2B3NaturalGas","HDV45NaturalGas",
                      "HDV67NaturalGas","HDV8NaturalGas","BusNaturalGas"])
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  #
  fuel = Select(Fuel, "LPG")
  tech = Select(Tech,["LDVPropane","LDTPropane","HDV2B3Propane","HDV45Propane","HDV67Propane",
                      "HDV8Propane","BusPropane"])
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  #
  fuel = Select(Fuel, "Diesel")
  tech = Select(Tech, ["LDVDiesel","LDTDiesel","HDV2B3Diesel","HDV45Diesel","HDV67Diesel",
                       "HDV8Diesel","BusDiesel","MarineLight","TrainDiesel"])
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  #
  fuel = Select(Fuel, "Gasoline")
  tech = Select(Tech, ["LDVGasoline","LDVHybrid","LDTGasoline","LDTHybrid","HDV2B3Gasoline","HDV45Gasoline",
                       "HDV67Gasoline","HDV8Gasoline","Motorcycle","BusGasoline"])
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  #
  fuel = Select(Fuel, "Electric")
  tech = Select(Tech, ["LDVElectric","HDV2B3Electric","HDV45Electric",
                       "HDV67Electric","HDV8Electric","BusElectric","TrainElectric"])
  xDmFrac[:,fuel,tech,:,:,:] .= 1
  ...
  fuels = Select(Fuel,["LFO","HFO","Gasoline","Diesel","AviationGasoline","Kerosene","PetroCoke",
                       "Asphalt","Asphaltines","Biodiesel","Lubricants",
                      "Naphtha","PetroFeed","NonEnergy"])
  for fuel in fuels
    vFsMap[fuel,1] = 1.0
  end
  ...
  ecc = Select(ECC,"Freight")
  ec  = Select(EC, "Freight")
  ECCMap[ec,ecc] = 1

  ecc = Select(ECC,"AirPassenger")
  ec =  Select(EC, "AirPassenger")
  ECCMap[ec,ecc] = 1

  ecc = Select(ECC,"AirFreight")
  ec  = Select(EC, "AirFreight")
  ECCMap[ec,ecc] = 1

  ecc = Select(ECC,"ForeignPassenger")
  ec  = Select(EC, "ForeignPassenger")
  ECCMap[ec,ecc] = 1

  ecc = Select(ECC,"ForeignFreight")
  ec  = Select(EC, "ForeignFreight")
  ECCMap[ec,ecc] = 1

  ecc = Select(ECC,"ResidentialOffRoad")
  ec  = Select(EC, "ResidentialOffRoad")
  ECCMap[ec,ecc] = 1

  ecc = Select(ECC,"CommercialOffRoad")
  ...
  #
  @. ElecMap=0.0
  techs = Select(Tech,["LDVHybrid","LDTHybrid","LDVElectric","LDTElectric","HDV2B3Electric",
                       "HDV45Electric","HDV67Electric","HDV8Electric","BusElectric","TrainElectric"])
  ElecMap[techs] .= 1.0
  ...
  ########################
  #
  # FTMap[Fuel,EC,Tech] Map between Fuel and Tech
  # 1 is the proper value for transportation (maybe for all sectors) - Jeff Amlin 5/6/25
  # 0 is the  value for transportation ( for all sectors) in Spruce - Peter Volkmar 7/25/25
  #
  @. FTMap = 0
  ...
  xProcSw[Select(PI,"Cogeneration"),Zero]=NonExist
  ...
  #    80 percent of the physical lifetime (25 years)
  #
  @. CgTL=CgPL*0.80
  ...
  #
  # 1. The default value is 50 percent.
  # 2. The values are assigned with an equation.
  # 3. P. Cross 7/18/94
  #
  @. CgCUFP=0.50
  ...
  #
  # For passenger vehicles we have this from KG Duleep, ICF’s auto guru.  His thoughts:
  # Normal O&M non-fuel costs are 3 to 5 cents a mile depending on vehicle size. An average car 
  # does about 12,500 miles/year so cost would be ~ $500 which is about 2 percent of ave. first
  # cost (~$25,000).  Normal replacement parts add about $2500 over the life of the car, which 
  # is about $165/year, so about 2.7% would be good. (This does not include registration fees,
  # taxes, or insurance which we assume should be left out.) 
  #
  # Note from Jeff Amlin - registration fees, taxes, or insurance should be included in the final number. 
  #
  ec = Select(EC,"Passenger")
  DOCF[:,:,ec,:,:] .= 0.027
  ec = Select(EC,["Freight","ResidentialOffRoad","CommercialOffRoad"])
  DOCF[:,:,ec,:,:] .= 0.057

  techs = Select(Tech,["BusGasoline","BusDiesel","BusElectric","BusFuelCell",
                       "TrainDiesel","TrainElectric","TrainFuelCell"])
  DOCF[:,techs,:,:,:] .= 0.057
  techs = Select(Tech,["PlaneJetFuel","PlaneGasoline","PlaneFuelCell"])
  DOCF[:,techs,:,:,:] .= 0.080
  techs = Select(Tech,["MarineLight","MarineHeavy","MarineFuelCell"])
  DOCF[:,techs,:,:,:] .= 0.057
  ...
  # Passenger 
  # 
  # Assume mostly on-peak charging of electric vehicles - V. Keller 02/02/2024 
  #
  Summer=Select(Month,"Summer")
  Winter=Select(Month,"Winter")
  ec=Select(EC,"Passenger")
  #                               Peak  Ave   Min
  xLSF[1,ec,1,Days,Summer,1]=[2.500 1.000 0.500]
  xLSF[1,ec,1,Days,Winter,1]=[2.500 1.000 0.500]

  day=Select(Day,"Average")
  for enduse in Enduses
    SSum[enduse]=sum(xLSF[enduse,ec,hour,day,month,1]*HoursPerMonth[month]/8760 for hour in Hours, month in Months)
  end
  for enduse in Enduses, hour in Hours, month in Months
    @finite_math xLSF[enduse,ec,hour,day,month,1]=xLSF[enduse,ec,hour,day,month,1]/SSum[enduse]
  end
  for area in Areas, month in Months, day in Days, hour in Hours, enduse in Enduses
    xLSF[enduse,ec,hour,day,month,area]=xLSF[enduse,ec,hour,day,month,1]
  end
  ...
  ec=Select(EC,"Freight")
  #                               Peak  Ave   Min
  xLSF[1,ec,1,Days,Summer,1]=[1.000 1.000 1.000]
  xLSF[1,ec,1,Days,Winter,1]=[1.000 1.000 1.000]
  ...
  for area in Areas, month in Months, day in Days, hour in Hours, enduse in Enduses
    xLSF[enduse,ec,hour,day,month,area]=xLSF[enduse,Freight,hour,day,month,area]
  end
  ...
  # *
  # Define Variable
  # TrPCovMap(Tech,EC,PCov,Area,Year)  'Pollution Coverage Map (1=Mapped)
  #  Disk(Input,TrPCovMap(Tech,EC,PCov,Area,Year))
  # End Define Variable
  # * 
  @. TrPCovMap = 0

  #
  # PCov(Oil) defaults to cover all technologies
  #
  ...
  TrPCovMap[techs,:,pcov,:,:] .= 1

  #
  # Set Remaining Technologies to zero
  #
  techs = Select(Tech,["LDVElectric","LDVEthanol","LDVFuelCell","LDTEthanol",
                       "LDTFuelCell","BusElectric","TrainElectric","TrainFuelCell","PlaneFuelCell",
                       "HDV2B3FuelCell","HDV45FuelCell","HDV67FuelCell","HDV8FuelCell","MarineFuelCell"])
  TrPCovMap[techs,:,:,:,:] .= 0
  pcov = Select(PCov,"Energy")
  TrPCovMap[techs,:,pcov,:,:] .= 1
  WriteDisk(db,"$Input/TrPCovMap",TrPCovMap)
  # *
  # *******************************************************************************
  # *
  # Define Variable
  ...
  #  1. The values of this variable are zero and are commented out to save 
  #    execution time.
  #  2. The values are assigned with an equation.  This variable is not active.
  #  3. P. Cross 10/24/95
  # *
  YCgMSM[:,:,:,Zero] .= 1
  ...
  # *
  # Define Variable
  # BCMultTr(Fuel,Tech,EC,Area,Year)    'Multipler between Black Carbon and PM 2.5 (Tonnes/Tonnes)',
  #  Disk(Input,BCMultTr(Fuel,Tech,EC,Area,Year))
  # End Define Variable
  # 
  @. BCMultTr = 0
  WriteDisk(db,"$Input/BCMultTr",BCMultTr)

end # end TData_Inputs

function Control(db)
  @info "TData.jl - Control"
  TData_Inputs(db)
end
