#
# EnergyDemand_VB_Ind.jl - Map industrial energy demands from VBInput
#
using EnergyModel

module EnergyDemand_VB_Ind
...
Base.@kwdef struct IControl
  ...
  vFsMap::VariableArray{2} = ReadDisk(db,"$Input/vFsMap") # [Fuel,Tech] Maps the Feedstock Fuels from vData into Tech
  ...
  # Scratch Variables
  # CgDmdTech  'Cogeneration Energy Demand for Specific Technology (TBtu/Yr)'
  # DmdTech  'Energy Demand for Specific Technology (TBtu/Yr)'
  # FsDmdTech  'Feedstock Energy Demand for Specific Technology (TBtu/Yr)'
  TotDmd::VariableArray{3} = zeros(Float32,length(Enduse),length(Tech),length(EC)) # [Enduse,Tech,EC] Total Demand Across Area (TBtu/Yr)
  TotFsDmd::VariableArray{3} = zeros(Float32,length(Enduse),length(Tech),length(EC)) # [Enduse,Tech,EC] Total Feedstock Demand Across Area (TBtu/Yr)
end

function ICalibration(db)
  data = IControl(; db)
  (;Input,Areas,ECCs,ECs,Enduse,Enduses,Fuels,Tech,Techs,Years,vEnduses,ECCMap) = data
  (;vDmd,vEUMap,vFlMap,vFsMap,vFsDmd,xDmd) = data
  (;xEuDemand,xFsDemand,xFsDmd,DmdFuel,FsDmdFuel) = data
          ...
          for tech in Techs
            for fuel in findall(vFlMap[Fuels,tech] .== 1.0)
              for area in Areas, year in Years
                ...
                if Enduse[eu] != "OffRoad"
                  if Tech[tech] != "OffRoad"
                    DmdFuel[eu,fuel,tech,ec,area,year] = vDmd[veu,fuel,ecc,area,year]
                  end

                elseif Enduse[eu] == "OffRoad"
                  if Tech[tech] == "OffRoad"
                    DmdFuel[eu,fuel,tech,ec,area,year] = vDmd[veu,fuel,ecc,area,year]
                  end
...
function CalibrationControl(db)
  @info "EnergyDemand_VB_Ind.jl - CalibrationControl"
