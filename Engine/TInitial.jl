#
# TInitial.jl - Transportation Constants and initial values
#
# Write (" TInitial.jl, Transportation Constants and initial values")
#
...
Base.@kwdef struct Data
  ...
  PEM::VariableArray{4} = ReadDisk(db,"$CalDB/PEM") # Maximum Process Efficiency ($/Btu) [Enduse,Tech,EC,Area]
  ...
  AvPEM::VariableArray{3} = zeros(Float32,length(Enduse),length(Tech),length(EC)) # [Enduse,Tech,EC] Average Value of PEM
  ...
  DmdFuelTech::VariableArray{5} = zeros(Float32,length(Enduse),length(Fuel),length(Tech),length(EC),length(Area)) # [Enduse,Fuel,Tech,EC,Area] Energy Demands (TBtu/Yr)
  ...
  PEMTech::VariableArray{1} = zeros(Float32,length(Tech)) # [Tech] Scratch Varable to Sort Fuels
  xDmfracFuel::VariableArray{1} = zeros(Float32,length(Fuel)) # [Fuel] Scratch Varable to Sort Fuels
...
function Initial(data,InitialYear,ec,ecc,areas)
  ...
  for area in areas, tech in Techs, enduse in Enduses
    @finite_math PEEA[enduse,tech,ec,area,InitialYear] = 
      ...
      PER[enduse,tech,ec,area,InitialYear]*xPEMM[enduse,tech,ec,area,InitialYear]
  end
  ...
  for enduse in Enduses
    for area in areas, age in Ages, tech in Techs
      fuels = findall(xDmFrac[enduse,Fuels,tech,ec,area,InitialYear] .>= 0)
      #
...
function PEffCurve(data,InitialYear,ec,ecc,areas)
  ...
  #
  for area in areas, tech in Techs, enduse in Enduses
    @finite_math PEM[enduse,tech,ec,area] = PEEA[enduse,tech,ec,area,InitialYear]*
                 PEMX[enduse,tech,ec,area]/xPEMM[enduse,tech,ec,area,InitialYear]
  end
  ...
  for area in areas, tech in Techs, enduse in Enduses
    if PEMX[enduse,tech,ec,area] != 1.0
      ...
      PCTC[enduse,tech,ec,area,InitialYear] = 
        ...
        (1-PEEA[enduse,tech,ec,area,InitialYear]/PEM[enduse,tech,ec,area]))
  ...
  for area in areas, tech in Techs, enduse in Enduses
    @finite_math PFPN[enduse,tech,ec,area] = 
      -(PCCRN[enduse,tech,ec,area,Zero]+POCF[enduse,tech,ec,area])*
      xPCC[enduse,tech,ec,area,InitialYear]*PEM[enduse,tech,ec,area]*1000000/
      (PCTC[enduse,tech,ec,area,InitialYear]*(PEM[enduse,tech,ec,area]/
  ...
  for enduse in Enduses
    ...
    #
      @finite_math PCCN[enduse,tech,ec,area] = 
        abs(xPCC[enduse,tech,ec,area,InitialYear]/(PEM[enduse,tech,ec,area] /
            PEE[enduse,tech,ec,area,InitialYear]-1)^
            (1/PCTC[enduse,tech,ec,area,InitialYear]))     
      end
    else
...
function PEffAdjust(data,InitialYear,ec,ecc)
  ...
  for enduse in Enduses, tech in Techs
    ...
    @finite_math AvPEM[enduse,tech,ec] = sum(PEM[enduse,tech,ec,area]*
  ...
  for area in areas
    if Driver[ecc,area,InitialYear] < 0.0001
      for enduse in Enduses, tech in Techs
        ...
        PEM[enduse,tech,ec,area] = AvPEM[enduse,tech,ec]
  ...
  #
  # Use highest PEM for Techs with no historical data
  #
  areas = Select(Area,!=("MX"))
  for area in areas, ec in ECs, enduse in Enduses
    for tech in Techs
      PEMTech[tech] = PEM[enduse,tech,ec,area]
    end
    MaxValue, MaxTech = findmax(PEMTech)
    for tech in Techs
      if PEM[enduse,tech,ec,area] == 0.0
      
        PEM[enduse,tech,ec,area] = PEM[enduse,MaxTech,ec,area]/
          PDif[enduse,MaxTech,ec,area]*PDif[enduse,tech,ec,area]
          
        POCF[enduse,tech,ec,area] = POCF[enduse,MaxTech,ec,area]
        PCTC[enduse,tech,ec,area,InitialYear] = PCTC[enduse,MaxTech,ec,area,InitialYear]
        PFTC[enduse,tech,ec,area,InitialYear] = PCTC[enduse,MaxTech,ec,area,InitialYear]
        PFPN[enduse,tech,ec,area] = PFPN[enduse,MaxTech,ec,area]
        PCCN[enduse,tech,ec,area] = PCCN[enduse,MaxTech,ec,area]  
      end
    end
  end
...
function Pollution(data)
  ...
  for area in Areas, poll in Polls, tech in Techs, ec in ECs, fuelep in FuelEPs, enduse in Enduses
    POCA[enduse,fuelep,tech,ec,poll,area,Zero] = POCX[enduse,fuelep,tech,ec,poll,area,Zero]
  end
