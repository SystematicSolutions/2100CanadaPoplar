#
# TCalib.jl - Transportation Calibration Segment
#
# Write (" TCalib.jl, Transportation Calibration Segment")
#
...
Base.@kwdef struct Data
  ...
  DmdFEPTech::VariableArray{4} = ReadDisk(db,"$Outpt/DmdFEPTech",year) # [FuelEP,Tech,EC,Area,Year] Energy Demands (TBtu/Yr)
  DmdFEPTechPrior::VariableArray{4} = ReadDisk(db,"$Outpt/DmdFEPTech",prior) # Energy Demands (TBtu/Yr) [FuelEP,Tech,EC,Area]
  DmdFuelTech::VariableArray{5} = ReadDisk(db,"$Outpt/DmdFuelTech",year) # [Enduse,Fuel,Tech,EC,Area,Year] Energy Demands (TBtu/Yr)
  ...
  PCost::VariableArray{4} = ReadDisk(db,"$Outpt/PCost",year) # [Tech,EC,Poll,Area,Year] Permit Cost ($/Tonne)
  PCostExo::VariableArray{4} = ReadDisk(db,"$Input/PCostExo",year) # [Tech,EC,Poll,Area,Year] Marginal Exogenous Permit Cost (Real $/Tonnes)
  ...
  PolMarginal::VariableArray{4} = ReadDisk(db,"$Outpt/PolMarginal",year) # [Tech,EC,Poll,Area,Year] Marginal Emissions (Tonnes/Yr)
  ...
  RM::VariableArray{4} = ReadDisk(db,"$Outpt/RM",year) # [Tech,EC,Poll,Area,Year] Reduction Multiplier (Tonnes/Tonnes)
...
function TPrice(data)
  ...
  (;FFPMap,FPCFSFuel,FPCFSLast,FPCFS,FPCFSNet,FPCFSObligated,FPCFSTech) = data
  ...
  for area in Areas, poll in Polls, ec in ECs, tech in Techs
    PolMarginal[tech,ec,poll,area] = 
      sum(POCX[enduse,fuelep,tech,ec,poll,area]*(1-ZeroFr[fuelep,poll,area])*
      RM[tech,ec,poll,area]*
      max(DmdFEPTechPrior[fuelep,tech,ec,area],1e-12) for enduse in Enduses, fuelep in FuelEPs)
  end
  ...
  for area in Areas, ec in ECs, tech in Techs
    ExpCP[tech,ec,area] = sum(PolMarginal[tech,ec,poll,area]*
      (PCost[tech,ec,poll,area]+PCostExo[tech,ec,poll,area])*
      Inflation[area] for poll in Polls)/1e6      
  end
  ...
  # FPCP
  #
  for area in Areas, ec in ECs, fuel in Fuels, ec in ECs
    @finite_math FPCP[fuel,ec,area] = sum(PCostTech[tech,ec,area]*
      DmdFuelTechPrior[enduse,fuel,tech,ec,area]*FPCPFrac[fuel,ec,area] for tech in Techs, enduse in Enduses)/
      sum(DmdFuelTechPrior[enduse,fuel,tech,ec,area]*FPCPFrac[fuel,ec,area] for tech in Techs, enduse in Enduses)
  end
...
function CapacityFactorCalibration(data)
  (;db,year,prior,CTime) = data
  (;CUF,Dmd,xDmd) = data

  #
  # Second pass for CUF due to historical conversions
  #
  @. @finite_math CUF = CUF/Dmd*xDmd
