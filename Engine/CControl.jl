#
# CControl.jl - Commercial energy demand control segment
#
...
import ...EnergyModel: Engine.CCalib
import ...EnergyModel: Engine.CFuture
import ...EnergyModel: Engine.CDemand
import ...EnergyModel: Engine.CLoad
...
Base.@kwdef struct DataCControl
  db::String
...
# ############################
# #
# # Write (" CControl.src, ",SectorDS::0," Load Control")
# #
# DBOpen
# ProcSw=xProcSw
# #
# Open Segment "CLoad.xeq"
# Read Segment LOAD
# #
# DBClose
...
function History(db,YEndTime)
  ...
  for year in years
    ...
    CCalib.Initial(CCalib.Data(;db,year,prior,next,CTime))
  end
  ...
  while CalibPass <= 2
    ...
    while CTime <= YEndTime
      ...
      CDemand.Control(CDemand.Data(;db,year,prior,next,CTime,SceName,ProcSw))

      @info "$SectorName Control.jl - History - Call CalDEMM $CTime"
      CCalib.CalDEMM(CCalib.Data(;db,year,prior,next,CTime))
      ...
      CCalib.CalPEMM(CCalib.Data(;db,year,prior,next,CTime))
      ...
      CCalib.Coefficients(CCalib.Data(;db,year,prior,next,CTime),CalibPass)    
...
function CLoadCalib(db,RunTime,CalSw)
  data = DataCControl(; db)
  (;db) = data
  (;PI) = data  
  (;Endogenous,NonExist,ProcSw,xProcSw) = data
  
  @info "CControl - CLoadCalib - Loadcurve Calibration"
  ...
  if xProcSw[Loadcurve] != NonExist
    ...
    if CalSw == False
      @info "CControl.jl - CLoadCalib - LoadShape"
      #CCalib.LoadShape(CCalib.DataLoadShape(db))
      CCalib.LoadShape(db)      
    else
      @info "CControl.jl - CLoadCalib - Normalize"   
      #
      # CCalib.Normalize(CCalib.DataLoadShape(db))
      #
      CCalib.Normalize(db)
    end
    ...
    while CTime <= RunTime
      ...
      @info "CControl.jl - CLoadCalib - CLoad - Control - $CTime"
...
function Calib(db)
  ...
  @info "CControl - Calib - Demand Calibration"
