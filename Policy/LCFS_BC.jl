...
Base.@kwdef struct TControl
  ...
  # Scratch Variables
  #
  BDTarget::VariableArray{1} = zeros(Float32,length(Year))
...
function TPolicy(db)
  ...
  # start blending in 2024 ramping to 10% by 2030
  #
  ...
  for year in years
    ETTarget[year] = 
      ETTarget[year-1]+(ETTarget[Yr(2030)]-ETTarget[Last])/(Yr(2030)-Last)
  end
  ...
  for year in years
    BDTarget[year] =
      BDTarget[year-1]+(BDTarget[Yr(2030)]-BDTarget[Last])/(Yr(2030)-Last)
  end
