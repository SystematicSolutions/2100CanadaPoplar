...
function ResPolicy(db)
  ...
  for year in years
    EEImprovement[BC,year] = 0.908
  end

  ON = Select(Area,"ON")
  years = collect(Yr(2030):Final)
  for year in years
    EEImprovement[ON,year] = 0.647
  end

  QC = Select(Area,"QC")
  years = collect(Yr(2030):Final)
  for year in years
    EEImprovement[QC,year] = 0.275
  end
  
  areas1 = Select(Area,["AB","MB"])
  areas2 = Select(Area,(from="SK",to="NU"))
  areas = union(areas1,areas2)
  for year in years, area in areas
    EEImprovement[area,year] = 0.22
  end
  ...
  years = collect(Yr(2030):Final)
  for tech in Techs, ec in ECs, year in years
    PEMM[Heat,tech,ec,ON,year] = max(PEMM[Heat,tech,ec,ON,year],
      PEMMBase[Heat,tech,ec,ON,Yr(2009)]*(1+EIImprovement[ON,year]))
    PEStdP[Heat,tech,ec,ON,year] = max(PEStd[Heat,tech,ec,ON,year],
      PEStdP[Heat,tech,ec,ON,year],min(PEM[Heat,ec,ON]*PEMM[Heat,tech,ec,ON,year]*
        0.98,PEEBase[Heat,tech,ec,ON,Yr(2009)]*(1+EIImprovement[ON,year])))
  end

  QC = Select(Area,"QC")
  years = collect(Yr(2030):Final)
  for tech in Techs, ec in ECs, year in years
    PEMM[Heat,tech,ec,QC,year] = max(PEMM[Heat,tech,ec,QC,year],
      PEMMBase[Heat,tech,ec,QC,Yr(2012)]*(1+EIImprovement[QC,year]))
    PEStdP[Heat,tech,ec,QC,year] = max(PEStd[Heat,tech,ec,QC,year],
      PEStdP[Heat,tech,ec,QC,year],min(PEM[Heat,ec,QC]*PEMM[Heat,tech,ec,QC,year]*
        0.98,PEEBase[Heat,tech,ec,QC,Yr(2012)]*(1+EIImprovement[QC,year])))
  end
...
function PolicyControl(db)
  @info "Res_BldgNZ.jl - PolicyControl"
  ResPolicy(db)
end
