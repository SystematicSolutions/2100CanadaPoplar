...
module Electric_Patch_Exports_AM
...
function ElecPolicy(db)
  ...
  for year in years 
    xLLVC[AB,SK,year]=xLLVC[AB,SK,year]*4 #3 #2
  end 
  years = collect(Yr(2027):Yr(2027))
  for year in years 
    xLLVC[AB,SK,year]=xLLVC[AB,SK,year]*5 #4 #3 #2
  end 
  ...
  years = collect(Yr(2027):Yr(2027))
  for year in years
    xLLVC[ON,QC,year]=xLLVC[ON,QC,year]*4.5 #3.5 #3 #4
  end
  ...
  years = collect(Yr(2029):Yr(2029))
  for year in years
    xLLVC[ON,MISW,year] = xLLVC[ON,MISW,year]*3.5 #3 #2
  end
...
function PolicyControl(db)
  @info "Electric_Patch_Exports_AM.jl - PolicyControl"
  ElecPolicy(db)
end
