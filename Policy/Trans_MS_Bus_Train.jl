...
Base.@kwdef struct TControl
  ...
  # Scratch Variables
  BusTotal::VariableArray{2} = zeros(Float32,length(Area),length(Year)) # [Area,Year] Bus Market Share Total (Driver/Driver)
  EVBusShare::VariableArray{2} = zeros(Float32,length(Area),length(Year)) # [Area,Year] Bus Market Share for Policy Vehicles (Driver/Driver)
