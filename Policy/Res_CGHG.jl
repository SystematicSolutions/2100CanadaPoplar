...
function AllocateReduction(data::RControl,year,DmdTotal,areas,ecs,techs,enduses)
  (; ECs) = data
...
function ResPolicy(db)
  data = RControl(; db)
  (; Input,Outpt) = data
  (; ECs,Enduse,Nation,Techs) = data 
  
  (; Adjustment,ANMap,Dmd,Expenses) = data
  ...
  #  
  ReductionAdditional[Yr(2024)] = 2.6
  ReductionAdditional[Yr(2025)] = 5.3
  ReductionAdditional[Yr(2026)] = 8.0
  ...
  for year in years
    AllocateReduction(data,year,DmdTotal,areas,ECs,Techs,enduses)
  end
