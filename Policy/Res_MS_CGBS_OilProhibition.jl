#
# Res_CGBS_FossilFuelHeatingProhibition.jl - based on 'EL_Bldg_HPs.txp' - Policy Targets for FuelShares - Jeff Amlin 5/10/16
#
# This policy simulates the prohibition of fossil fuel heating as part
# of the Buildings Sector Strategy.  he prohibition is implemented into 
# software code by setting to zero the Marginal Market Share Fraction MMSF
# of the Space heater technolgies "Coal", "Gas", "LPG", and "Oil" .
# Only provinces are affects; territories are exempt.
# Last updated by Kevin Palmer-Wilson on 2023-03-22
#
using EnergyModel

module Res_MS_CGBS_OilProhibition
...
function PolicyControl(db)
  @info "Res_MS_CGBS_OilProhibition - PolicyControl"
  data = RControl(; db)
  (; CalDB,Outpt) = data
  ...
  for year in years, area in areas, ec in ECs, enduse in enduses
    xMMSF[enduse,heatpump,ec,area,year] = 
  ...
  #
  # 5 - Use 15+5 years for Res lifespan
  #
  BC = Select(Area,"BC")
  enduses=Select(Enduse,["Heat","HW"])
