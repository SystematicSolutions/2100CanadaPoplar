...
Base.@kwdef struct IControl
  ...
  # Scratch Variables
  Reduce::VariableArray{3} = zeros(Float32,length(EC),length(Poll),length(Year)) # [EC,Poll,Year] Scratch Variable For Input Reductions
end
...
Base.@kwdef struct TControl
  ...
  # Scratch Variables
  Reduce::VariableArray{3} = zeros(Float32,length(Tech),length(Poll),length(Year)) # [Tech,Poll,Year] Scratch Variable For Input Reductions
end
