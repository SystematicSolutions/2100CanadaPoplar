...
Base.@kwdef struct EControl
  ...
  UnPolRef::VariableArray{4} = ReadDisk(BCNameDB,"EGOutput/UnPol") # [Unit,FuelEP,Poll,Year] Pollution in Reference Case (Tonnes) 
...
function ElecPolicy(db)
  data = EControl(; db)
    ...
    (; ReReductionsAB,UnArea,UnCogen,UnCoverage) = data
  ...
  for unit in Units
    if (UnPlant[unit] !== "Null") && (UnArea[unit] !== "Null") && (UnSector[unit] !== "Null")
      plant,area,ecc = GetUnitSets(data,unit)
      for year in years, poll in polls
        if (UnCoverage[unit,poll,year] == 1) &&
           (AreaMarket[area,market,year] == 1) &&
           (ECCMarket[ecc,market,year] == 1)
