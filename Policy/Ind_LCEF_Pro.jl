...
Base.@kwdef struct IControl
  ...
  # Scratch Variables
  AnnualAdjustment::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [Area,Year] Adjustment for energy savings rebound
  ...
  DmdTotal::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [Area,Year] Total Demand (TBtu/Yr)
  Expenses::VariableArray{1} = zeros(Float32,length(Year)) # [Year] Program Expenses (2015 CN$M)
  FractionRemovedAnnually::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [Area,Year] Fraction of Energy Requirements Removed (Btu/Btu)
  PolicyCost::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [Area,Year] Policy Cost ($/TBtu)
  ReductionAdditional::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [Area,Year] Demand Reduction from this Policy Cumulative over Years (TBtu/Yr)
  ReductionTotal::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [Area,Year] Demand Reduction from this Policy Cumulative over Years (TBtu/Yr)
end
