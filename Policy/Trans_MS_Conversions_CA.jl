...
Base.@kwdef struct TControl
  ...
  CFrac::VariableArray{6} = zeros(Float32,length(Enduse),length(Tech),length(CTech),length(EC),length(Area),length(Year)) # [Enduse,Tech,CTech,EC,Area,Year] Fraction of Production Capacity open to Conversion ($/$)
  CMSM0Max::VariableArray{5} = zeros(Float32,length(Enduse),length(CTech),length(EC),length(Area),length(Year)) # [Enduse,CTech,EC,Area,Year] Maximum Conversion Non-Price Factor ($/$)
  DPLGoal::VariableArray{5} = zeros(Float32,length(Enduse),length(Tech),length(EC),length(Area),length(Year)) # [Enduse,Tech,EC,Area,Year] Physical Life of Equipment Goal (Years)
end
...
function TransPolicy(db)
  ...
  for year in years, tech in techs, enduse in Enduses
    DPLGoal[enduse,tech,Passenger,CA,year] = DPL[enduse,tech,Passenger,CA,year]
  end
