...
Base.@kwdef struct IControl
  ...
  PInvExo::VariableArray{5} = ReadDisk(db,"$Input/DInvTechExo") # [Enduse,Tech,EC,Area,Year] Process Exogenous Investments (M$/Yr)
  ...
  PolicyCost::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [EC,Area,Year] Policy Cost ($/TBtu)
...
function IndPolicy(db)
  ...
  WriteDisk(db,"$Input/DInvTechExo",PInvExo)
