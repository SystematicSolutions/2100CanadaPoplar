...
function DividePolicyCosts(db)
  ...
  (; DInvTechExo,PolicyCost,xInflation) = data
  ...
  for year in years
    DInvTechExo[Heat,Coal,Cement,QC,year] = DInvTechExo[Heat,Coal,Cement,QC,year]+
  ...
  Gas = Select(Tech,"Gas")
  for year in years
    DInvTechExo[Heat,Gas,Food,QC,year] = DInvTechExo[Heat,Gas,Food,QC,year]+
      (PolicyCost[QC,year]/xInflation[QC,year])/2
  end
