#
# Ind_H2_Feedstocks_RefA.jl
#

using EnergyModel

module Ind_H2_Feedstocks_RefA
...
function IndPolicy(db)
  ...
  # TODOPromula: Check that ECCC really wants all the years selected for line below - Ian 08/25/25
  for ec in ecs, fuel in fuels, year in Years
    FsFracMax[fuel,Gas,ec,AB,year] = xFsFrac[fuel,Gas,ec,AB,year]
    FsFracMin[fuel,Gas,ec,AB,year] = xFsFrac[fuel,Gas,ec,AB,year]
  end
  ...
  for year in years
    ...
    xFsFrac[Hydrogen,     Gas,Petroleum,        AB,year] = 0.9925
    xFsFrac[NaturalGas,   Gas,Petroleum,        AB,year] = 0.0000
    xFsFrac[NaturalGasRaw,Gas,Petroleum,        AB,year] = 0.0000
    xFsFrac[RNG,          Gas,Petroleum,        AB,year] = 0.0000
    xFsFrac[StillGas,     Gas,Petroleum,        AB,year] = 0.0000
  
    xFsFrac[Hydrogen,     Gas,OilSandsUpgraders,AB,year] = 0.3574
    xFsFrac[NaturalGas,   Gas,OilSandsUpgraders,AB,year] = 0.6426
    xFsFrac[NaturalGasRaw,Gas,OilSandsUpgraders,AB,year] = 0.0000
    xFsFrac[RNG,          Gas,OilSandsUpgraders,AB,year] = 0.0000
    xFsFrac[StillGas,     Gas,OilSandsUpgraders,AB,year] = 0.0000
  end
  ...
  xFsFrac[Hydrogen,tech,ec,area,Yr(2025)] = 0.7069
  xFsFrac[NaturalGas,tech,ec,area,Yr(2025)] = 0.2931
  ...
  # Simulates demand from the Braya facility in NL.
  # This facility falls under the OtherChemicals NAICS classification
  area = Select(Area,"AB")
  ...
  # Set 2025 fractions for QC facility
  xFsFrac[Hydrogen,tech,ec,area,Yr(2028)] = 0.1029
  xFsFrac[NaturalGas,tech,ec,area,Yr(2028)] = 0.8969
  xFsFrac[NaturalGasRaw,tech,ec,area,Yr(2028)] = 0.0000
  xFsFrac[RNG,tech,ec,area,Yr(2028)] = 0.0000
  xFsFrac[StillGas,tech,ec,area,Yr(2028)] = 0.0000
  xFsFrac[LPG,tech,ec,area,Yr(2028)] = 0.0000
  
  # Apply 2025 values through 2030
  years = collect(Yr(2029):Yr(2030))
  for year in years, fuel in fuels
    xFsFrac[fuel,tech,ec,area,year] = xFsFrac[fuel,tech,ec,area,Yr(2028)]
  end
...
function PolicyControl(db)
  @info "Ind_H2_Feedstocks_RefA.jl - PolicyControl"
  IndPolicy(db)
end
