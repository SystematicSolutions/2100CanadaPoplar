...
Base.@kwdef struct IControl
  ...
  # Scratch Variables
  AnnualAdjustment::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [EC,Year] Adjustment for energy savings rebound
  ...
  DmdTotal::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [EC,Year] Total Demand (TBtu/Yr)
  Expenses::VariableArray{1} = zeros(Float32,length(Year)) # [Year] Program Expenses (2015 CN$M)
  FractionRemovedAnnually::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [EC,Year] Fraction of Energy Requirements Removed (Btu/Btu)
  ...
  ReductionAdditional::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [EC,Year] Demand Reduction from this Policy Cumulative over Years (TBtu/Yr)
  ReductionTotal::VariableArray{3} = zeros(Float32,length(EC),length(Area),length(Year)) # [EC,Year] Demand Reduction from this Policy Cumulative over Years (TBtu/Yr)
end
...
function IndPolicy(db::String)
  ...
  #  
  for year in years
    ReductionAdditional[ecs,areas,year] = ReductionAdditional[ecs,areas,year]/KJBtu*AnnualAdjustment[ecs,areas,year]
  end
