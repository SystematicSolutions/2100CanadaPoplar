...
Base.@kwdef struct EControl
  ...
  Holyrood::VariableArray{1} = zeros(Float32,length(Year)) # [Year] Gratis Permits for Holyrood unit in NL (Tonnes/Yr)
...
function ElecPolicy(db)
  ...
  (; ExYear,FacSw,FBuyFr,GoalPolSw,GPEUSw,GratSw,Holyrood,ISaleSw) = data
  ...
  years = collect(Yr(2020):Yr(2022))
  eccs =  Select(ECC,["HeavyOilMining","OtherNonferrous"])
  for area in areas, pcov in PCovs, poll in polls, ecc in eccs, year in years
  ...
  # TODOJulia: Line below is different than problem version - Ian 06/12/25
  # Select Year(2019-2030)
  # OBAFraction(ECC,Area,Y)=0.94-0.02*(Yrv(Y)-2019)
  ...
  for area in areas 
    ...
    OBAFraction[HeavyOilMining,area,Yr(2021)] = 1.0
    OBAFraction[HeavyOilMining,area,Yr(2022)] = 1.0
    OBAFraction[HeavyOilMining,area,Yr(2023)] = 0.92
    OBAFraction[HeavyOilMining,area,Yr(2024)] = 0.87
  end
