...
Base.@kwdef struct TControl
  ...
  PEMBase::VariableArray{4} = ReadDisk(OGRefNameDB,"$CalDB/PEM")        # [Enduse,Tech,EC,Area] Process Efficiency ($/Btu)
...
function InputData(data::TControl,enduse,ec,area,years)
  (; Areas,ECs) = data
  (; Enduses) = data
  (; Techs) = data
  ...
  for tech in Techs, year in years
    @finite_math PEELocal[enduse,tech,ec,area,year] = max(PEMBase[enduse,tech,ec,area]*
