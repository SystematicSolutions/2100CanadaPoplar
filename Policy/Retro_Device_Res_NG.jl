...
Base.@kwdef struct RControl
  ...
  DERRRExoTotal::VariableArray{1} = zeros(Float32,length(Year)) # [Year] Total Device Energy Removed (mmBtu/Yr)
  ...
  DERRemoved::VariableArray{5} = zeros(Float32,length(Enduse),length(Tech),length(EC),length(Area),length(Year)) # [Enduse,Tech,EC,Area,Year] Policy-specific Device Energy Removed ((mmBtu/Yr)/Yr)
  DERRemovedTotal::VariableArray{2} = zeros(Float32,length(Tech),length(Year)) # [Tech,Year] Policy-specific Total Device Energy Removed (mmBtu/Yr)
...
function ResPolicy(db)
  data = RControl(; db)
  (; Input) = data
  (; Area,ECs,Enduse,Tech,Techs,Years) = data
  (; AnnualAdjustment) = data
  (; DERRemoved,DERRemovedTotal,DInvTechExo) = data
  (; Expenses,Increment,Reduction) = data
  ...
  Reduction[tech, years] = [
    # 2024    2025    2026    2027    2028    2029    2030    2031    2032    2033    2034    2035    2036    2037    2038    2039    2040    2041    2042    2043    2044    2045    2046    2047    2048    2049    2050
       0.3     0.6     0.9     1.2     1.6     1.9     1.8     1.9     2.0     2.1     2.2     2.3     2.4     2.5     2.6     2.7     2.8     2.9     3.0     3.1     3.2     3.3     3.5     3.6     3.7     3.8     4.0
    ]
  ...
  Expenses[tech, years] = [
    # 2024    2025    2026    2027    2028    2029    2030    2031    2032    2033    2034    2035    2036    2037    2038    2039    2040    2041    2042    2043    2044    2045    2046    2047    2048    2049    2050
     12.8    13.4    14.1    14.7    15.0    15.3    15.6    15.9    16.2    16.6    16.9    17.2    17.6    17.9    18.3    18.7    19.0    19.4    19.8    20.2    20.6    21.0    21.4    21.9    22.3    22.7    23.2
  ]
  for year in years
  ...
  for year in years, ec in ECs, eu in enduses
    @finite_math DInvTechExo[eu,tech,ec,area,year] = DInvTechExo[eu,tech,ec,area,year]+
