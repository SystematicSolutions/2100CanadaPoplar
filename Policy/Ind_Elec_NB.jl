...
function IndPolicy(db)
  ...
  Reduction[tech, years] = [
    # 2024    2025    2026    2027    2028    2029    2030    2031    2032    2033    2034    2035    2036    2037    2038    2039    2040    2041    2042    2043    2044    2045    2046    2047    2048    2049    2050
      0.11    0.23    0.36    0.54    0.68    0.84    1.02    1.20    1.36    1.53    1.71    1.89    1.94    2.25    2.44    2.64    2.84    3.04    3.25    3.25    3.25    3.25    3.25    3.25    3.25    3.25    3.25
    ]
  #! format: on
  ...
  years = collect(Yr(2029):Yr(2032))
  for year in years
    Increment[tech,year] = 0.20
  end

  years = collect(Yr(2033):Yr(2043))
  for year in years
    Increment[tech,year] = 0.10
  end

    years = collect(Yr(2044):Final)
  for year in years
    Increment[tech,year] = 0.35
  end
...
function PolicyControl(db)
  @info "Ind_Elec_NB.jl - PolicyControl"
  IndPolicy(db)
end
