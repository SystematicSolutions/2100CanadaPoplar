...
Base.@kwdef struct IControl
  ...
  # Scratch Variables
  Change::VariableArray{1} = zeros(Float32,length(Year)) # [Year] Change in Policy Variable
  Temp::VariableArray{5} = zeros(Float32,length(ECC),length(Poll),length(Area),length(FuelEP),length(Year)) # [ECC,Poll,Area,FuelEP,Year] Scratch Variable for Input
  TempCost::VariableArray{3} = zeros(Float32,length(ECC),length(Area),length(Year)) # [ECC,Area,Year] Scratch Input Variable
  TempProcess::VariableArray{4} = zeros(Float32,length(ECC),length(Poll),length(Area),length(Year)) # [ECC,Poll,Area,Year] Scratch Variable for Input for Process
end
...
function IndPolicy(db)
  ...
  ApplyToCogeneration(data,fueleps,NOX,Yr(2024):Final,eccs,AB)
  ...
  ApplyToCogeneration(data,fueleps,NOX,Yr(2024):Final,eccs,BC)
