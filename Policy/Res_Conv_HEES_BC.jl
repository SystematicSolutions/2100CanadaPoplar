#
# Res_Conv_HEES_BC.jl - based on ResCom_MS_Conversions.txp
#
...
function ResPolicy(db)
  ...
  #
  # 5 - Use 15+5 years for Res lifespans
  #
  enduses=Select(Enduse,["Heat","HW"])
  techs=Select(Tech,["Oil","Gas"])
  years = collect(Yr(2030):Yr(2050))
  for year in years, ec in ECs, enduse in enduses, tech in techs
    DPL[enduse,tech,ec,area,year] = 20
  end
...
function PolicyControl(db)
  @info "Res_Conv_HEES_BC.jl - PolicyControl"
  ResPolicy(db)
end
