...
Base.@kwdef struct CControl
  ...
  DmdTotal::VariableArray{4} = zeros(Float32,length(Enduse),length(Tech),length(Area),length(Year)) # [Enduse,Tech,Area,Year] Total Demand (TBtu/Yr)
...
function ComPolicy(db::String)
  ...
  (; DInvTechExo,DmdFrac,DmdRef,DmdTotal,PolicyCost,xInflation,xMMSF) = data
  ...
  for year in years
    ...
    for ec in ECs
      ...
      DInvTechExo[enduse,HeatPump,ec,area,year] = DInvTechExo[enduse,HeatPump,ec,area,year]+PolicyCost[year]/
