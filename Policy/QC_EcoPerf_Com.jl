...
Base.@kwdef struct CControl
  ...
  DmdTotal::VariableArray{1} = zeros(Float32,length(Year)) # Total Demand (TBtu/Yr)
  ...
  PERRemoved::VariableArray{5} = zeros(Float32,length(Enduse),length(Tech),length(EC),length(Area),length(Year)) # [Enduse,Tech,EC,Area,Year] Process Energy Removed ((mmBtu/Yr)/Yr)
  PERRemovedTotal::VariableArray{1} = zeros(Float32,length(Year)) # [Year] Total Process Energy Removed (mmBtu/Yr)
...
function ComPolicy(db)
  ...
  #  TODO Promula: This policy overwrites PERRRExo and should be additive as 
  # commented out below. PNV - 5/19/2025
  for enduse in enduses, tech in techs, ec in ECs, year in years
    ...
    # PER[enduse,tech,ec,area]*FractionRemovedAnnually[year]
    PERRRExo[enduse,tech,ec,area,year] = PER[enduse,tech,ec,area]*FractionRemovedAnnually[year]
  end
  ...
  # Program Costs
  #     
  years = Select(Year,(from = "2023", to = "2030"))
  Expenses[years] = [
