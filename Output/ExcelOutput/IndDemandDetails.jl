...
Base.@kwdef struct IndDemandDetailsData
  db::String
  Input = "IInput"
  Outpt = "IOutput"
...
function IndDemandDetails_DtaRun(data,area,ec,)
  ...
  println(iob)
  println(iob,"This file was produced by IndDemandDetails.jl")
  println(iob)
  ...
  filename = "IndDemandDetails-$(AreaKey[area])-$(ECCKey[ecc])-$SceName.dta"
...
function IndDemandDetails_DtaControl(db)
  @info "IndDemandDetails_DtaControl"
  data = IndDemandDetailsData(; db)
  (; Area, EC, ECC) = data
  ...
  for ec in ecs
    for area in areas
      IndDemandDetails_DtaRun(data,area,ec)
    end
  end
...
IndDemandDetails_DtaControl(DB)
end
