...
Base.@kwdef struct ResInputsDisplayData
  db::String
  Input = "RInput"
  Outpt = "ROutput"
  CalDB = "RCalDB"
...
function ResInputsDisplay_DtaRun(data, area, ec)
  ...
  println(iob, " ")
  println(iob, "This file was produced by ResInputsDisplay.jl")
  println(iob, " ")
...
#    end
  ...
  filename = "ResInputsDisplay-$(AreaKey[area])-$(ECCKey[ecc])-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function ResInputsDisplay_DtaControl(db)
  @info "ResInputsDisplay_DtaControl"
  data = ResInputsDisplayData(; db)
  Area = data.Area
  EC = data.EC
  #
  areas = Select(Area,["ON","QC","BC","MB","CA","ENC","MX","ROW"])
  ecs = Select(EC, ["SingleFamilyDetached","MultiFamily"])
  for ec in ecs
    for area in areas
      ResInputsDisplay_DtaRun(data, area, ec)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
ResInputsDisplay_DtaControl(DB)
end
