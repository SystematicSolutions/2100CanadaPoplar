...
Base.@kwdef struct ComInputsDisplayData
  db::String
  Input = "CInput"
  Outpt = "COutput"
  CalDB = "CCalDB"
...
function ComInputsDisplay_DtaRun(data, area, ec)
  ...
  println(iob, " ")
  println(iob, "This file was produced by ComInputsDisplay.jl")
  println(iob, " ")
...
#    end
  ...
  filename = "ComInputsDisplay-$(AreaKey[area])-$(ECCKey[ecc])-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function ComInputsDisplay_DtaControl(db)
  @info "ComInputsDisplay_DtaControl"
  data = ComInputsDisplayData(; db)
  Area = data.Area
  EC = data.EC
  #
  areas = Select(Area,["ON","QC","AB","BC","MB","CA","ENC","MX","ROW"])
  ecs = Select(EC,["Wholesale","Offices"])
  for ec in ecs
    for area in areas
      ComInputsDisplay_DtaRun(data, area, ec)
    end
  end
end
if abspath(PROGRAM_FILE) == @__FILE__
ComInputsDisplay_DtaControl(DB)
end
