...
Base.@kwdef struct TransVintagesData
  db::String
  Input = "TInput"
  Outpt = "TOutput"
  ...
  TechKey::SetArray   = ReadDisk(db,"$Input/TechKey")
...
function TransVintages_DtaRun(data,area,ec,tech,enduse)
  (; SceName,Year,AgeDS,Ages,Area,AreaDS,AreaKey,EC,ECDS,ECC,ECCDS,ECCKey) = data
  (; EnduseDS,Enduses,TechDS,TechKey,Techs,VintageDS,Vintages,DERV) = data
  ...
  println(iob)
  println(iob,"This file was produced by TransVintages.jl")
  println(iob)
  ...
  filename = "TransVintages-$(AreaKey[area])-$(ECCKey[ecc])-$(TechKey[tech])-$SceName.dta"
...
function TransVintages_DtaControl(db)
  @info "TransVintages_DtaControl"
  data = TransVintagesData(; db)
  (; Area,EC,ECC,Enduse,Tech) = data
  ...
  for area in areas
    TransVintages_DtaRun(data,area,ec,tech,enduse)
  end
end
if abspath(PROGRAM_FILE) == @__FILE__
TransVintages_DtaControl(DB)
end
