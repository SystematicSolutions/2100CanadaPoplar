...
Base.@kwdef struct UnitGenerationData
  db::String
  ...
  UnHRt::VariableArray{1} = ReadDisk(db, "EGInput/UnHRt", Yr2005) #[Unit,Year]  Heat Rate (BTU/KWh)
...
function UnitGeneration_DtaRun(data)
  (; SceName,Unit,Year, Units) = data
  (; UnCode,UnCogen,UnEAF,UnGC,UnGCYLoc,UnGenCo,UnHRt) = data
  (; UnName,UnNode,UnOR,UnOwner,UnPlant,UnEGA,xUnEGA,ZZZ) = data
  ...
  filename = "UnitGeneration-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function UnitGeneration_DtaControl(db)
  @info "UnitGeneration_DtaControl"
  data = UnitGenerationData(; db)

  UnitGeneration_DtaRun(data)

end


if abspath(PROGRAM_FILE) == @__FILE__
UnitGeneration_DtaControl(DB)
end
