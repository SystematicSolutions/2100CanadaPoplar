...
Base.@kwdef struct UnitMarginalUnitData
  db::String
...
function UnitMarginalUnit_DtaRun(data, node)
  ...
  print(iob, "Fuel;")
  print(iob, "Bid Price (2010 \$/MWH);")
  print(iob, "System Price (2010 \$/MWH);")
  print(iob, "Dispatched (MW);")
  print(iob, "Available (MW);")
  print(iob, "Variable Costs (2010 \$/MWH);")
  print(iob, "Fixed Costs (2010 \$/KW/Yr);")
  print(iob, "Fuel Costs (2010 \$/MWH);")
  print(iob, "Variable O&M Costs (2010 \$/MWH);")
  print(iob, "Emission Costs (2010 \$/MWH);")
  ...
  print(iob, "Capital Cost (2010 \$/Kw);")          
    ...
    for unit in SelectedUnits
      if UnEG[unit,timep,month,year] > 0.0
        if (UnPlant[unit] !="Null") && (UnGenCo[unit] !="Null")
          ...
          UnUOMC[unit,year]=UnUOMC[unit,year]/InflationUnit[unit,year]*InflationUnit[unit,Yr2010]
          UnUFOMC[unit,year]=UnUFOMC[unit,year]/InflationUnit[unit,year]*InflationUnit[unit,Yr2010]
          UnVCost[unit,timep,month,year]=UnVCost[unit,timep,month,year]/InflationUnit[unit,year]*InflationUnit[unit,Yr2010]
          UnAFC[unit,year]=UnAFC[unit,year]/InflationUnit[unit,year]*InflationUnit[unit,Yr2010]
          UnAVC[unit,year]=UnAVC[unit,year]/InflationUnit[unit,year]*InflationUnit[unit,Yr2010]
          UnFP[unit,month,year]=UnFP[unit,month,year]/InflationUnit[unit,year]*InflationUnit[unit,Yr2010]
          UnGCCC[unit,year]=UnGCCC[unit,year]/InflationUnit[unit,year]*InflationUnit[unit,Yr2010]
          HDPrA[node,timep,month,year]=HDPrA[node,timep,month,year]/InflationNode[node,year]*InflationNode[node,Yr2010]
  ...
  filename = "UnitMarginalUnit-$(Node[node])-$SceName.dta"
...
function RunYears(data, node)
  UnitMarginalUnit_DtaRun(data, node)
end

function UnitMarginalUnit_DtaControl(db)
  @info "UnitMarginalUnit_DtaControl"
  data = UnitMarginalUnitData(; db)
  (; Node) = data
...
UnitMarginalUnit_DtaControl(DB)
end
