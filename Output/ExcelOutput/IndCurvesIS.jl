...
Base.@kwdef struct IndCurvesISData
  db::String
...
function IndCurvesIS_DtaRun(data,nation,areas,areaName,areaKey,enduse,ecs,eccs)
  (; Area, AreaDS, Nation, EC, ECDS, ECC, ECCDS, Enduse, EnduseDS, Techs, TechDS, Year) = data
  (; CERSM,CUF,Dmd,Driver,ECFP,MCFU,PCC,PEE,PEEA,PEECurve,PEEPrice) = data
  (; ANMap, Inflation, CDTime, CDYear,SceName) = data
  ...
  filename = "IndCurvesIS-$AreaKeyOut-$EnduseKeyOut-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function IndCurvesIS_DtaControl(db)
  @info "IndCurvesIS_DtaControl"
  data = IndCurvesISData(; db)
  ...
  for area in areas
    ...
    for enduse in enduses
      IndCurvesIS_DtaRun(data,nations,area,areaName,areaKey,enduse,ecs,eccs)
    end
  end
  ...
  areaKey = "CN"
  for enduse in enduses
    IndCurvesIS_DtaRun(data,nations,areas,areaName,areaKey,enduse,ecs,eccs)
  end
end
if abspath(PROGRAM_FILE) == @__FILE__
IndCurvesIS_DtaControl(DB)
end
