...
function KPIAInd_DtaRun(data, area, ec)
  ...
  println(iob, " ")
  println(iob, "This file was produced by KPIAInd_DtaRun.jl")
  println(iob, " ")
  ...
  for enduse in Select(Enduse, "OffRoad")
    for tech in oil
  ...
  filename = "KPIAIndOutputs-$(AreaKey[area])-$(ECCKey[ecc])-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function KPIAIndOutputs_DtaControl(db)
  @info "KPIAIndOutputs_DtaControl"
  data = KPIAIndData(; db)
  Area = data.Area
  EC = data.EC
  ...
  for ec in ecs
    for area in areas
      KPIAInd_DtaRun(data, area, ec)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
KPIAIndOutputs_DtaControl(DB)
end
