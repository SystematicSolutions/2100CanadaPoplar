...
Base.@kwdef struct PollutionAllData
  db::String
...
function PollutionAll_DtaRun(data,polls,areas,AreaName,AreaKey,PollName,PolUnit)
  ...
  #
  # Include all sectors except Land Use
  #
  OfficialECCs = Select(ECC, !=("LandUse"))  
  CO2=Select(Poll,"CO2")
  ...
  #
  filename = "PollutionAll-$PollName-$AreaKey-$SceName.dta"
...
function Pollution_DtaControl(db)
  @info "PollutionAll_DtaControl"

  data = PollutionAllData(; db)
  ...
  for area in Select(Area, (from = "ON", to = "NU")), poll in polls
    ...
    PollutionAll_DtaRun(data,poll,area,AreaName,AreaKey,PollName,PolUnit)
  end
  ...
  for poll in polls
    PollName = PollKey[poll]
    PolUnit = PolUnits[poll]
    PollutionAll_DtaRun(data,poll,areas,AreaName,AreaKey,PollName,PolUnit)
  end
  ...
  for areas in Areas
    AreaName = AreaDS[areas]
    AreaKey = Area[areas]
    PollutionAll_DtaRun(data,polls,areas,AreaName,AreaKey,PollName,PolUnit)
  end
