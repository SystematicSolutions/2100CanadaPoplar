...
Base.@kwdef struct ResVintagesData
  db::String
  Input = "RInput"
  Outpt = "ROutput"
...
function ResVintages_DtaRun(data,area,ec)
  ...
  println(iob)
  println(iob,"This file was produced by ResVintages.jl")
  println(iob)
  ...
  filename = "ResVintages-$(AreaKey[area])-$(ECCKey[ecc])-$SceName.dta"
...
function ResVintages_DtaControl(db)
  @info "ResVintages_DtaControl"
  data = ResVintagesData(; db)
  (; Area, EC, ECC) = data
  areas = Select(Area,["ON","QC"])
  ecs = Select(EC,["SingleFamilyDetached","MultiFamily"])
  for ec in ecs
    for area in areas
      ResVintages_DtaRun(data,area,ec)
    end
  end
end
if abspath(PROGRAM_FILE) == @__FILE__
ResVintages_DtaControl(DB)
end
