...
Base.@kwdef struct ResDemandDetailsData
  db::String
  Input = "RInput"
  Outpt = "ROutput"
...
function ResDemandDetails_DtaRun(data,area,ec)
  ...
  println(iob)
  println(iob,"This file was produced by ResDemandDetails.jl")
  println(iob)
  ...
  filename = "ResDemandDetails-$(AreaKey[area])-$(ECCKey[ecc])-$SceName.dta"
...
function ResDemandDetails_DtaControl(db)
  @info "ResDemandDetails_DtaControl"
  data = ResDemandDetailsData(; db)
  (; Area, EC, ECC) = data
  areas = Select(Area,["ON","QC","BC","AB","MB","CA","ENC","MX","ROW"])
  ecs = Select(EC,["SingleFamilyDetached","MultiFamily"])
  for ec in ecs
    for area in areas
      ResDemandDetails_DtaRun(data,area,ec)
    end
  end
end
...
ResDemandDetails_DtaControl(DB)
end
