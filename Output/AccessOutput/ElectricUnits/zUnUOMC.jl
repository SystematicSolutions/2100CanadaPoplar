...
Base.@kwdef struct zUnUOMCData
  ...
  zUnUOMC::VariableArray{2} = ReadDisk(db, "EGInput/UnUOMC") #[Unit,Year]  Variable O&M Costs ($/Kw/Yr)
  zUnUOMCRef::VariableArray{2} = ReadDisk(RefNameDB, "EGInput/UnUOMC") #[Unit,Year]  Variable O&M Costs ($/Kw/Yr)
...
function zUnUOMC_AssignConversions(data)
  ...
  UnitsDS[US] = " US \$/MWh/Yr"
  UnitsDS[CN] = " CN \$/MWh/Yr"
  
end

function zUnUOMC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; CDTime,Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnUOMC,zUnUOMCRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnUOMC[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnUOMCRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnUOMC;",Year[year],";",OutputUnCode,";",CDTime,UnitsDS[nation],
...
function zUnUOMC_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnUOMC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnUOMC-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnUOMC_DtaRun

function zUnUOMC_DtaControl(db)
  data = zUnUOMCData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnUOMC_DtaControl"

  zUnUOMC_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnUOMC_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnUOMC_DtaControl(DB)
end
