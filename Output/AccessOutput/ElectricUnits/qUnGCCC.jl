...
Base.@kwdef struct qUnGCCCData
  ...
  qUnGCCC::VariableArray{2} = ReadDisk(db, "EGOutput/UnGCCC") #[Unit,Year]  Generating Unit Capital Cost ($/KW)
  qUnGCCCRef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnGCCC") #[Unit,Year]  Generating Unit Capital Cost ($/KW)
...
function qUnGCCC_AssignConversions(data)
...
function qUnGCCC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnGCCC,qUnGCCCRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnGCCC[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnGCCCRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnGCCC;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnGCCC_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnGCCC,qUnGCCCRef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnGCCC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnGCCC-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnGCCC_DtaRun

function qUnGCCC_DtaControl(db, SceName)
  data = qUnGCCCData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnGCCC_DtaControl"

  qUnGCCC_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnGCCC_DtaRun(data,nation,SceName)
    end
  end
end
