...
Base.@kwdef struct zUnNationData
  db::String
  ...
  zUnNation::Array{String} = ReadDisk(db,"EGInput/UnNation") # [Unit] Nation
  zUnNationRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnNation") # [Unit] Nation
...
function zUnNation_AssignConversions(data)
...
function zUnNation_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnNation,zUnNationRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnNation[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnNationRef[UnitReference]
      OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnNation;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnNation_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnNation,zUnNationRef) = data

  if BaseSw != 0
    @. zUnNationRef = zUnNation
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnNation_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnNation-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnNation_DtaRun

function zUnNation_DtaControl(db)
  data = zUnNationData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnNation_DtaControl"

  zUnNation_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnNation_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnNation_DtaControl(DB)
end
