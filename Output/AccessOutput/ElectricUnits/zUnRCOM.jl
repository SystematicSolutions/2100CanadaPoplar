...
Base.@kwdef struct zUnRCOMData
  ...
  zUnRCOM::VariableArray{2} = ReadDisk(db, "EGOutput/UnRCOM") # [Unit,Year] Emission Reduction O&M Costs (M$/Yr)
  zUnRCOMRef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnRCOM") # [Unit,Year] Emission Reduction O&M Costs (M$/Yr)
  #
...
function zUnRCOM_AssignConversions(data)
...
function zUnRCOM_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnRCOM,zUnRCOMRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnRCOM[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnRCOMRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnRCOM;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnRCOM_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnRCOM_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnRCOM-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnRCOM_DtaRun

function zUnRCOM_DtaControl(db)
  data = zUnRCOMData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnRCOM_DtaControl"

  zUnRCOM_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnRCOM_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnRCOM_DtaControl(DB)
end
