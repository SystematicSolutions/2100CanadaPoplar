...
Base.@kwdef struct zUUnEGAData
  ...
  zUUnEGA::VariableArray{2} = ReadDisk(db, "EGOutput/UUnEGA") #[Unit,Year]  Generation (GWh)
  zUUnEGARef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UUnEGA") #[Unit,Year]  Generation (GWh)
...
function zUUnEGA_AssignConversions(data)
...
function zUUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,zUUnEGA,zUUnEGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUUnEGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUUnEGARef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUUnEGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUUnEGA_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUUnEGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUUnEGA_DtaRun

function zUUnEGA_DtaControl(db)
  data = zUUnEGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUUnEGA_DtaControl"

  zUUnEGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUUnEGA_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUUnEGA_DtaControl(DB)
end
