...
Base.@kwdef struct zUnSectorData
  db::String
  ...
  zUnSector::Array{String} = ReadDisk(db, "EGInput/UnSector") # [Unit] Unit Type (Utility or Industry)
  zUnSectorRef::Array{String} = ReadDisk(RefNameDB, "EGInput/UnSector") # [Unit] Unit Type (Utility or Industry)
...
function zUnSector_AssignConversions(data)
...
function zUnSector_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnSector,zUnSectorRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnSector[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnSectorRef[UnitReference]
        OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnSector;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnSector_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnSector,zUnSectorRef) = data

  if BaseSw != 0
    @. zUnSectorRef = zUnSector
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnSector_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnSector-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnSector_DtaRun

function zUnSector_DtaControl(db)
  data = zUnSectorData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnSector_DtaControl"

  zUnSector_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnSector_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnSector_DtaControl(DB)
end
