...
Base.@kwdef struct zUnLimitedData
  db::String
  ...
  zUnLimited::VariableArray{2} = ReadDisk(db, "EGInput/UnLimited") #[Unit,Year]  Limited Energy Units Switch (Switch) (1=Limited Energy Unit)
  zUnLimitedRef::VariableArray{2} = ReadDisk(RefNameDB, "EGInput/UnLimited") #[Unit,Year]  Limited Energy Units Switch (Switch) (1=Limited Energy Unit)
...
function zUnLimited_AssignConversions(data)
...
function zUnLimited_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Nation,Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,zUnLimited,zUnLimitedRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnLimited[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnLimitedRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if UnNation[UnitPolicy] == Nation[nation]
        println(iob,"zUnLimited;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnLimited_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnLimited,zUnLimitedRef,SceName) = data

  if BaseSw != 0
    @. zUnLimitedRef = zUnLimited
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnLimited_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnLimited-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnLimited_DtaRun

function zUnLimited_DtaControl(db)
  data = zUnLimitedData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnLimited_DtaControl"

  zUnLimited_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnLimited_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnLimited_DtaControl(DB)
end
