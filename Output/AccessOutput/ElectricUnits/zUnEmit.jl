...
Base.@kwdef struct zUnEmitData
  ...
  zUnEmit::VariableArray{1} = ReadDisk(db,"EGInput/UnEmit") #[Unit]  Does this Unit Emit Pollution? (1=Yes)
  zUnEmitRef::VariableArray{1} = ReadDisk(db,"EGInput/UnEmit") #[Unit]  Does this Unit Emit Pollution? (1=Yes)
...
function zUnEmit_AssignConversions(data)
...
function zUnEmit_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  (; Nation,Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,UnNation,zUnEmit,zUnEmitRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
      ...
      if UnitPolicy != 0
        ZZZ = zUnEmit[UnitPolicy]*Conversion[nation]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnEmitRef[UnitReference]*Conversion[nation]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if UnNation[UnitPolicy] == Nation[nation]
        println(iob,"zUnEmit;",OutputUnCode,";",UnitsDS[nation],
...
function zUnEmit_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnEmit,zUnEmitRef,SceName) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnEmit_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
    end
  end
  ...
  filename = "zUnEmit-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnEmit_DtaRun

function zUnEmit_DtaControl(db)
  data = zUnEmitData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnEmit_DtaControl"

  zUnEmit_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnEmit_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnEmit_DtaControl(DB)
end
