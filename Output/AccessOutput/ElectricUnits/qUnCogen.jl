...
Base.@kwdef struct qUnCogenData
  db::String
  ...
  qUnCogen::VariableArray{1} = ReadDisk(db, "EGInput/UnCogen") #[Unit]  Industrial Self-Generation Flag (1=Self-Generation)
  qUnCogenRef::VariableArray{1} = ReadDisk(RefNameDB, "EGInput/UnCogen") #[Unit]  Industrial Self-Generation Flag (1=Self-Generation)
...
function qUnCogen_AssignConversions(data)
...
function qUnCogen_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnCogen,qUnCogenRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
      ...
      if UnitPolicy != 0
        ZZZ = qUnCogen[UnitPolicy]*Conversion[nation]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnCogenRef[UnitReference]*Conversion[nation]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnCogen;",OutputUnCode,";",UnitsDS[nation],
...
function qUnCogen_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnCogen,qUnCogenRef) = data

  if BaseSw != 0
    @. qUnCogenRef = qUnCogen
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnCogen_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  end
  ...
  filename = "qUnCogen-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnCogen_DtaRun

function qUnCogen_DtaControl(db, SceName)
  data = qUnCogenData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnCogen_DtaControl"

  qUnCogen_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnCogen_DtaRun(data,nation,SceName)
    end
  end
end
