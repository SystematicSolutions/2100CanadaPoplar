...
Base.@kwdef struct zUnOUREGData
  db::String
  ...
  zUnOUREG::VariableArray{2} = ReadDisk(db,"EGInput/UnOUREG") # [Unit,Year] Own Use Rate for Generation (GWh/GWh)
  zUnOUREGRef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/UnOUREG") # [Unit,Year] Own Use Rate for Generation (GWh/GWh)
...
function zUnOUREG_AssignConversions(data)
...
function zUnOUREG_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnOUREG,zUnOUREGRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if !isnothing(UnitPolicy)
        ZZZ = zUnOUREG[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if !isnothing(UnitReference)
        CCC = zUnOUREGRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnOUREG;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnOUREG_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnOUREG,zUnOUREGRef,SceName) = data

  if BaseSw != 0
    @. zUnOUREGRef = zUnOUREG
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnOUREG_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnOUREG-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnOUREG_DtaRun

function zUnOUREG_DtaControl(db)
  data = zUnOUREGData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnOUREG_DtaControl"

  zUnOUREG_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnOUREG_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnOUREG_DtaControl(DB)
end
