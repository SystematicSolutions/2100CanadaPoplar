...
Base.@kwdef struct qUnPCFMaxData
  db::String
  ...
  qUnPCFMax::VariableArray{2} = ReadDisk(db,"EGInput/UnPCFMax") #[Unit,Year]  Unit Capacity Factor (MW/MW)
  qUnPCFMaxRef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/UnPCFMax") #[Unit,Year]  Unit Capacity Factor (MW/MW)
...
function qUnPCFMax_AssignConversions(data)
...
function qUnPCFMax_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnPCFMax,qUnPCFMaxRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnPCFMax[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnPCFMaxRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnPCFMax;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnPCFMax_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnPCFMax,qUnPCFMaxRef) = data

  if BaseSw != 0
    @. qUnPCFMaxRef = qUnPCFMax
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnPCFMax_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnPCFMax-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnPCFMax_DtaRun

function qUnPCFMax_DtaControl(db, SceName)
  data = qUnPCFMaxData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnPCFMax_DtaControl"

  qUnPCFMax_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnPCFMax_DtaRun(data,nation,SceName)
    end
  end
end
