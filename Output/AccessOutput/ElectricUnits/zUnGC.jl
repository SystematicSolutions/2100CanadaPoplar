...
Base.@kwdef struct zUnGCData
  ...
  zUnGC::VariableArray{2} = ReadDisk(db,"EGOutput/UnGC") #[Unit,Year]  Generating Capacity (MW)
  zUnGCRef::VariableArray{2} = ReadDisk(RefNameDB,"EGOutput/UnGC") #[Unit,Year]  Generating Capacity (MW)
...
function zUnGC_AssignConversions(data)
...
function zUnGC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnGC,zUnGCRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnGC[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnGCRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnGC;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnGC_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnGC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnGC-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnGC_DtaRun

function zUnGC_DtaControl(db)
  data = zUnGCData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnGC_DtaControl"

  zUnGC_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnGC_DtaRun(data,nation)
    end
  end
end
