...
Base.@kwdef struct zUnPCFMaxData
  db::String
  ...
  zUnPCFMax::VariableArray{2} = ReadDisk(db,"EGInput/UnPCFMax") #[Unit,Year]  Unit Capacity Factor (MW/MW)
  zUnPCFMaxRef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/UnPCFMax") #[Unit,Year]  Unit Capacity Factor (MW/MW)
...
function zUnPCFMax_AssignConversions(data)
...
function zUnPCFMax_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnPCFMax,zUnPCFMaxRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnPCFMax[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnPCFMaxRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnPCFMax;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnPCFMax_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnPCFMax,zUnPCFMaxRef) = data

  if BaseSw != 0
    @. zUnPCFMaxRef = zUnPCFMax
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnPCFMax_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnPCFMax-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnPCFMax_DtaRun

function zUnPCFMax_DtaControl(db)
  data = zUnPCFMaxData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnPCFMax_DtaControl"

  zUnPCFMax_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnPCFMax_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnPCFMax_DtaControl(DB)
end
