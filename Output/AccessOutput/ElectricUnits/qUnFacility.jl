...
Base.@kwdef struct qUnFacilityData
  db::String
  ...
  qUnFacility::Array{String} = ReadDisk(db,"EGInput/UnFacility") # [Unit] Facility Name
  qUnFacilityRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnFacility") # [Unit] Facility Name
...
function qUnFacility_AssignConversions(data)
...
function qUnFacility_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnFacility,qUnFacilityRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = qUnFacility[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = qUnFacilityRef[UnitReference]
      OutputUnCode = UnCode[UnitReference]
    end  
    
    if (ZZZ != 0 || CCC != 0)
      println(iob,"qUnFacility;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function qUnFacility_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnFacility,qUnFacilityRef) = data

  if BaseSw != 0
    @. qUnFacilityRef = qUnFacility
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnFacility_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnFacility-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnFacility_DtaRun

function qUnFacility_DtaControl(db, SceName)
  data = qUnFacilityData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnFacility_DtaControl"

  qUnFacility_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnFacility_DtaRun(data,nation,SceName)
    end
  end
end
