...
Base.@kwdef struct zUnEGAGridSalesData
  db::String
  ...
  zUnEGAGridSales::VariableArray{2} = ReadDisk(db,"EGOutput/UnEGAGridSales") #[Unit,Year]  Generation Sold to Grid (GWh)
  zUnEGAGridSalesRef::VariableArray{2} = ReadDisk(RefNameDB,"EGOutput/UnEGAGridSales") #[Unit,Year]  Generation Sold to Grid (GWh)
...
function zUnEGAGridSales_AssignConversions(data)
...
function zUnEGAGridSales_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnEGAGridSales,zUnEGAGridSalesRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnEGAGridSales[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnEGAGridSalesRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnEGAGridSales;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnEGAGridSales_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnEGAGridSales,zUnEGAGridSalesRef,SceName) = data

  if BaseSw != 0
    @. zUnEGAGridSalesRef = zUnEGAGridSales
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnEGAGridSales_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnEGAGridSales-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnEGAGridSales_DtaRun

function zUnEGAGridSales_DtaControl(db)
  data = zUnEGAGridSalesData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnEGAGridSales_DtaControl"

  zUnEGAGridSales_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnEGAGridSales_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnEGAGridSales_DtaControl(DB)
end
