...
Base.@kwdef struct qUnStorageData
  db::String
  ...
  qUnStorage::VariableArray{1} = ReadDisk(db,"EGInput/UnStorage") #[Unit]  Storage Switch (1=Storage Unit)
  qUnStorageRef::VariableArray{1} = ReadDisk(RefNameDB,"EGInput/UnStorage") #[Unit]  Storage Switch (1=Storage Unit)
...
function qUnStorage_AssignConversions(data)
...
function qUnStorage_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnStorage,qUnStorageRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
      ...
      if UnitPolicy != 0
        ZZZ = qUnStorage[UnitPolicy]*Conversion[nation]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnStorageRef[UnitReference]*Conversion[nation]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnStorage;",OutputUnCode,";",UnitsDS[nation],
...
function qUnStorage_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnStorage,qUnStorageRef) = data

  if BaseSw != 0
    @. qUnStorageRef = qUnStorage
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnStorage_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  end
  ...
  filename = "qUnStorage-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnStorage_DtaRun

function qUnStorage_DtaControl(db, SceName)
  data = qUnStorageData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnStorage_DtaControl"

  qUnStorage_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnStorage_DtaRun(data,nation,SceName)
    end
  end
end
