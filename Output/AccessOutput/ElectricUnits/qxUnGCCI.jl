...
Base.@kwdef struct qxUnGCCIData
  db::String
  ...
  qxUnGCCI::VariableArray{2} = ReadDisk(db, "EGInput/xUnGCCI") # [Unit,Year] Exogenous Generating Capacity Initiated (MW)
  qxUnGCCIRef::VariableArray{2} = ReadDisk(RefNameDB, "EGInput/xUnGCCI") # [Unit,Year] Exogenous Generating Capacity Initiated (MW)
...
function qxUnGCCI_AssignConversions(data)
...
function qxUnGCCI_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qxUnGCCI,qxUnGCCIRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qxUnGCCI[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qxUnGCCIRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qxUnGCCI;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qxUnGCCI_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qxUnGCCI,qxUnGCCIRef) = data

  if BaseSw != 0
    @. qxUnGCCIRef = qxUnGCCI
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qxUnGCCI_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qxUnGCCI-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qxUnGCCI_DtaRun

function qxUnGCCI_DtaControl(db, SceName)
  data = qxUnGCCIData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qxUnGCCI_DtaControl"

  qxUnGCCI_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qxUnGCCI_DtaRun(data,nation,SceName)
    end
  end
end
