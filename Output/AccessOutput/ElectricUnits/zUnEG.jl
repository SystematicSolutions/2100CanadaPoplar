...
Base.@kwdef struct zUnEGData
  ...
  zUnEG::VariableArray{4} = ReadDisk(db, "EGOutput/UnEG") # [Unit,TimeP,Month,Year] Generation (GWh)
  zUnEGRef::VariableArray{4} = ReadDisk(RefNameDB, "EGOutput/UnEG") # [Unit,TimeP,Month,Year] Generation (GWh)
...
function zUnEG_AssignConversions(data)
...
function zUnEG_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Month,Months,TimeP,TimePs,Unit,Units,Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnEG,zUnEGRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years, month in Months, timep in TimePs
      ...
      if UnitPolicy != 0
        ZZZ = zUnEG[UnitPolicy,timep,month,year]*Conversion[year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnEGRef[UnitReference,timep,month,year]*Conversion[year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
          println(iob,"zUnEG;",Year[year],";",Month[month],";",TimeP[timep],";",
                       OutputUnCode,";",UnitsDS[nation],
...
function zUnEG_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnEG_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnEG-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnEG_DtaRun

function zUnEG_DtaControl(db)
  data = zUnEGData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnEG_DtaControl"

  zUnEG_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnEG_DtaRun(data,nation)
    end
  end
end
