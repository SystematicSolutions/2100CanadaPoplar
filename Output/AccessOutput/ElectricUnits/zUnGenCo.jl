...
Base.@kwdef struct zUnGenCoData
  db::String
  ...
  zUnGenCo::Array{String} = ReadDisk(db, "EGInput/UnGenCo") # [Unit] Generating Company
  zUnGenCoRef::Array{String} = ReadDisk(RefNameDB, "EGInput/UnGenCo") # [Unit] Generating Company
...
function zUnGenCo_AssignConversions(data)
...
function zUnGenCo_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnGenCo,zUnGenCoRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnGenCo[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnGenCoRef[UnitReference]
      OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnGenCo;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnGenCo_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnGenCo,zUnGenCoRef) = data

  if BaseSw != 0
    @. zUnGenCoRef = zUnGenCo
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnGenCo_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnGenCo-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnGenCo_DtaRun

function zUnGenCo_DtaControl(db)
  data = zUnGenCoData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnGenCo_DtaControl"

  zUnGenCo_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnGenCo_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnGenCo_DtaControl(DB)
end
