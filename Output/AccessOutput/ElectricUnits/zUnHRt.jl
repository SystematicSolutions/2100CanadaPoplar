...
Base.@kwdef struct zUnHRtData
  ...
  zUnHRt::VariableArray{2} = ReadDisk(db, "EGInput/UnHRt") #[Unit,Year]  Heat Rate (BTU/KWh)
  zUnHRtRef::VariableArray{2} = ReadDisk(RefNameDB, "EGInput/UnHRt") #[Unit,Year]  Heat Rate (BTU/KWh)
...
function zUnHRt_AssignConversions(data)
...
function zUnHRt_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnHRt,zUnHRtRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnHRt[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnHRtRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnHRt;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnHRt_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnHRt,zUnHRtRef,SceName) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnHRt_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnHRt-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnHRt_DtaRun

function zUnHRt_DtaControl(db)
  data = zUnHRtData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnHRt_DtaControl"

  zUnHRt_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnHRt_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnHRt_DtaControl(DB)
end
