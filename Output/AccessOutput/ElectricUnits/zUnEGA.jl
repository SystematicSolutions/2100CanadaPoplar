...
Base.@kwdef struct zUnEGAData
  ...
  zUnEGA::VariableArray{2} = ReadDisk(db, "EGOutput/UnEGA") # [Unit,Year] Generation (GWh)
  zUnEGARef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnEGA") # [Unit,Year] Generation (GWh)
...
function zUnEGA_AssignConversions(data)
...
function zUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnEGA,zUnEGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnEGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnEGARef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnEGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnEGA_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnEGA,zUnEGARef,SceName) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnEGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnEGA_DtaRun

function zUnEGA_DtaControl(db)
  data = zUnEGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnEGA_DtaControl"

  zUnEGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnEGA_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnEGA_DtaControl(DB)
end
