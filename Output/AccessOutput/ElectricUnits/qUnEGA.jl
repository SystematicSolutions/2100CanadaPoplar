...
Base.@kwdef struct qUnEGAData
  ...
  qUnEGA::VariableArray{2} = ReadDisk(db, "EGOutput/UnEGA") # [Unit,Year] Generation (GWh)
  qUnEGARef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnEGA") # [Unit,Year] Generation (GWh)
...
function qUnEGA_AssignConversions(data)
...
function qUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnEGA,qUnEGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnEGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnEGARef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnEGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnEGA_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnEGA,qUnEGARef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnEGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnEGA_DtaRun

function qUnEGA_DtaControl(db, SceName)
  data = qUnEGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnEGA_DtaControl"

  qUnEGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnEGA_DtaRun(data,nation,SceName)
    end
  end
end
