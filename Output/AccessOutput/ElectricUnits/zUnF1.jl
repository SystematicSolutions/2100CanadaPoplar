...
Base.@kwdef struct zUnF1Data
  ...
  zUnF1::Array{String} = ReadDisk(db, "EGInput/UnF1") #[Unit]  Fuel Source 1
  zUnF1Ref::Array{String} = ReadDisk(RefNameDB, "EGInput/UnF1") #[Unit]  Fuel Source 1
...
function zUnF1_AssignConversions(data)
...
function zUnF1_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnF1,zUnF1Ref) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnF1[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnF1Ref[UnitReference]
      OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnF1;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnF1_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnF1,zUnF1Ref,SceName) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnF1_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnF1-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnF1_DtaRun

function zUnF1_DtaControl(db)
  data = zUnF1Data(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnF1_DtaControl"

  zUnF1_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnF1_DtaRun(data,nation)
    end
  end
end
