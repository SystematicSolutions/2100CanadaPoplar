...
Base.@kwdef struct zUnRCGAData
  ...
  zUnRCGA::VariableArray{2} = ReadDisk(db, "EGOutput/UnRCGA") # [Unit,Year] Emission Reduction Capital Costs (M$/Yr)
  zUnRCGARef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnRCGA") # [Unit,Year] Emission Reduction Capital Costs (M$/Yr)
...
function zUnRCGA_AssignConversions(data)
...
function zUnRCGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnRCGA,zUnRCGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnRCGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnRCGARef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnRCGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnRCGA_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnRCGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnRCGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnRCGA_DtaRun

function zUnRCGA_DtaControl(db)
  data = zUnRCGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnRCGA_DtaControl"

  zUnRCGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnRCGA_DtaRun(data,nation)
    end
  end
end
