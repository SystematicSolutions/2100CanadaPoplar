...
Base.@kwdef struct zUnNodeData
  ...
  zUnNode::Array{String} = ReadDisk(db,"EGInput/UnNode") # [Unit] Transmission Node
  zUnNodeRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnNode") # [Unit] Transmission Node
...
function zUnNode_AssignConversions(data)
...
function zUnNode_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnNode,zUnNodeRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnNode[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnNodeRef[UnitReference]
      OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnNode;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnNode_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnNode,zUnNodeRef) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnNode_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnNode-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnNode_DtaRun

function zUnNode_DtaControl(db)
  data = zUnNodeData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnNode_DtaControl"

  zUnNode_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnNode_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnNode_DtaControl(DB)
end
