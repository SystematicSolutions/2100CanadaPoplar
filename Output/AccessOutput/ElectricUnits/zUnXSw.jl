...
Base.@kwdef struct zUnXSwData
  ...
  zUnXSw::VariableArray{2} = ReadDisk(db,"EGInput/UnXSw") #[Unit,Year]  Exogneous Unit Data Switch (Switch) (0=Exogenous)
  zUnXSwRef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/UnXSw") #[Unit,Year]  Exogneous Unit Data Switch (Switch) (0=Exogenous)
...
function zUnXSw_AssignConversions(data)
...
function zUnXSw_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Nation,Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,zUnXSw,zUnXSwRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnXSw[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
    
      if UnitReference != 0
        CCC = zUnXSwRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if UnNation[UnitPolicy] == Nation[nation]
        println(iob,"zUnXSw;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnXSw_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnXSw_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnXSw-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnXSw_DtaRun

function zUnXSw_DtaControl(db)
  data = zUnXSwData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnXSw_DtaControl"

  zUnXSw_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnXSw_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnXSw_DtaControl(DB)
end
