...
Base.@kwdef struct qUnMustRunData
  db::String
  ...
  qUnMustRun::VariableArray{1} = ReadDisk(db, "EGInput/UnMustRun") #[Unit]  Must Run (1=Must Run)
  qUnMustRunRef::VariableArray{1} = ReadDisk(RefNameDB, "EGInput/UnMustRun") #[Unit]  Must Run (1=Must Run)
...
function qUnMustRun_AssignConversions(data)
...
function qUnMustRun_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnMustRun,qUnMustRunRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
      ...
      if UnitPolicy != 0
        ZZZ = qUnMustRun[UnitPolicy]*Conversion[nation]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnMustRunRef[UnitReference]*Conversion[nation]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnMustRun;",OutputUnCode,";",UnitsDS[nation],
...
function qUnMustRun_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnMustRun,qUnMustRunRef) = data

  if BaseSw != 0
    @. qUnMustRunRef = qUnMustRun
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnMustRun_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  end
  ...
  filename = "qUnMustRun-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnMustRun_DtaRun

function qUnMustRun_DtaControl(db, SceName)
  data = qUnMustRunData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnMustRun_DtaControl"

  qUnMustRun_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnMustRun_DtaRun(data,nation,SceName)
    end
  end
end
