...
Base.@kwdef struct zxUnGCCIData
  db::String
  ...
  zxUnGCCI::VariableArray{2} = ReadDisk(db, "EGInput/xUnGCCI") # [Unit,Year] Exogenous Generating Capacity Initiated (MW)
  zxUnGCCIRef::VariableArray{2} = ReadDisk(RefNameDB, "EGInput/xUnGCCI") # [Unit,Year] Exogenous Generating Capacity Initiated (MW)
...
function zxUnGCCI_AssignConversions(data)
...
function zxUnGCCI_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,zxUnGCCI,zxUnGCCIRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zxUnGCCI[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zxUnGCCIRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zxUnGCCI;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zxUnGCCI_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zxUnGCCI,zxUnGCCIRef) = data

  if BaseSw != 0
    @. zxUnGCCIRef = zxUnGCCI
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zxUnGCCI_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zxUnGCCI-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zxUnGCCI_DtaRun

function zxUnGCCI_DtaControl(db)
  data = zxUnGCCIData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zxUnGCCI_DtaControl"

  zxUnGCCI_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zxUnGCCI_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zxUnGCCI_DtaControl(DB)
end
