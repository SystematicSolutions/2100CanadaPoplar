...
Base.@kwdef struct qUnOURGCData
  db::String
  ...
  qUnOURGC::VariableArray{2} = ReadDisk(db,"EGInput/UnOURGC") # [Unit,Year] Own Use Rate for Generating Capacity (MW/MW)
  qUnOURGCRef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/UnOURGC") # [Unit,Year] Own Use Rate for Generating Capacity (MW/MW)
...
function qUnOURGC_AssignConversions(data)
...
function qUnOURGC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnOURGC,qUnOURGCRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnOURGC[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnOURGCRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnOURGC;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnOURGC_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnOURGC,qUnOURGCRef) = data

  if BaseSw != 0
    @. qUnOURGCRef = qUnOURGC
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnOURGC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnOURGC-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnOURGC_DtaRun

function qUnOURGC_DtaControl(db, SceName)
  data = qUnOURGCData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnOURGC_DtaControl"

  qUnOURGC_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnOURGC_DtaRun(data,nation,SceName)
    end
  end
end
