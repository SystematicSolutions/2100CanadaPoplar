...
Base.@kwdef struct qUnF1Data
  ...
  qUnF1::Array{String} = ReadDisk(db, "EGInput/UnF1") #[Unit]  Fuel Source 1
  qUnF1Ref::Array{String} = ReadDisk(RefNameDB, "EGInput/UnF1") #[Unit]  Fuel Source 1
...
function qUnF1_AssignConversions(data)
...
function qUnF1_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnF1,qUnF1Ref) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = qUnF1[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = qUnF1Ref[UnitReference]
      OutputUnCode = UnCode[UnitReference]
    end  
    
    if (ZZZ != 0 || CCC != 0)
      println(iob,"qUnF1;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function qUnF1_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnF1,qUnF1Ref) = data
  ...
  for CurrentUnit in Units
    ...
    qUnF1_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnF1-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnF1_DtaRun

function qUnF1_DtaControl(db, SceName)
  data = qUnF1Data(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnF1_DtaControl"

  qUnF1_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnF1_DtaRun(data,nation,SceName)
    end
  end
end
