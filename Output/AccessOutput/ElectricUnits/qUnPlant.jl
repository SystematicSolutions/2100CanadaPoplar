...
Base.@kwdef struct qUnPlantData
  db::String
  ...
  qUnPlant::Array{String} = ReadDisk(db,"EGInput/UnPlant") # [Unit] Plant Type
  qUnPlantRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnPlant") # [Unit] Plant Type
...
function qUnPlant_AssignConversions(data)
...
function qUnPlant_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnPlant,qUnPlantRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = qUnPlant[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = qUnPlantRef[UnitReference]
      OutputUnCode = UnCode[UnitReference]
    end  
    
    if (ZZZ != 0 || CCC != 0)
      println(iob,"qUnPlant;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function qUnPlant_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnPlant,qUnPlantRef) = data

  if BaseSw != 0
    @. qUnPlantRef = qUnPlant
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnPlant_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnPlant-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnPlant_DtaRun

function qUnPlant_DtaControl(db, SceName)
  data = qUnPlantData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnPlant_DtaControl"

  qUnPlant_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnPlant_DtaRun(data,nation,SceName)
    end
  end
end
