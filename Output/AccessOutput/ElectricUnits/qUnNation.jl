...
Base.@kwdef struct qUnNationData
  db::String
  ...
  qUnNation::Array{String} = ReadDisk(db,"EGInput/UnNation") # [Unit] Nation
  qUnNationRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnNation") # [Unit] Nation
...
function qUnNation_AssignConversions(data)
...
function qUnNation_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnNation,qUnNationRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = qUnNation[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = qUnNationRef[UnitReference]
      OutputUnCode = UnCode[UnitReference]
    end  
    
    if (ZZZ != 0 || CCC != 0)
      println(iob,"qUnNation;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function qUnNation_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnNation,qUnNationRef) = data

  if BaseSw != 0
    @. qUnNationRef = qUnNation
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnNation_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnNation-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnNation_DtaRun

function qUnNation_DtaControl(db, SceName)
  data = qUnNationData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnNation_DtaControl"

  qUnNation_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnNation_DtaRun(data,nation,SceName)
    end
  end
end
