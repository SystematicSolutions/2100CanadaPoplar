...
Base.@kwdef struct zUnORData
  ...
  zUnOR::VariableArray{4} = ReadDisk(db, "EGInput/UnOR") #[Unit,TimeP,Month,Year]  Outage Rate (MW/MW)
  zUnORRef::VariableArray{4} = ReadDisk(RefNameDB, "EGInput/UnOR") #[Unit,TimeP,Month,Year]  Outage Rate (MW/MW)
...
function zUnOR_AssignConversions(data)
...
function zUnOR_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years, month in Months, timep in TimePs
      ...
      if UnitPolicy != 0
        ZZZ = zUnOR[UnitPolicy,timep,month,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnORRef[UnitReference,timep,month,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
          println(iob,"zUnOR;",Year[year],";",Month[month],";",TimeP[timep],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnOR_DtaRun(data,nation)
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnOR_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnOR-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnOR_DtaRun

function zUnOR_DtaControl(db)
  data = zUnORData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnOR_DtaControl"

  zUnOR_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnOR_DtaRun(data,nation)
    end
  end
end
