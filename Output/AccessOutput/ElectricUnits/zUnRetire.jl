...
Base.@kwdef struct zUnRetireData
  db::String
  ...
  zUnRetire::VariableArray{2} = ReadDisk(db,"EGInput/UnRetire") # [Unit,Year] Retirement Date (Year)
  zUnRetireRef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/UnRetire") # [Unit,Year] Retirement Date (Year)
...
function zUnRetire_AssignConversions(data)
...
function zUnRetire_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnRetire,zUnRetireRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnRetire[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnRetireRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnRetire;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnRetire_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnRetire,zUnRetireRef) = data

  if BaseSw != 0
    @. zUnRetireRef = zUnRetire
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnRetire_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnRetire-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnRetire_DtaRun

function zUnRetire_DtaControl(db)
  data = zUnRetireData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnRetire_DtaControl"

  zUnRetire_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnRetire_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnRetire_DtaControl(DB)
end
