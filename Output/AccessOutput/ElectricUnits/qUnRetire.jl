...
Base.@kwdef struct qUnRetireData
  db::String
  ...
  qUnRetire::VariableArray{2} = ReadDisk(db,"EGInput/UnRetire") # [Unit,Year] Retirement Date (Year)
  qUnRetireRef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/UnRetire") # [Unit,Year] Retirement Date (Year)
...
function qUnRetire_AssignConversions(data)
...
function qUnRetire_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnRetire,qUnRetireRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnRetire[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnRetireRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnRetire;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnRetire_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnRetire,qUnRetireRef) = data

  if BaseSw != 0
    @. qUnRetireRef = qUnRetire
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnRetire_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnRetire-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnRetire_DtaRun

function qUnRetire_DtaControl(db, SceName)
  data = qUnRetireData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnRetire_DtaControl"

  qUnRetire_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnRetire_DtaRun(data,nation,SceName)
    end
  end
end
