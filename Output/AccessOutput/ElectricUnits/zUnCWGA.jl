...
Base.@kwdef struct zUnCWGAData
  ...
  zUnCWGA::VariableArray{2} = ReadDisk(db, "EGOutput/UnCWGA") # [Unit,Year] Construction Costs to Gross Assets ($M)
  zUnCWGARef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnCWGA") # [Unit,Year] Construction Costs to Gross Assets ($M)
...
function zUnCWGA_AssignConversions(data)
  (; Nation,Years) = data
  (; CDTime,CDYear,Conversion,InflationNation,UnitsDS) = data
  ...
  for year in Years
    ...
    Conversion[CN,year] = 1/InflationNation[US,year]*InflationNation[US,CDYear]
  end

  UnitsDS[US] = "$CDTime US M\$"
  UnitsDS[CN] = "$CDTime CN M\$"
  
end

function zUnCWGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnCWGA,zUnCWGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = zUnCWGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnCWGA[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnCWGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnCWGA_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnCWGA,zUnCWGARef,SceName) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnCWGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnCWGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnCWGA_DtaRun

function zUnCWGA_DtaControl(db)
  data = zUnCWGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnCWGA_DtaControl"

  zUnCWGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnCWGA_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnCWGA_DtaControl(DB)
end
