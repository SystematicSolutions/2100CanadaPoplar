...
Base.@kwdef struct qxUnEGAData
  ...
  qxUnEGA::VariableArray{2} = ReadDisk(db,"EGInput/xUnEGA") # [Unit,Year] Generation in Reference Case (GWh)
  qxUnEGARef::VariableArray{2} = ReadDisk(RefNameDB,"EGInput/xUnEGA") # [Unit,Year] Generation in Reference Case (GWh)
...
function qxUnEGA_AssignConversions(data)
...
function qxUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qxUnEGA,qxUnEGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qxUnEGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qxUnEGARef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qxUnEGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qxUnEGA_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qxUnEGA,qxUnEGARef) = data
  ...
  for CurrentUnit in Units
    ...
    qxUnEGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qxUnEGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qxUnEGA_DtaRun

function qxUnEGA_DtaControl(db, SceName)
  data = qxUnEGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qxUnEGA_DtaControl"

  qxUnEGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qxUnEGA_DtaRun(data,nation,SceName)
    end
  end
end
