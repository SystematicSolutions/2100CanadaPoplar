...
Base.@kwdef struct qUnOnLineData
  db::String
  ...
  qUnOnLine::VariableArray{1} = ReadDisk(db, "EGInput/UnOnLine") #[Unit]  On-Line Date (Year)
  qUnOnLineRef::VariableArray{1} = ReadDisk(RefNameDB, "EGInput/UnOnLine") #[Unit]  On-Line Date (Year)
...
function qUnOnLine_AssignConversions(data)
...
function qUnOnLine_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnOnLine,qUnOnLineRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
      ...
      if UnitPolicy != 0
        ZZZ = qUnOnLine[UnitPolicy]*Conversion[nation]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnOnLineRef[UnitReference]*Conversion[nation]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnOnLine;",OutputUnCode,";",UnitsDS[nation],
...
function qUnOnLine_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnOnLine,qUnOnLineRef) = data

  if BaseSw != 0
    @. qUnOnLineRef = qUnOnLine
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnOnLine_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  end
  ...
  filename = "qUnOnLine-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnOnLine_DtaRun

function qUnOnLine_DtaControl(db, SceName)
  data = qUnOnLineData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnOnLine_DtaControl"

  qUnOnLine_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnOnLine_DtaRun(data,nation,SceName)
    end
  end
end
