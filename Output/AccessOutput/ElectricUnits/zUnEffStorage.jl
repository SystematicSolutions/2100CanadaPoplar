...
Base.@kwdef struct zUnEffStorageData
  db::String
  ...
  zUnEffStorage::VariableArray{1} = ReadDisk(db,"EGInput/UnEffStorage") # [Unit] Storage Efficiency (GWH/GWH)
  zUnEffStorageRef::VariableArray{1} = ReadDisk(RefNameDB,"EGInput/UnEffStorage") # [Unit] Storage Efficiency (GWH/GWH)
...
function zUnEffStorage_AssignConversions(data)
...
function zUnEffStorage_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS,zUnEffStorage,zUnEffStorageRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
      ...
      if UnitPolicy != 0
        ZZZ = zUnEffStorage[UnitPolicy]*Conversion[nation]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnEffStorageRef[UnitReference]*Conversion[nation]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"zUnEffStorage;",OutputUnCode,";",UnitsDS[nation],
...
function zUnEffStorage_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnEffStorage,zUnEffStorageRef,SceName) = data

  if BaseSw != 0
    @. zUnEffStorageRef = zUnEffStorage
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnEffStorage_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
    end
  end
  ...
  filename = "zUnEffStorage-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnEffStorage_DtaRun

function zUnEffStorage_DtaControl(db)
  data = zUnEffStorageData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnEffStorage_DtaControl"

  zUnEffStorage_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnEffStorage_DtaRun(data,nation)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zUnEffStorage_DtaControl(DB)
end
