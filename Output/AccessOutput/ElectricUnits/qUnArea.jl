...
Base.@kwdef struct qUnAreaData
  ...
  qUnArea::Array{String} = ReadDisk(db,"EGInput/UnArea") # [Unit] Area Pointer
  qUnAreaRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnArea") # [Unit] Area Pointer
...
function qUnArea_AssignConversions(data)
...
function qUnArea_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnArea,qUnAreaRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = qUnArea[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = qUnAreaRef[UnitReference]
      OutputUnCode = UnCode[UnitReference]
    end  
    
    if (ZZZ != 0 || CCC != 0)
      println(iob,"qUnArea;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function qUnArea_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnArea,qUnAreaRef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnArea_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnArea-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnArea_DtaRun

function qUnArea_DtaControl(db, SceName)
  data = qUnAreaData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnArea_DtaControl"

  qUnArea_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnArea_DtaRun(data,nation,SceName)
    end
  end
end
