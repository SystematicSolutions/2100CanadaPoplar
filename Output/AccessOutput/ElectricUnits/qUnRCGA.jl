...
Base.@kwdef struct qUnRCGAData
  ...
  qUnRCGA::VariableArray{2} = ReadDisk(db, "EGOutput/UnRCGA") # [Unit,Year] Emission Reduction Capital Costs (M$/Yr)
  qUnRCGARef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnRCGA") # [Unit,Year] Emission Reduction Capital Costs (M$/Yr)
...
function qUnRCGA_AssignConversions(data)
...
function qUnRCGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnRCGA,qUnRCGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnRCGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnRCGARef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnRCGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnRCGA_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnRCGA,qUnRCGARef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnRCGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnRCGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnRCGA_DtaRun

function qUnRCGA_DtaControl(db, SceName)
  data = qUnRCGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnRCGA_DtaControl"

  qUnRCGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnRCGA_DtaRun(data,nation,SceName)
    end
  end
end
