...
Base.@kwdef struct qUnUOMCData
  ...
  qUnUOMC::VariableArray{2} = ReadDisk(db, "EGInput/UnUOMC") #[Unit,Year]  Variable O&M Costs ($/Kw/Yr)
  qUnUOMCRef::VariableArray{2} = ReadDisk(RefNameDB, "EGInput/UnUOMC") #[Unit,Year]  Variable O&M Costs ($/Kw/Yr)
...
function qUnUOMC_AssignConversions(data)
...
function qUnUOMC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnUOMC,qUnUOMCRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnUOMC[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnUOMCRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnUOMC;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnUOMC_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnUOMC,qUnUOMCRef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnUOMC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnUOMC-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnUOMC_DtaRun

function qUnUOMC_DtaControl(db, SceName)
  data = qUnUOMCData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnUOMC_DtaControl"

  qUnUOMC_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnUOMC_DtaRun(data,nation,SceName)
    end
  end
end
