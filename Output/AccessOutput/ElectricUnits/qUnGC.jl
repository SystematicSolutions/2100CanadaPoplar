...
Base.@kwdef struct qUnGCData
  ...
  qUnGC::VariableArray{2} = ReadDisk(db,"EGOutput/UnGC") #[Unit,Year]  Generating Capacity (MW)
  qUnGCRef::VariableArray{2} = ReadDisk(RefNameDB,"EGOutput/UnGC") #[Unit,Year]  Generating Capacity (MW)
...
function qUnGC_AssignConversions(data)
...
function qUnGC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnGC,qUnGCRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnGC[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnGCRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnGC;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnGC_DtaRun(data,nation,SceName)
  ...
  for CurrentUnit in Units
    ...
    qUnGC_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnGC-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnGC_DtaRun

function qUnGC_DtaControl(db, SceName)
  data = qUnGCData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnGC_DtaControl"

  qUnGC_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnGC_DtaRun(data,nation,SceName)
    end
  end
end
