...
Base.@kwdef struct qUnNodeData
  ...
  qUnNode::Array{String} = ReadDisk(db,"EGInput/UnNode") # [Unit] Transmission Node
  qUnNodeRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnNode") # [Unit] Transmission Node
...
function qUnNode_AssignConversions(data)
...
function qUnNode_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnNode,qUnNodeRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = qUnNode[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = qUnNodeRef[UnitReference]
      OutputUnCode = UnCode[UnitReference]
    end  
    
    if (ZZZ != 0 || CCC != 0)
      println(iob,"qUnNode;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function qUnNode_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnNode,qUnNodeRef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnNode_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnNode-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnNode_DtaRun

function qUnNode_DtaControl(db, SceName)
  data = qUnNodeData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnNode_DtaControl"

  qUnNode_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnNode_DtaRun(data,nation,SceName)
    end
  end
end
