...
Base.@kwdef struct zUnSqFrData
  ...
  zUnSqFr::VariableArray{3} = ReadDisk(db,"EGInput/UnSqFr") # [Unit,Poll,Year] Sequestered Pollution Fraction (Tonnes/Tonnes)
  zUnSqFrRef::VariableArray{3} = ReadDisk(RefNameDB,"EGInput/UnSqFr") # [Unit,Poll,Year] Sequestered Pollution Fraction (Tonnes/Tonnes)
...
function zUnSqFr_AssignConversions(data)
...
function zUnSqFr_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Month,Months,Poll,PollDS,Polls,Unit,Units,Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnSqFr,zUnSqFrRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years, poll in Polls
      ...
      if UnitPolicy != 0
        ZZZ = zUnSqFr[UnitPolicy,poll,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = zUnSqFrRef[UnitReference,poll,year]*Conversion[nation,year]
        OutputUnCode = UnCodeRef[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
          println(iob,"zUnSqFr;",Year[year],";",PollDS[poll],";",OutputUnCode,";",UnitsDS[nation],
...
function zUnSqFr_DtaRun(data,nation)
  ...
  iob = IOBuffer()
  println(iob,"Variable;Year;Pollutant;Unit;Units;zData;zInitial")
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnSqFr_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnSqFr-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnSqFr_DtaRun

function zUnSqFr_DtaControl(db)
  data = zUnSqFrData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnSqFr_DtaControl"

  zUnSqFr_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnSqFr_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnSqFr_DtaControl(DB)
end
