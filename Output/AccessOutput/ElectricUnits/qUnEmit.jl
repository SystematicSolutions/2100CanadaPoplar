...
Base.@kwdef struct qUnEmitData
  ...
  qUnEmit::VariableArray{1} = ReadDisk(db,"EGInput/UnEmit") #[Unit]  Does this Unit Emit Pollution? (1=Yes)
  qUnEmitRef::VariableArray{1} = ReadDisk(db,"EGInput/UnEmit") #[Unit]  Does this Unit Emit Pollution? (1=Yes)
...
function qUnEmit_AssignConversions(data)
...
function qUnEmit_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnEmit,qUnEmitRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
      ...
      if UnitPolicy != 0
        ZZZ = qUnEmit[UnitPolicy]*Conversion[nation]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnEmitRef[UnitReference]*Conversion[nation]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnEmit;",OutputUnCode,";",UnitsDS[nation],
...
function qUnEmit_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnEmit,qUnEmitRef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnEmit_WriteValues(data,iob,UnitPolicy,UnitReference,nation)
  end
  ...
  filename = "qUnEmit-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnEmit_DtaRun

function qUnEmit_DtaControl(db, SceName)
  data = qUnEmitData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnEmit_DtaControl"

  qUnEmit_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnEmit_DtaRun(data,nation,SceName)
    end
  end
end
