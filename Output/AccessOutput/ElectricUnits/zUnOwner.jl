...
Base.@kwdef struct zUnOwnerData
  db::String
  ...
  zUnOwner::Array{String} = ReadDisk(db,"EGInput/UnOwner") # [Unit] Generating Company
  zUnOwnerRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnOwner") # [Unit] Generating Company
...
function zUnOwner_AssignConversions(data)
...
function zUnOwner_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnOwner,zUnOwnerRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnOwner[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnOwnerRef[UnitReference]
        OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnOwner;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnOwner_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,SceName,zUnOwner,zUnOwnerRef) = data

  if BaseSw != 0
    @. zUnOwnerRef = zUnOwner
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnOwner_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnOwner-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnOwner_DtaRun

function zUnOwner_DtaControl(db)
  data = zUnOwnerData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnOwner_DtaControl"

  zUnOwner_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnOwner_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnOwner_DtaControl(DB)
end
