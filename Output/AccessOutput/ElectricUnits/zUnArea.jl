...
Base.@kwdef struct zUnAreaData
  ...
  zUnArea::Array{String} = ReadDisk(db,"EGInput/UnArea") # [Unit] Area Pointer
  zUnAreaRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnArea") # [Unit] Area Pointer
...
function zUnArea_AssignConversions(data)
...
function zUnArea_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnArea,zUnAreaRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnArea[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnAreaRef[UnitReference]
      OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnArea;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnArea_DtaRun(data,nation)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnArea,zUnAreaRef,SceName) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnArea_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnArea-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnArea_DtaRun

function zUnArea_DtaControl(db)
  data = zUnAreaData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnArea_DtaControl"
  
  zUnArea_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnArea_DtaRun(data,nation)
    end
  end
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zUnArea_DtaControl(DB)
end
