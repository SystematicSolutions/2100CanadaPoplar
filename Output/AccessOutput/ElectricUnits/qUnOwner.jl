...
Base.@kwdef struct qUnOwnerData
  db::String
  ...
  qUnOwner::Array{String} = ReadDisk(db,"EGInput/UnOwner") # [Unit] Generating Company
  qUnOwnerRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnOwner") # [Unit] Generating Company
...
function qUnOwner_AssignConversions(data)
...
function qUnOwner_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnOwner,qUnOwnerRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = qUnOwner[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = qUnOwnerRef[UnitReference]
      OutputUnCode = UnCode[UnitReference]
    end  
    
    if (ZZZ != 0 || CCC != 0)
      println(iob,"qUnOwner;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function qUnOwner_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnOwner,qUnOwnerRef) = data

  if BaseSw != 0
    @. qUnOwnerRef = qUnOwner
    @. UnCodeRef = UnCode
  ...
  for CurrentUnit in Units
    ...
    qUnOwner_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnOwner-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnOwner_DtaRun

function qUnOwner_DtaControl(db, SceName)
  data = qUnOwnerData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnOwner_DtaControl"

  qUnOwner_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnOwner_DtaRun(data,nation,SceName)
    end
  end
end
