...
Base.@kwdef struct qUnHRtData
  ...
  qUnHRt::VariableArray{2} = ReadDisk(db, "EGInput/UnHRt") #[Unit,Year]  Heat Rate (BTU/KWh)
  qUnHRtRef::VariableArray{2} = ReadDisk(RefNameDB, "EGInput/UnHRt") #[Unit,Year]  Heat Rate (BTU/KWh)
...
function qUnHRt_AssignConversions(data)
...
function qUnHRt_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnHRt,qUnHRtRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnHRt[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnHRtRef[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnHRt;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnHRt_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnHRt,qUnHRtRef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnHRt_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnHRt-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnHRt_DtaRun

function qUnHRt_DtaControl(db, SceName)
  data = qUnHRtData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnHRt_DtaControl"

  qUnHRt_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnHRt_DtaRun(data,nation,SceName)
    end
  end
end
