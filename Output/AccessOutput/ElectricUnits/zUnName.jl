...
Base.@kwdef struct zUnNameData
  ...
  zUnName::Array{String} = ReadDisk(db,"EGInput/UnName") # [Unit] Plant Name
  zUnNameRef::Array{String} = ReadDisk(RefNameDB,"EGInput/UnName") # [Unit] Plant Name
...
function zUnName_AssignConversions(data)
...
function zUnName_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnCodeRef,UnitsDS) = data
  (; zUnName,zUnNameRef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    if UnitPolicy != 0
      ZZZ = zUnName[UnitPolicy]
      OutputUnCode = UnCode[UnitPolicy]
    end
    
    if UnitReference != 0
      CCC = zUnNameRef[UnitReference]
      OutputUnCode = UnCodeRef[UnitReference]
    end  
    
    if (ZZZ != "" || CCC != "")
      println(iob,"zUnName;",OutputUnCode,";",UnitsDS[nation],";",ZZZ,";",CCC)
    end
      
  end
end

function zUnName_DtaRun(data,nation)
  (; Nation,Units,Year,Years,SceName) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,zUnName,zUnNameRef) = data
  ...
  for CurrentUnit in Units
    if UnNation[CurrentUnit] == Nation[nation]
      ...
      zUnName_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
    end
  end
  ...
  filename = "zUnName-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function zUnName_DtaRun

function zUnName_DtaControl(db)
  data = zUnNameData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "zUnName_DtaControl"

  zUnName_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    BuildUnitList(nation)
    if NationOutputMap[nation] == 1
      zUnName_DtaRun(data,nation)
    end
  end
end
