...
Base.@kwdef struct qUnCWGAData
  ...
  qUnCWGA::VariableArray{2} = ReadDisk(db, "EGOutput/UnCWGA") # [Unit,Year] Construction Costs to Gross Assets ($M)
  qUnCWGARef::VariableArray{2} = ReadDisk(RefNameDB, "EGOutput/UnCWGA") # [Unit,Year] Construction Costs to Gross Assets ($M)
...
function qUnCWGA_AssignConversions(data)
...
function qUnCWGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  (; Year) = data
  (; Conversion,UnCode,UnitsDS,qUnCWGA,qUnCWGARef) = data
  ...
  if UnitPolicy != 0 || UnitReference != 0
    ...
    for year in years
      ...
      if UnitPolicy != 0
        ZZZ = qUnCWGA[UnitPolicy,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitPolicy]
      end
      
      if UnitReference != 0
        CCC = qUnCWGA[UnitReference,year]*Conversion[nation,year]
        OutputUnCode = UnCode[UnitReference]
      end  
      
      if (ZZZ != 0 || CCC != 0)
        println(iob,"qUnCWGA;",Year[year],";",OutputUnCode,";",UnitsDS[nation],
...
function qUnCWGA_DtaRun(data,nation,SceName)
  (; Nation,Units,Year,Years) = data
  (; BaseSw,Conversion,EndTime,UnCode,UnCodeRef,UnitsDS) = data
  (; UnNation,UnNationRef,qUnCWGA,qUnCWGARef) = data
  ...
  for CurrentUnit in Units
    ...
    qUnCWGA_WriteValues(data,iob,UnitPolicy,UnitReference,nation,years)
  end
  ...
  filename = "qUnCWGA-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end # function qUnCWGA_DtaRun

function qUnCWGA_DtaControl(db, SceName)
  data = qUnCWGAData(; db)
  (; Nation)= data
  (; NationOutputMap)= data
  
  @info "qUnCWGA_DtaControl"

  qUnCWGA_AssignConversions(data)
  nations = Select(Nation,["CN","US"])
  for nation in nations
    if NationOutputMap[nation] == 1
      qUnCWGA_DtaRun(data,nation,SceName)
    end
  end
end
