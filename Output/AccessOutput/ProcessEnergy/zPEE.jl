...
Base.@kwdef struct zPEEResData
  ...
  zPEE::VariableArray{5} = ReadDisk(db,"$Outpt/PEE") # [Enduse,Tech,EC,Area,Year] Process Efficiency ($/Btu)
  zPEERef::VariableArray{5} = ReadDisk(RefNameDB,"$Outpt/PEE") # [Enduse,Tech,EC,Area,Year] Process Efficiency ($/Btu)
...
function AssignConversions_zPEERes(data)
  (; Nation,Years) = data
...
function zPEERes_DtaRun(data,iob,nation,SceName)
  (; Ages,Area,AreaDS,EC,ECDS,ECs,EnduseDS,Enduses) = data
  (; Nation,NationDS,Tech,TechDS,Techs,Year) = data
  (; ANMap,BaseSw,CDTime,EndTime,EUPC,EUPCRef,zPEE,zPEERef) = data
  ...
  for ec in ECs
    for enduse in Enduses
      for tech in Techs
        for year in years
          @finite_math ZZZ[year] = sum(zPEE[enduse,tech,ec,area,year]*
            ...
            @finite_math CCC[year] = sum(zPEERef[enduse,tech,ec,area,year]*
          ...
          if ZZZ[year] != 0 || CCC[year] != 0
            ...
            println(iob,"zPEE;", Year[year],";", NationDS[nation],";",ECDS[ec],";",TechDS[tech],";",
              EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
          end

          for area in areas
            @finite_math ZZZ[year] = (zPEE[enduse,tech,ec,area,year]*
              sum(EUPC[enduse,tech,age,ec,area,year] for age in Ages))/
              sum(EUPC[enduse,tech,age,ec,area,year] for age in Ages)*
              Conversion[nation,year]

            @finite_math CCC[year] = (zPEERef[enduse,tech,ec,area,year]*
            ...
            if ZZZ[year] != 0 || CCC[year] != 0
              zData = @sprintf("%.6E",ZZZ[year])
              zInitial = @sprintf("%.6E",CCC[year])
              println(iob,"zPEE;", Year[year],";", AreaDS[area],";",ECDS[ec],";",TechDS[tech],";",
                EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
            end
    ...
    for tech in Techs
      for year in years
        ...
        if ZZZ[year] != 0 || CCC[year] != 0
          zData = @sprintf("%.6E",ZZZ[year])
          zInitial = @sprintf("%.6E",CCC[year])
          println(iob,"zPEE;", Year[year],";", NationDS[nation],";Residential;",TechDS[tech],";",
            EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
        end
        ...
        @finite_math CCC[year] = sum(zPEERef[enduse,tech,ec,area,year]*
          ...
          if ZZZ[year] != 0 || CCC[year] != 0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zPEE;", Year[year],";", AreaDS[area],";Residential;",TechDS[tech],";",
              EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
          end
        end # for area in areas
      end # for year in years
...
end # function zPEERes_DtaRun

#
# Commercial
#
Base.@kwdef struct zPEEComData
  db::String
...
function AssignConversions_zPEECom(data)
  (; Nation,Years) = data
  (; CDTime,CDYear,Conversion,ConversionRef) = data
  (; InflationNation,InflationNationRef,UnitsDS) = data
...
function zPEECom_DtaRun(data,iob,nation,SceName)
  (; Ages,Area,AreaDS,EC,ECDS,ECs,EnduseDS,Enduses) = data
  (; Nation,NationDS,Tech,TechDS,Techs,Year) = data
  (; ANMap,BaseSw,CDTime,EndTime,EUPC,EUPCRef,zPEE,zPEERef) = data
  (; CCC,Conversion,ConversionRef,UnitsDS,ZZZ) = data
    ...
    for tech in Techs
      for year in years
        ...
        if ZZZ[year] != 0 || CCC[year] != 0
          zData = @sprintf("%.6E",ZZZ[year])
          zInitial = @sprintf("%.6E",CCC[year])
          println(iob,"zPEE;", Year[year],";", NationDS[nation],";Commercial;",TechDS[tech],";",
            EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
        end
        ...
        @finite_math CCC[year] = sum(zPEERef[enduse,tech,ec,area,year]*
          ...
          if ZZZ[year] != 0 || CCC[year] != 0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zPEE;", Year[year],";", AreaDS[area],";Commercial;",TechDS[tech],";",
              EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
          end
        end # for area in areas
      end # for year in years
...
end # function zPEECom_DtaRun

#
# Industrial
#
Base.@kwdef struct zPEEIndData
  db::String
...
function AssignConversions_zPEEInd(data)
  (; Nation,Years) = data
  (; CDTime,CDYear,Conversion,ConversionRef) = data
  (; InflationNation,InflationNationRef,UnitsDS) = data
...
function zPEEInd_DtaRun(data,iob,nation,SceName)
  (; Ages,Area,AreaDS,EC,ECDS,ECs,EnduseDS,Enduses) = data
  (; Nation,NationDS,Tech,TechDS,Techs,Year) = data
  (; ANMap,BaseSw,CDTime,EndTime,EUPC,EUPCRef,zPEE,zPEERef) = data
  (; CCC,Conversion,ConversionRef,UnitsDS,ZZZ) = data
    ...
    for tech in Techs
      for year in years
        ...
        if ZZZ[year] != 0 || CCC[year] != 0
          zData = @sprintf("%.6E",ZZZ[year])
          zInitial = @sprintf("%.6E",CCC[year])
          println(iob,"zPEE;", Year[year],";", NationDS[nation],";Industrial;",TechDS[tech],";",
            EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
        end
        ...
        @finite_math CCC[year] = sum(zPEERef[enduse,tech,ec,area,year]*
          ...
          if ZZZ[year] != 0 || CCC[year] != 0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zPEE;", Year[year],";", AreaDS[area],";Industrial;",TechDS[tech],";",
              EnduseDS[enduse],";",UnitsDS[nation],";",zData,";",zInitial)
          end
        end # for area in areas
      end # for year in years
...
end # function zPEEInd_DtaRun

function zPEE_Residential(db,iob,nation,SceName)
  data = zPEEResData(; db)
  AssignConversions_zPEERes(data)
  zPEERes_DtaRun(data,iob,nation,SceName)
end

function zPEE_Commercial(db,iob,nation,SceName)
  data = zPEEComData(; db)
  AssignConversions_zPEECom(data)
  zPEECom_DtaRun(data,iob,nation,SceName)
end

function zPEE_Industrial(db,iob,nation,SceName)
  data = zPEEIndData(; db)
  AssignConversions_zPEEInd(data)
  zPEEInd_DtaRun(data,iob,nation,SceName)
end

function CreatezPEEOutputFile(db,iob,nationkey,SceName)
  filename = "zPEE-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function zPEE_DtaControl(db)
  data = zPEEResData(; db)
  (; Nation,Nations) = data
  (; NationOutputMap,SceName) = data

  @info "zPEE_DtaControl"
  ...
  for nation in Nations
    if NationOutputMap[nation] == 1
      zPEE_Residential(db,iob,nation,SceName)
      zPEE_Commercial(db,iob,nation,SceName)
      zPEE_Industrial(db,iob,nation,SceName)

      CreatezPEEOutputFile(db,iob,Nation[nation],SceName)
    end
  end
