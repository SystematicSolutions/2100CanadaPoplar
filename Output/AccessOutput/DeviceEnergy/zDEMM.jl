#
# zDEMM.jl - Maximum Device Efficiency Multiplier
#
using EnergyModel
...
Base.@kwdef struct zDEMMResidentialData
  db::String
  ...
  zDEMM::VariableArray{5} = ReadDisk(db, "$CalDB/DEMM") # [Enduse,Tech,EC,Area,Year] Maximum Device Efficiency Multiplier (Btu/Btu)
  zDEMMRef::VariableArray{5} = ReadDisk(RefNameDB, "$CalDB/DEMM") # [Enduse,Tech,EC,Area,Year] Maximum Device Efficiency Multiplier (Btu/Btu)
...
Base.@kwdef struct zDEMMCommercialData
  db::String
...
Base.@kwdef struct zDEMMIndustrialData
  db::String
...
function zDEMM_DtaRun(data,nation,segment_name)
  (; Area,AreaDS,Areas,EC,ECDS,ECs,Enduse,EnduseDS,Enduses,Nation,NationDS,Nations) = data
  (; Plants,PlantDS,Tech,TechDS,Techs,Year,Ages,SectorDS,SceName) = data
  (; ANMap,BaseSw,CDTime,CDYear,Conversion,EndTime,KJBtu,UnitsDS) = data
  (; zDEMM,zDEMMRef,EUPC,EUPCRef,NationOutputMap) = data
  ...
  # Set up units (no conversion needed for efficiency multipliers)
  US = Select(Nation,"US")
  CN = Select(Nation,"CN")
      ...
      for tech in Techs
        for year in years
          ...
          if total_cap > 0 || total_cap_ref > 0
            ZZZ = sum(zDEMM[enduse,t,ec,area,year]*sum(EUPC[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, area in areas) / max(total_cap, 1e-12) * Conversion[nation,year]
            CCC = sum(zDEMMRef[enduse,t,ec,area,year]*sum(EUPCRef[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, area in areas) / max(total_cap_ref, 1e-12) * Conversion[nation,year]
            
            if ZZZ != 0 || CCC != 0
              println(iob,"zDEMM;",Year[year],";",NationDS[nation],";",ECDS[ec],";",TechDS[tech],";",
          ...
          for area in areas
            ...
            if total_cap_area > 0 || total_cap_ref_area > 0
              ...
              if ZZZ != 0 || CCC != 0
                println(iob,"zDEMM;",Year[year],";",AreaDS[area],";",ECDS[ec],";",TechDS[tech],";",
                  EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
              end
            end
          end
    ...
    for tech in Techs
      for year in years
        ...
        if total_cap > 0 || total_cap_ref > 0
          ...
          if ZZZ != 0 || CCC != 0
            println(iob,"zDEMM;",Year[year],";",NationDS[nation],";",SectorDS[1],";",TechDS[tech],";",
              EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
          end
        end
        ...
        for area in areas
          ...
          if total_cap_area > 0 || total_cap_ref_area > 0
            ...
            if ZZZ != 0 || CCC != 0
              println(iob,"zDEMM;",Year[year],";",AreaDS[area],";",SectorDS[1],";",TechDS[tech],";",
                EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
            end
          end
        end
