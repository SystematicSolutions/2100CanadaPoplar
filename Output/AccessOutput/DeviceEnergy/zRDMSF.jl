#
# zRDMSF.jl - Device Retrofit Market Share Fraction by Device
#
using EnergyModel
...
Base.@kwdef struct zRDMSFResidentialData
  db::String
  ...
  zRDMSF::VariableArray{5} = ReadDisk(db, "$Outpt/RDMSF") # [Enduse,Tech,EC,Area,Year] Device Retrofit Market Share Fraction by Device (1/Yr)
  zRDMSFRef::VariableArray{5} = ReadDisk(RefNameDB, "$Outpt/RDMSF") # [Enduse,Tech,EC,Area,Year] Device Retrofit Market Share Fraction by Device (1/Yr)
...
Base.@kwdef struct zRDMSFCommercialData
  db::String
...
Base.@kwdef struct zRDMSFIndustrialData
  db::String
...
function zRDMSF_DtaRun(data,nation,segment_name)
  ...
  (; ANMap,BaseSw,CDTime,CDYear,EndTime,UnitsDS) = data
  (; zRDMSF,zRDMSFRef,EUPC,EUPCRef,NationOutputMap) = data
      ...
      for tech in Techs
        for year in years
          ...
          if total_cap > 0 || total_cap_ref > 0
            ZZZ = sum(zRDMSF[enduse,t,ec,area,year]*sum(EUPC[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, area in areas) / max(total_cap, 1e-12)
            CCC = sum(zRDMSFRef[enduse,t,ec,area,year]*sum(EUPCRef[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, area in areas) / max(total_cap_ref, 1e-12)
            
            if ZZZ != 0 || CCC != 0
              println(iob,"zRDMSF;",Year[year],";",NationDS[nation],";",ECDS[ec],";",TechDS[tech],";",
          ...
          for area in areas
            ...
            if total_cap_area > 0 || total_cap_ref_area > 0
              ZZZ = sum(zRDMSF[enduse,t,ec,area,year]*sum(EUPC[enduse,t,age,ec,area,year] for age in Ages) 
                       for t in Techs) / max(total_cap_area, 1e-12)
              CCC = sum(zRDMSFRef[enduse,t,ec,area,year]*sum(EUPCRef[enduse,t,age,ec,area,year] for age in Ages) 
                       for t in Techs) / max(total_cap_ref_area, 1e-12)
              
              if ZZZ != 0 || CCC != 0
                println(iob,"zRDMSF;",Year[year],";",AreaDS[area],";",ECDS[ec],";",TechDS[tech],";",
                  EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
              end
            end
          end
  ...
  for enduse in Enduses
    for tech in Techs
      for year in years
        ...
        if total_cap > 0 || total_cap_ref > 0
          ZZZ = sum(zRDMSF[enduse,t,ec,area,year]*sum(EUPC[enduse,t,age,ec,area,year] for age in Ages) 
                   for t in Techs, ec in ECs, area in areas) / max(total_cap, 1e-12)
          CCC = sum(zRDMSFRef[enduse,t,ec,area,year]*sum(EUPCRef[enduse,t,age,ec,area,year] for age in Ages) 
                   for t in Techs, ec in ECs, area in areas) / max(total_cap_ref, 1e-12)
          
          if ZZZ != 0 || CCC != 0
            println(iob,"zRDMSF;",Year[year],";",NationDS[nation],";",SectorDS[1],";",TechDS[tech],";",
              EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
          end
        end
        ...
        for area in areas
          ...
          if total_cap_area > 0 || total_cap_ref_area > 0
            ZZZ = sum(zRDMSF[enduse,t,ec,area,year]*sum(EUPC[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, ec in ECs) / max(total_cap_area, 1e-12)
            CCC = sum(zRDMSFRef[enduse,t,ec,area,year]*sum(EUPCRef[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, ec in ECs) / max(total_cap_ref_area, 1e-12)
            
            if ZZZ != 0 || CCC != 0
              println(iob,"zRDMSF;",Year[year],";",AreaDS[area],";",SectorDS[1],";",TechDS[tech],";",
                EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
            end
          end
        end
  ...
  filename = "zRDMSF-$segment_name-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do file
    write(file, String(take!(iob)))
  end
end

function zRDMSF_DtaControl(db)
  @info "zRDMSF_DtaControl"

  # Process CN for all segments
  CN = Select(ReadDisk(db, "MainDB/NationKey"),"CN")
  
  # Process Residential segment for CN
  res_data = zRDMSFResidentialData(; db)
  zRDMSF_DtaRun(res_data,CN,"Residential")

  # Process Commercial segment for CN  
  com_data = zRDMSFCommercialData(; db)
  zRDMSF_DtaRun(com_data,CN,"Commercial")

  # Process Industrial segment for CN
  ind_data = zRDMSFIndustrialData(; db)
  zRDMSF_DtaRun(ind_data,CN,"Industrial")

  # Process US for all segments
  US = Select(ReadDisk(db, "MainDB/NationKey"),"US")
  
  # Process Residential segment for US
  zRDMSF_DtaRun(res_data,US,"Residential")

  # Process Commercial segment for US  
  zRDMSF_DtaRun(com_data,US,"Commercial")

  # Process Industrial segment for US
  zRDMSF_DtaRun(ind_data,US,"Industrial")
end
if abspath(PROGRAM_FILE) == @__FILE__
  zRDMSF_DtaControl(DB)
end
