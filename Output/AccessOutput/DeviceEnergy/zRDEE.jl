...
Base.@kwdef struct zRDEEResidentialData
  db::String
  ...
  zRDEE::VariableArray{5} = ReadDisk(db, "$Outpt/RDEE") # [Enduse,Tech,EC,Area,Year] Process Efficiency ($/Btu)
  zRDEERef::VariableArray{5} = ReadDisk(RefNameDB, "$Outpt/RDEE") # [Enduse,Tech,EC,Area,Year] Process Efficiency ($/Btu)
...
Base.@kwdef struct zRDEECommercialData
  db::String
...
Base.@kwdef struct zRDEEIndustrialData
  db::String
...
function zRDEE_DtaRun(data,nation,segment_name)
  ...
  (; zRDEE,zRDEERef,EUPC,EUPCRef,NationOutputMap) = data
      ...
      for tech in Techs
        for year in years
          ...
          if total_cap > 0 || total_cap_ref > 0
            ZZZ = sum(zRDEE[enduse,t,ec,area,year]*sum(EUPC[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, area in areas) / max(total_cap, 1e-12)
            CCC = sum(zRDEERef[enduse,t,ec,area,year]*sum(EUPCRef[enduse,t,age,ec,area,year] for age in Ages) 
                     for t in Techs, area in areas) / max(total_cap_ref, 1e-12)
            
            if ZZZ != 0 || CCC != 0
              println(iob,"zRDEE;",Year[year],";",NationDS[nation],";",ECDS[ec],";",TechDS[tech],";",
          ...
          for area in areas
            ...
            if total_cap_area > 0 || total_cap_ref_area > 0
              ...
              if ZZZ != 0 || CCC != 0
                println(iob,"zRDEE;",Year[year],";",AreaDS[area],";",ECDS[ec],";",TechDS[tech],";",
                  EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
              end
            end
          end
    ...
    for tech in Techs
      for year in years
        ...
        if total_cap > 0 || total_cap_ref > 0
          ...
          if ZZZ != 0 || CCC != 0
            println(iob,"zRDEE;",Year[year],";",NationDS[nation],";",SectorDS[1],";",TechDS[tech],";",
              EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
          end
        end
        ...
        for area in areas
          ...
          if total_cap_area > 0 || total_cap_ref_area > 0
            ...
            if ZZZ != 0 || CCC != 0
              println(iob,"zRDEE;",Year[year],";",AreaDS[area],";",SectorDS[1],";",TechDS[tech],";",
                EnduseDS[enduse],";",UnitsDS[nation],";",@sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
            end
          end
        end
