#
# zDInvTechInd.jl - Write Process Investments for Access Database
#
#
# Industrial
#
Base.@kwdef struct zDInvTechIndData
  db::String
...
function AssignConversions_zDInvTechInd(data)
...
function zDInvTechInd_DtaRun(data,iob,nation,SceName)
...
end # function zDInvTechInd_DtaRun


function zDInvTech_Industrial(db,iob,nation,SceName)
  data = zDInvTechIndData(; db)
  AssignConversions_zDInvTechInd(data)
  zDInvTechInd_DtaRun(data,iob,nation,SceName)
end

function CreateDInvTechIndOutputFile(db,iob,nationkey,SceName)
  filename = "zDInvTechInd-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function zDInvTechInd_DtaControl(db,SceName)
  data = zDInvTechResData(; db)
  (; Nation,Nations) = data
  (; NationOutputMap) = data

  @info "zDInvTechInd_DtaControl"
  ...
  for nation in Nations
    if NationOutputMap[nation] == 1
      zDInvTech_Industrial(db,iob,nation,SceName)
      CreateDInvTechIndOutputFile(db,iob,Nation[nation],SceName)
    end
  end
