#
# zDInvTechRes.jl - Write Process Investments for Access Database
#

Base.@kwdef struct zDInvTechResData
  db::String
...
function AssignConversions_zDInvTechRes(data)
...
function zDInvTechRes_DtaRun(data,iob,nation,SceName)
...
end # function zDInvTechRes_DtaRun

function zDInvTech_Residential(db,iob,nation,SceName)
  data = zDInvTechResData(; db)
  AssignConversions_zDInvTechRes(data)
  zDInvTechRes_DtaRun(data,iob,nation,SceName)
end

function CreateDInvTechResOutputFile(db,iob,nationkey,SceName)
  filename = "zDInvTechRes-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function zDInvTechRes_DtaControl(db,SceName)
  data = zDInvTechResData(; db)
  (; Nation,Nations) = data
  (; NationOutputMap) = data

  @info "zDInvTechRes_DtaControl"
  ...
  for nation in Nations
    if NationOutputMap[nation] == 1
      zDInvTech_Residential(db,iob,nation,SceName)
      CreateDInvTechResOutputFile(db,iob,Nation[nation],SceName)
    end
  end
