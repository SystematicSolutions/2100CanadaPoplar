...
Base.@kwdef struct zFuPolData
  ...
  zFuPol::VariableArray{4} = ReadDisk(db,"SOutput/FuPol")  # [ECC,Poll,Area,Year]  Other Fugitive Emissions (Tonnes/Yr)
  zFuPolRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/FuPol")  # [ECC,Poll,Area,Year]  Other Fugitive Emissions (Tonnes/Yr)
...
function zFuPol_DtaRun(data,polls,nation)
  ...
  for ecc in ECCs
    for poll in polls
      for area in areas
        for year in years
          ZZZ[year] = zFuPol[ecc,poll,area,year]*Conversion[poll]
          CCC[year] = zFuPolRef[ecc,poll,area,year]*Conversion[poll]
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zFuPol;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
  ...
  if polltype == "GHG"
    for ecc in ECCs
      for area in areas
        for year in years
          ZZZ[year] = sum(zFuPol[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          CCC[year] = sum(zFuPolRef[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zFuPol;",Year[year],";",AreaDS[area],";",polltype,";",
              ECCDS[ecc],";",pollunit,";",ZZZ[year],";",CCC[year])
          end
        end
      end
    end
