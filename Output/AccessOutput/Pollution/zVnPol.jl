...
Base.@kwdef struct zVnPolData
  ...
  zVnPol::VariableArray{4} = ReadDisk(db,"SOutput/VnPol") # [ECC,Poll,Area,Year]  Venting Emissions (Tonnes/Yr)
  zVnPolRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/VnPol") # [ECC,Poll,Area,Year]  Venting Emissions (Tonnes/Yr)
...
function zVnPol_DtaRun(data,polls,nation)
  ...
  for ecc in ECCs
    for poll in polls
      for area in areas
        for year in years
          ZZZ[year] = zVnPol[ecc,poll,area,year]*Conversion[poll]
          CCC[year] = zVnPolRef[ecc,poll,area,year]*Conversion[poll]
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zVnPol;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
  ...
  if polltype == "GHG"
    for ecc in ECCs
      for area in areas
        for year in years
          ZZZ[year] = sum(zVnPol[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          CCC[year] = sum(zVnPolRef[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zVnPol;",Year[year],";",AreaDS[area],";",polltype,";",ECCDS[ecc],";",pollunit,";",zData,";",zInitial)
          end
        end
      end
    end
