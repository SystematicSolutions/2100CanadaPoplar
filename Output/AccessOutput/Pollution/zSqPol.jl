...
Base.@kwdef struct zSqPolData
  ...
  zSqPol::VariableArray{4} = ReadDisk(db,"SOutput/SqPol") # [ECC,Poll,Area,Year] Sequestering Emissions (Tonnes/Yr)
  zSqPolRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/SqPol") # [ECC,Poll,Area,Year] Sequestering Emissions (Tonnes/Yr)
...
function zSqPol_DtaRun(data,polls,nation)
  ...
  for ecc in ECCs
    for poll in polls
      for area in areas
        for year in years
          ZZZ[year] = zSqPol[ecc,poll,area,year]*Conversion[poll]
          CCC[year] = zSqPolRef[ecc,poll,area,year]*Conversion[poll]
          if ZZZ[year] != 0 || CCC[year] != 0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zSqPol;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
  ...
  if polltype == "GHG"
    for ecc in ECCs
      for area in areas
        for year in years
          ZZZ[year] = sum(zSqPol[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          CCC[year] = sum(zSqPolRef[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zSqPol;",Year[year],";",AreaDS[area],";",polltype,";",
