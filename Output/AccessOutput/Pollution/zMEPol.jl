...
Base.@kwdef struct zMEPolData
  ...
  zMEPol::VariableArray{4} = ReadDisk(db,"SOutput/MEPol") #[ECC,Poll,Area,Year] Non-Energy Related Pollution (Tonnes/Yr) 
  zMEPolRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/MEPol") #[ECC,Poll,Area,Year] Non-Energy Related Pollution (Tonnes/Yr) 
...
function zMEPol_DtaRun(data,polls,nation)
  ...
  (; UnitsDS,zMEPol,zMEPolRef,ZZZ,SceName) = data
  ...
  for poll in Polls
    ...
    for ecc in ECCs
      for poll in polls
        for area in areas
          for year in years
            ZZZ[year] = zMEPol[ecc,poll,area,year]*Conversion[poll]
            CCC[year] = zMEPolRef[ecc,poll,area,year]*Conversion[poll]
            if ZZZ[year] != 0.0 || CCC[year] != 0.0
              zData = @sprintf("%.6E",ZZZ[year])
              zInitial = @sprintf("%.6E",CCC[year])
              println(iob,"zMEPol;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
  ...
  if polltype == "GHG"
    for ecc in ECCs
      for area in areas
        for year in years
          ZZZ[year] = sum(zMEPol[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          CCC[year] = sum(zMEPolRef[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zMEPol;",Year[year],";",AreaDS[area],";",polltype,";",
