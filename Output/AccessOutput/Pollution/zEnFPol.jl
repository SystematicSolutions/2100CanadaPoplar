...
function zEnFPol_DtaRun(data,polls,nation)
  ...
  (; PollType,UnitsDS,zEnFPol,zEnFPolRef,ZZZ) = data
  ...
  for fuelep in FuelEPs
    for ecc in ECCs
      for poll in polls
        for area in areas
          for year in years
            ZZZ[year] = zEnFPol[fuelep,ecc,poll,area,year]*Conversion[poll]
            CCC[year] = zEnFPolRef[fuelep,ecc,poll,area,year]*Conversion[poll]
            if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
              println(iob,"zEnFPol;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
  ...
  if polltype == "GHG"
    for fuelep in FuelEPs
      for ecc in ECCs
        for area in areas
          for year in years
            ZZZ[year] = sum(zEnFPol[fuelep,ecc,poll,area,year]*
              PolConv[poll] for poll in polls)/1000
            CCC[year] = sum(zEnFPolRef[fuelep,ecc,poll,area,year]*
              PolConv[poll] for poll in polls)/1000
            if ZZZ[year] != 0.0 || CCC[year] != 0.0
              zData = @sprintf("%.6E",ZZZ[year])
              zInitial = @sprintf("%.6E",CCC[year])
              println(iob,"zEnFPol;",Year[year],";",AreaDS[area],";",polltype,";",
