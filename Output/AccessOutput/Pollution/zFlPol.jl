...
Base.@kwdef struct zFlPolData
  ...
  zFlPol::VariableArray{4} = ReadDisk(db,"SOutput/FlPol")  # [ECC,Poll,Area,Year]  Fugitive Flaring Emissions (Tonnes/Yr)
  zFlPolRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/FlPol")  #[ECC,Poll,Area,Year]  Fugitive Flaring Emissions (Tonnes/Yr)
...
function zFlPol_DtaRun(data,polls,nation)
  ...
  for ecc in ECCs
    for poll in polls
      for area in areas
        for year in years
          ZZZ[year] = zFlPol[ecc,poll,area,year]*Conversion[poll]
          CCC[year] = zFlPolRef[ecc,poll,area,year]*Conversion[poll]
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zFlPol;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
  ...
  if polltype == "GHG"
    for ecc in ECCs
      for area in areas
        for year in years
          ZZZ[year] = sum(zFlPol[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          CCC[year] = sum(zFlPolRef[ecc,poll,area,year]*PolConv[poll] for poll in polls)/1000
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zFlPol;",Year[year],";",AreaDS[area],";",polltype,";",
