#
# zQ_Production.jl - Comparison of TOM baseline values to E2020 transfers
#
using EnergyModel
...
Base.@kwdef struct zQ_ProductionData
  db::String
...
function zQ_Production_DtaRun(data,nationkey,areatoms)
  (; AreaTOM,AreaTOMDS,FuelAggTOMs,FuelAggTOMDS,Year) = data
  (; BaseSw,EndTime,Q,Qe) = data
  ...
  for areatom in areatoms
    for fuelaggtom in FuelAggTOMs
      for year in years
        TTT[year] = Q[fuelaggtom,areatom,year]
        EEE[year] = Qe[fuelaggtom,areatom,year]
        DIF[year] = (TTT[year]-EEE[year])
        PDIF[year] = (TTT[year]-EEE[year])/EEE[year]
        println(iob,"Q;Energy Production (TBtu/Yr);",AreaTOMDS[areatom],
  ...
  filename = "zQ_Production-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename),"w") do filename
    write(filename, String(take!(iob)))
  end
end

function zQ_Production_DtaControl(db)
  data = zQ_ProductionData(; db)
  (; AreaTOMs,Nation)= data
  (; MapAreaTOMNation) = data

  @info "zQ_Production_DtaControl"

  CN = Select(Nation,"CN")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,CN] .== 1.0)
  zQ_Production_DtaRun(data,Nation[CN],areatoms)

  US = Select(Nation,"US")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,US] .== 1.0)
  zQ_Production_DtaRun(data,Nation[US],areatoms)
end
if abspath(PROGRAM_FILE) == @__FILE__
  zQ_Production_DtaControl(DB)
end
