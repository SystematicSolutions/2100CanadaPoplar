#
# EInt_EnergyIntensity_Trans.txo - Comparison of TOM baseline values to E2020 transfers
#
using EnergyModel
...
Base.@kwdef struct zEInt_EnergyIntensityTransData
  db::String
...
function EnergyIntensityTrans_DtaRun(data,nationkey,areatoms)
  (; AreaTOM,AreaTOMDS,FuelTOM,FuelTOMs,FuelTOMDS) = data
  (; ToTOMVariable,VehicleTOM,VehicleTOMDS,VehicleTOMs,Year) = data
  (; BaseSw,EndTime,EIntTr,EIntETr,IsActiveToFuelTOM,TOMBaseTime) = data
  (; DIF,EEE,PDIF,TTT,SceName) = data

  iob = IOBuffer()
  println(iob,"Variable;Title;Area;Fuel;VehicleTOM;Year;ENERGY2020;TOM;DIF;PercentDIF;AnnualDIF")
    ...
    for fueltom in fueltoms
      for vehicletom in VehicleTOMs
        for year in years
          ...
          PDIF[year] = (DIF[year]/EEE[year])*100 
        end
        TotYears = sum(EEE[year] for year in years)
        if TotYears != 0
          for year in years
            AnnualDif = ((EEE[year]-EEE[year-1])/EEE[year-1])*100
            ...
            #
            # TODOJulia - How do we check if year is 1986? 6/3/25 R.Levesque
            #            if (EEE[year] != 0) && (EEE[year-1] == 0) && (Year[year] != "1986")
            if (EEE[year] != 0) && (EEE[year-1] == 0)
              AnnualDif = 100
            end

            println(iob,"EIntTr;Energy Intensity (mmBtu/$TOMBaseTime \$1000);",AreaTOMDS[areatom],";",
              FuelTOMDS[fueltom],";",VehicleTOMDS[vehicletom],";",Year[year],";",EEE[year],";",TTT[year],
  ...
  filename = "zEInt_EnergyIntensity_Trans-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename),"w") do filename
    write(filename, String(take!(iob)))
  end
end

function zEInt_EnergyIntensity_Trans_DtaControl(db)
  data = zEInt_EnergyIntensityTransData(; db)
  (; AreaTOMs,Nation)= data
  (; MapAreaTOMNation) = data

  @info "zEInt_EnergyIntensity_Trans_DtaControl"

  CN = Select(Nation,"CN")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,CN] .== 1.0)
  EnergyIntensityTrans_DtaRun(data,Nation[CN],areatoms)

  US = Select(Nation,"US")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,US] .== 1.0)
  EnergyIntensityTrans_DtaRun(data,Nation[US],areatoms)
end
if abspath(PROGRAM_FILE) == @__FILE__
  zEInt_EnergyIntensity_Trans_DtaControl(DB)
end
