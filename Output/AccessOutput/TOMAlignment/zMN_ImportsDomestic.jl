#
# zMN_ImportsDomestic.jl - Comparison of TOM baseline values to E2020 transfers
#
using EnergyModel
...
Base.@kwdef struct zMN_ImportsDomesticData
  db::String
  ...
  MN::VariableArray{3} = ReadDisk(db,"KOutput/MN") # [FuelAggTOM,AreaTOM,Year] Domestic Imports (TBtu/Yr)
...
function zImportsDomestic_DtaRun(data,nationkey,areatoms)
  (; AreaTOM,AreaTOMDS,FuelAggTOMs,FuelAggTOMDS,Year) = data
  (; BaseSw,EndTime,MN,MNe) = data
  ...
  for areatom in areatoms
    for fuelaggtom in FuelAggTOMs
      for year in years
        TTT[year] = MN[fuelaggtom,areatom,year]
        EEE[year] = MNe[fuelaggtom,areatom,year]
        DIF[year] = (TTT[year]-EEE[year])
        @finite_math PDIF[year] = (TTT[year]-EEE[year])/EEE[year]
        println(iob,"MN;Domestic Imports (TBtu/Yr);",AreaTOMDS[areatom],
  ...
  filename = "zMN_ImportsDomestic-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename),"w") do filename
    write(filename, String(take!(iob)))
  end
end

function zMN_ImportsDomestic_DtaControl(db)
  data = zMN_ImportsDomesticData(; db)
  (; AreaTOMs,Nation)= data
  (; MapAreaTOMNation) = data

  @info "zMN_ImportsDomestic_DtaControl"

  CN = Select(Nation,"CN")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,CN] .== 1.0)
  zImportsDomestic_DtaRun(data,Nation[CN],areatoms)

  US = Select(Nation,"US")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,US] .== 1.0)
  zImportsDomestic_DtaRun(data,Nation[US],areatoms)
end
  
if abspath(PROGRAM_FILE) == @__FILE__
  zMN_ImportsDomestic_DtaControl(DB)
end
