#
# zEInt_EnergyIntensity_Spikes.jl - Comparison of TOM baseline values to E2020 transfers
#
using EnergyModel
...
Base.@kwdef struct zEInt_EnergyIntensity_SpikesData
  db::String
  ...
  HasAnnualSpike::VariableArray{4} = zeros(Float32,length(FuelTOM),length(ECCTOM),length(AreaTOM),length(Year))
  PDIF = zeros(Float32,length(Year))
  TTT = zeros(Float32,length(Year))
end

function EnergyIntensity_Spikes_DtaRun(data,nationkey,areatoms)
  (; AreaTOM,AreaTOMDS,ECCTOM,ECCTOMs,ECCTOMDS) = data
  (; FuelTOM,FuelTOMs,FuelTOMDS,ToTOMVariable,Year) = data
  (; BaseSw,EndTime,EInt,EIntE) = data
  (; DIF,EEE,HasAnnualSpike,IsActiveToECCTOM,PDIF,TOMBaseTime,TTT,SceName) = data
        ...
        if TotYears != 0
          for year in years
            ...
            if (EEE[year] != 0) && (EEE[year-1] == 0) && (year !=Yr(1986))
              AnnualDif = 100
              HasAnnualSpike[fueltom,ecctom,areatom,year] = 1
  ...
  for areatom in areatoms
    for fueltom in FuelTOMs
      for ecctom in ecctoms
        ...
        TotYears = sum(EEE[year] for year in years)
        if TotYears != 0
          if sum(HasAnnualSpike[fueltom,ecctom,areatom,year] for year in years) > 0
            for year in years
              @finite_math AnnualDif = ((EEE[year]-EEE[year-1])/EEE[year-1])*100
              if (EEE[year] != 0) && (EEE[year-1] == 0)
                AnnualDif = 100
              end
              println(iob,"EInt;Energy Intensity (mmBtu/$TOMBaseTime \$1000);",AreaTOMDS[areatom],";",
                FuelTOMDS[fueltom],";",ECCTOMDS[ecctom],";",Year[year],";",EEE[year],";",TTT[year],";",
                DIF[year],";",PDIF[year],";",AnnualDif)
            end
  ...
  filename = "zEInt_EnergyIntensity_Spikes-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename),"w") do filename
    write(filename, String(take!(iob)))
  end
end

function zEInt_EnergyIntensity_Spikes_DtaControl(db)
  data = zEInt_EnergyIntensity_SpikesData(; db)
  (; AreaTOMs,Nation)= data
  (; MapAreaTOMNation) = data

  @info "zEInt_EnergyIntensity_Spikes_DtaControl"

  CN = Select(Nation,"CN")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,CN] .== 1.0)
  EnergyIntensity_Spikes_DtaRun(data,Nation[CN],areatoms)

  US = Select(Nation,"US")
  areatoms = findall(MapAreaTOMNation[AreaTOMs,US] .== 1.0)
  EnergyIntensity_Spikes_DtaRun(data,Nation[US],areatoms)
end
if abspath(PROGRAM_FILE) == @__FILE__
  zEInt_EnergyIntensity_Spikes_DtaControl(DB)
end
