...
Base.@kwdef struct zFPECCData
  ...
  zFPECC::VariableArray{4} = ReadDisk(db,"SOutput/FPECC") # [Fuel,ECC,Area,Year] Delivered Fuel Price ($/mmBtu)
  zFPECCRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/FPECC") # [Fuel,ECC,Area,Year] Delivered Fuel Price ($/mmBtu)
...
function zFPECC_DtaRun(data,iob,nation,CurrencyConversion)
  ...
  (; InflationNation,InflationNationRef,TotDemand,TotDemandRef,zFPECC,zFPECCRef) = data
  ...
  for ecc in ECCs, fuel in Fuels, year in years
    ...
    @finite_math National[fuel,ecc,year] = sum(zFPECC[fuel,ecc,area,year]*DmdWeight[fuel,ecc,area,year] for area in areas)/
                                           sum(DmdWeight[fuel,ecc,area,year] for area in areas)
    @finite_math NationalRef[fuel,ecc,year] = sum(zFPECCRef[fuel,ecc,area,year]*DmdWeight[fuel,ecc,area,year] for area in areas)/
                                           sum(DmdWeight[fuel,ecc,area,year] for area in areas)
  end
    ...
    for ecc in ECCs
      for fuel in Fuels
        ...
        if (ZZZ[year] > 0.000001) || (CCC[year] > 0.000001)
          println(iob,"zFPECC;",Year[year],";",NationDS[nation],";",ECCDS[ecc],";",
  ...
  for year in years
    for area in areas
      for ecc in ECCs
        for fuel in Fuels
          @finite_math ZZZ[year] = zFPECC[fuel,ecc,area,year]*Conversion[nation,year]*vECF[fuel,area,year]*
                                   TotDemand[fuel,ecc,area,year]/TotDemand[fuel,ecc,area,year]
          @finite_math CCC[year] = zFPECCRef[fuel,ecc,area,year]*Conversion[nation,year]*vECF[fuel,area,year]*
                                   TotDemandRef[fuel,ecc,area,year]/TotDemandRef[fuel,ecc,area,year]
          if (ZZZ[year] > 0.000001) || (CCC[year] > 0.000001)
            println(iob,"zFPECC;",Year[year],";",AreaDS[area],";",ECCDS[ecc],";",
              FuelDS[fuel],";",UnitsDS[nation],vECFUnits[fuel],";",@sprintf("%.6E",ZZZ[year]),";",@sprintf("%.6E",CCC[year]))
          end
        end
      end
...
function CreatezFPECCOutputFile(db,iob,nationkey,SceName)
  filename = "zFPECC-$nationkey-$SceName.dta"
...
function zFPECC_DtaControl(db)
  data = zFPECCData(; db)
  (; Nation)= data
  (; NationOutputMap,SceName)= data

  @info "zFPECC_DtaControl"
  ...
  for nation in nations
    if NationOutputMap[nation] == 1
      ...
      zFPECC_DtaRun(data,iob,nation,CurrencyConversion)
      CurrencyConversion="RealPolicy"
      zFPECC_DtaRun(data,iob,nation,CurrencyConversion)
      CurrencyConversion="NominalReference"
      zFPECC_DtaRun(data,iob,nation,CurrencyConversion)
      CurrencyConversion="RealReference"
      zFPECC_DtaRun(data,iob,nation,CurrencyConversion)
      nationkey = Nation[nation]
      CreatezFPECCOutputFile(data,iob,nationkey,SceName)
    end
  end
end
if abspath(PROGRAM_FILE) == @__FILE__
zFPECC_DtaControl(DB)
end
