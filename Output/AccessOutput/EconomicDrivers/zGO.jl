...
Base.@kwdef struct zGOData
  ...
  zGO::VariableArray{3} = ReadDisk(db, "MOutput/GO") # [ECC,Area,Year] Economic Driver (Various Units)
  zGORef::VariableArray{3} = ReadDisk(RefNameDB, "MOutput/GO") # [ECC,Area,Year] Economic Driver (Various Units)
...
function zGO_DtaRun(data,nation)
  ...
  for ecc in ECCs
    for area in areas
      for year in years
        ZZZ[year] = zGO[ecc,area,year]*Conversion[nation,year]
        CCC[year] = zGORef[ecc,area,year]*Conversion[nation,year]
        if ZZZ[year] != 0 || CCC[year] != 0
          zData = @sprintf("%.6E",ZZZ[year])
          zInitial = @sprintf("%.6E",CCC[year])
          println(iob,"zGO;",Year[year],";",AreaDS[area],";",ECCDS[ecc],";",
  ...
  filename = "zGO-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function zGO_DtaControl(db,)
  data = zGOData(; db)
  (; Nation)= data
