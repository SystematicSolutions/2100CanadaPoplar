...
function zFPECCCFS_DtaRun(data,iob,nation,CurrencyConversion)
  ...
  for ecc in ECCs, fuel in Fuels, year in years
    ...
    @finite_math National[fuel,ecc,year] = sum(zFPECCCFS[fuel,ecc,area,year]*DmdWeight[fuel,ecc,area,year] for area in areas)/
                                           sum(DmdWeight[fuel,ecc,area,year] for area in areas)
    @finite_math NationalRef[fuel,ecc,year] = sum(zFPECCCFSRef[fuel,ecc,area,year]*DmdWeight[fuel,ecc,area,year] for area in areas)/
                                           sum(DmdWeight[fuel,ecc,area,year] for area in areas)
  end
    ...
    for ecc in ECCs
      for fuel in Fuels
        ...
        if (ZZZ[year] > 0.000001) || (CCC[year] > 0.000001)
          println(iob,"zFPECCCFS;",Year[year],";",NationDS[nation],";",ECCDS[ecc],";",
  ...
  for year in years
    for area in areas
      for ecc in ECCs
        for fuel in Fuels
          @finite_math ZZZ[year] = zFPECCCFS[fuel,ecc,area,year]*Conversion[nation,year]*vECF[fuel,area,year]*
                                   TotDemand[fuel,ecc,area,year]/TotDemand[fuel,ecc,area,year]
          @finite_math CCC[year] = zFPECCCFSRef[fuel,ecc,area,year]*Conversion[nation,year]*vECF[fuel,area,year]*
                                   TotDemandRef[fuel,ecc,area,year]/TotDemandRef[fuel,ecc,area,year]
          if (ZZZ[year] > 0.000001) || (CCC[year] > 0.000001)
            println(iob,"zFPECCCFS;",Year[year],";",AreaDS[area],";",ECCDS[ecc],";",
              FuelDS[fuel],";",UnitsDS[nation],vECFUnits[fuel],";",@sprintf("%.6E",ZZZ[year]),";",@sprintf("%.6E",CCC[year]))
          end
        end
      end
...
function CreatezFPECCCFSOutputFile(db,iob,nationkey,SceName)
  filename = "zFPECCCFS-$nationkey-$SceName.dta"
