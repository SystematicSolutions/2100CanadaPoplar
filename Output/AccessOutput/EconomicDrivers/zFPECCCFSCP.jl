...
Base.@kwdef struct zFPECCCFSCPData
  db::String
  ...
  zFPECCCFSCP::VariableArray{4} = ReadDisk(db,"SOutput/FPECCCFSCP") # [Fuel,ECC,Area,Year] Fuel Prices w/CFS and Carbon Price ($/mmBtu)
  zFPECCCFSCPRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/FPECCCFSCP") # [Fuel,ECC,Area,Year] Fuel Prices w/CFS and Carbon Price ($/mmBtu)
...
function zFPECCCFSCP_DtaRun(data,iob,nation,CurrencyConversion)
  ...
  (; InflationNation,InflationNationRef,TotDemand,TotDemandRef,zFPECCCFSCP,zFPECCCFSCPRef) = data
  (; CCC,Conversion,DmdWeight,National,NationalRef,KJBtu,UnitsDS,vECF,vECFUnits,ZZZ) = data

  if BaseSw != 0
    zFPECCCFSCPRef = zFPECCCFSCP
  end
  ...
  for ecc in ECCs, fuel in Fuels, year in years
    ...
    @finite_math National[fuel,ecc,year] = sum(zFPECCCFSCP[fuel,ecc,area,year]*DmdWeight[fuel,ecc,area,year] for area in areas)/
                                           sum(DmdWeight[fuel,ecc,area,year] for area in areas)
    @finite_math NationalRef[fuel,ecc,year] = sum(zFPECCCFSCPRef[fuel,ecc,area,year]*DmdWeight[fuel,ecc,area,year] for area in areas)/
                                           sum(DmdWeight[fuel,ecc,area,year] for area in areas)
  end
    ...
    for ecc in ECCs
      for fuel in Fuels
        ...
        if (ZZZ[year] > 0.000001) || (CCC[year] > 0.000001)
          println(iob,"zFPECCCFSCP;",Year[year],";",NationDS[nation],";",ECCDS[ecc],";",
  ...
  for year in years
    for area in areas
      for ecc in ECCs
        for fuel in Fuels
          @finite_math ZZZ[year] = zFPECCCFSCP[fuel,ecc,area,year]*Conversion[nation,year]*vECF[fuel,area,year]*
                                   TotDemand[fuel,ecc,area,year]/TotDemand[fuel,ecc,area,year]
          @finite_math CCC[year] = zFPECCCFSCPRef[fuel,ecc,area,year]*Conversion[nation,year]*vECF[fuel,area,year]*
                                   TotDemandRef[fuel,ecc,area,year]/TotDemandRef[fuel,ecc,area,year]
          if (ZZZ[year] > 0.000001) || (CCC[year] > 0.000001)
            println(iob,"zFPECCCFSCP;",Year[year],";",AreaDS[area],";",ECCDS[ecc],";",
              FuelDS[fuel],";",UnitsDS[nation],vECFUnits[fuel],";",@sprintf("%.6E",ZZZ[year]),";",@sprintf("%.6E",CCC[year]))
          end
        end
      end
...
function CreatezFPECCCFSCPOutputFile(db,iob,nationkey,SceName)
  filename = "zFPECCCFSCP-$nationkey-$SceName.dta"
...
function zFPECCCFSCP_DtaControl(db)
  data = zFPECCCFSCPData(; db)
  (; Nation)= data
  (; NationOutputMap,SceName)= data

  @info "zFPECCCFSCP_DtaControl"
  ...
  for nation in nations
    if NationOutputMap[nation] == 1
      ...
      zFPECCCFSCP_DtaRun(data,iob,nation,CurrencyConversion)
      CurrencyConversion="RealPolicy"
      zFPECCCFSCP_DtaRun(data,iob,nation,CurrencyConversion)
      nationkey = Nation[nation]
      CreatezFPECCCFSCPOutputFile(data,iob,nationkey,SceName)
    end
  end
end
if abspath(PROGRAM_FILE) == @__FILE__
zFPECCCFSCP_DtaControl(DB)
end
