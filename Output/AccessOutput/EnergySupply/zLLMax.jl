...
Base.@kwdef struct zLLMaxData
  ...
  LLMaxRef::VariableArray{5} = ReadDisk(RefNameDB,"EGInput/LLMax") #[Node,NodeX,TimeP,Month,Year]  Maximum Loading on Transmission Lines (MW)
...
function AssignzLLMaxNodes(data)
  (; Area,AreaDS,Areas,Node,NodeDS,Nodes,SceName) = data
  (; NodeX,NodeXDS,NodeXs,Year) = data
  (; CanNode,CanNodeX,NodeName,NodeXName) = data
...
function zLLMax_DtaRun(data,nation)
  (; Area,AreaDS,Areas,Nation,NationDS,Nations,Year,SceName) = data
  (; MonthDS,Months,Node,NodeDS,Nodes,NodeX,NodeXDS,NodeXs,TimePDS,TimePs) = data
  (; ANMap,BaseSw,CanNode,CanNodeX,CCC,EndTime) = data
  (; LLMax,LLMaxRef,NodeName,NodeXName,ZZZ) = data

  NodeName,NodeXName = AssignzLLMaxNodes(data)
  ...
  for nodex in NodeXs
    for node in Nodes
      ZZZSum = sum(LLMax[node,nodex,timep,month,year] for year in years, month in Months, timep in TimePs)
      if CanNode[node] == 1 || CanNodeX[nodex] == 1
        if ZZZSum > 0.0
          for year in years
            for month in Months
              for timep in TimePs
                ZZZ[year] = LLMax[node,nodex,timep,month,year]
                CCC[year] = LLMax[node,nodex,timep,month,year]
                println(iob,"zLLMax;", Year[year],";",NodeXName[nodex],";",NodeName[node],";",
  ...
  filename = "zLLMax-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end
 
function zLLMax_DtaControl(db)
  data = zLLMaxData(; db)
  (; db,Nation,Nations)= data
  (; NationOutputMap)= data

  @info "zLLMax_DtaControl"

  for nation in Nations
    if NationOutputMap[nation] == 1
      zLLMax_DtaRun(data,nation)
    end
  end
end
if abspath(PROGRAM_FILE) == @__FILE__
  zLLMax_DtaControl(DB)
end
