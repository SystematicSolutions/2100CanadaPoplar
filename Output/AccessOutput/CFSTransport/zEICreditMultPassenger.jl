#
# zEICreditMultPassenger.jl
#

Base.@kwdef struct zEICreditMultPassengerData
  db::String
...
function zEICreditMultPassenger_DtaRun(data,nation,SceName)
  ...
  # Select EC(Passenger) - only process Passenger sector
  ...
  for enduse in Enduses
    for fuel in Fuels
      for tech in Techs
        for area in areas
          for year in years
            ZZZ = zEICreditMult[enduse,fuel,tech,passenger_ec,area,year]*Conversion[nation,year]
            CCC = zEICreditMultRef[enduse,fuel,tech,passenger_ec,area,year]*Conversion[nation,year]
            if ZZZ != 0 || CCC != 0
              println(iob,"zEICreditMult;",YearDS[year],";",AreaDS[area],";",ECDS[passenger_ec],";",TechDS[tech],";",
  ...
  filename = "zEICreditMultPassenger-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end
 
function zEICreditMultPassenger_DtaControl(db,SceName)
  data = zEICreditMultPassengerData(; db)
  (; db,Nation,Nations)= data
  (; NationOutputMap)= data

  @info "zEICreditMultPassenger_DtaControl"

  for nation in Nations
    if NationOutputMap[nation] == 1
      zEICreditMultPassenger_DtaRun(data,nation,SceName)
    end
  end
end