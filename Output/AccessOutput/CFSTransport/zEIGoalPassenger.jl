...
Base.@kwdef struct zEIGoalPassengerData
  db::String
...
function zEIGoalPassenger_DtaRun(data,nation,SceName)
  ...
  filename = "zEIGoalPassenger-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end
 
function zEIGoalPassenger_DtaControl(db,SceName)
  data = zEIGoalPassengerData(; db)
  (; db,Nation,Nations)= data
  (; NationOutputMap)= data

  @info "zEIGoalPassenger_DtaControl"

  for nation in Nations
    if NationOutputMap[nation] == 1
      zEIGoalPassenger_DtaRun(data,nation,SceName)
    end
  end
end