...
Base.@kwdef struct zTrORMEPolData
  db::String
  ...
  zTrORMEPol::VariableArray{4} = ReadDisk(db,"SOutput/ORMEPol") #[ECC,Poll,Area,Year] Off-Road Pollution (Tonnes/Yr)
  zTrORMEPolRef::VariableArray{4} = ReadDisk(RefNameDB,"SOutput/ORMEPol") #[ECC,Poll,Area,Year] Off-Road Pollution (Tonnes/Yr) 
...
function zTrORMEPol_DtaRun(data,polls,nation)
  ...
  for ecc in ECCs
    for poll in polls
      for area in areas
        for year in years
          ZZZ[year] = zTrORMEPol[ecc,poll,area,year]*Conversion[poll]
          CCC[year] = zTrORMEPolRef[ecc,poll,area,year]*Conversion[poll]
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zTrORMEPol;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
  ...
  if polltype == "GHG"
    for ecc in ECCs
      for area in areas
        for year in years
          ZZZ[year] = sum(zTrORMEPol[ecc,poll,area,year]*Conversion[poll] for poll in polls)/1000
          CCC[year] = sum(zTrORMEPolRef[ecc,poll,area,year]*Conversion[poll] for poll in Polls)/1000
          if ZZZ[year] != 0.0 || CCC[year] != 0.0
            zData = @sprintf("%.6E",ZZZ[year])
            zInitial = @sprintf("%.6E",CCC[year])
            println(iob,"zTrORMEPol;",Year[year],";",AreaDS[area],";",polltype,";",
  ...
  filename = "zTrORMEPol-$polltype-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function zTrORMEPol_DtaControl(db)
  data = zTrORMEPolData(; db)
  (; db,Nation,Nations,Poll)= data
  (; ANMap,NationOutputMap)= data

  @info "zTrORMEPol_DtaControl"

  for nation in Nations
    if NationOutputMap[nation] == 1
      polls = Select(Poll,["SOX","COX","NOX","PMT","VOC","PM25","PM10","Hg","NH3","BC"])
      zTrORMEPol_DtaRun(data,polls,nation)
