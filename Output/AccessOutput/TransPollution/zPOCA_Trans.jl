...
Base.@kwdef struct TzPOCAData
  ...
  zPOCA::VariableArray{7} = ReadDisk(db,"$Outpt/POCA") # Average Pollution Coefficients (Tonnes/TBtu) [Enduse,FuelEP,Tech,EC,Poll,Area,Year]
  zPOCARef::VariableArray{7} = ReadDisk(RefNameDB,"$Outpt/POCA") # Average Pollution Coefficients (Tonnes/TBtu) [Enduse,FuelEP,Tech,EC,Poll,Area,Year]
...
function zPOCA_DtaRun_Transportation(db, nation, polls, polltype)
  data = TzPOCAData(; db)
  (; Area, AreaDS, Areas, EC, ECDS, ECs, Enduse, EnduseDS, Enduses) = data
  (; FuelEP, FuelEPDS, FuelEPs, Nation, NationDS, Nations, Poll, PollDS, Polls) = data
  (; Tech, TechDS, Techs, Year, SceName) = data
  ...
  iob = IOBuffer()
  println(iob,"Variable;Year;Area;Pollutant;Sector;Technology;Fuel;Enduse;Units;zData;zInitial")
        ...
        for fuelep in FuelEPs
          for tech in Techs
            for ec in ECs
              ...
              for poll in polls
                for area in areas
                  ZZZ = zPOCA[enduse,fuelep,tech,ec,poll,area,year] * Conversion[poll]
                  CCC = zPOCARef[enduse,fuelep,tech,ec,poll,area,year] * Conversion[poll]
                  
                  if ZZZ > 0.01 || CCC > 0.01
                    println(iob,"zPOCA;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
                      ECDS[ec],";",TechDS[tech],";",FuelEPDS[fuelep],";",EnduseDS[enduse],";",UnitsDS[poll],";",
  ...
  filename = "zPOCA_Trans-$polltype-$nationkey-$SceName.dta"
...
function zPOCA_Trans_DtaControl(db)
  ...
  for nation in nations
    ...
    zPOCA_DtaRun_Transportation(db, nation, polls, PollType)
    
  end

end
if abspath(PROGRAM_FILE) == @__FILE__
  zPOCA_Trans_DtaControl(DB)
end
