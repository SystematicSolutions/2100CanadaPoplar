...
Base.@kwdef struct RzFsPOCXData
  db::String
  ...
  zFsPOCX::VariableArray{5} = ReadDisk(db, "$Input/FsPOCX") # [Fuel,EC,Poll,Area,Year] Feedstock Marginal Pollution Coefficients (Tonnes/TBtu)
  zFsPOCXRef::VariableArray{5} = ReadDisk(RefNameDB, "$Input/FsPOCX") # [Fuel,EC,Poll,Area,Year] Feedstock Marginal Pollution Coefficients (Tonnes/TBtu)
...
Base.@kwdef struct CzFsPOCXData
  db::String
...
Base.@kwdef struct IzFsPOCXData
  db::String
...
function zFsPOCX_DtaRun_Residential(db, iob, nation, polls, SceName)
  data = RzFsPOCXData(; db)
  (; Area, AreaDS, Areas, Nation, NationDS, Nations) = data
  (; Poll, PollDS, Polls, Fuel, FuelDS, Fuels, EC, ECDS, ECs, Year) = data
    ...
    for year in years
      for fuel in Fuels
        for ec in ECs
          for poll in polls
            for area in areas
              ZZZ = zFsPOCX[fuel,ec,poll,area,year] * Conversion[poll]
              CCC = zFsPOCXRef[fuel,ec,poll,area,year] * Conversion[poll]
              
              if ZZZ > 0.01 || CCC > 0.01
                # Use first enduse as default since EnduseDS is referenced in output but not clear how it maps
                enduse_str = length(EnduseDS) > 0 ? EnduseDS[1] : ""
                println(iob,"zFsPOCX;",Year[year],";",AreaDS[area],";",PollDS[poll],";",
                  ECDS[ec],";",FuelDS[fuel],";",enduse_str,";",UnitsDS[poll],";",
...
function zFsPOCX_DtaRun_Commercial(db, iob, nation, polls, SceName)
  data = CzFsPOCXData(; db)
  (; Area, AreaDS, Areas, Nation, NationDS, Nations) = data
  (; Poll, PollDS, Polls, Fuel, FuelDS, Fuels, EC, ECDS, ECs, Year) = data
  (; Enduse, EnduseDS, Enduses) = data
  (; ANMap, BaseSw, Conversion, EndTime, UnitsDS, zFsPOCX, zFsPOCXRef, NationOutputMap) = data
...
function zFsPOCX_DtaRun_Industrial(db, iob, nation, polls, SceName)
  data = IzFsPOCXData(; db)
  (; Area, AreaDS, Areas, Nation, NationDS, Nations) = data
  (; Poll, PollDS, Polls, Fuel, FuelDS, Fuels, EC, ECDS, ECs, Year) = data
  (; Enduse, EnduseDS, Enduses) = data
  (; ANMap, BaseSw, Conversion, EndTime, UnitsDS, zFsPOCX, zFsPOCXRef, NationOutputMap) = data
...
function zFsPOCX_CreateOutputFile(iob,PollType,nationkey,SceName)
  filename = "zFsPOCX-$PollType-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end

function zFsPOCX_DtaControl(db)
  ...
  for nation in nations
    ...
    zFsPOCX_DtaRun_Residential(db, iob, nation, polls, SceName)
    zFsPOCX_DtaRun_Commercial(db, iob, nation, polls, SceName)
    zFsPOCX_DtaRun_Industrial(db, iob, nation, polls, SceName)
    
    zFsPOCX_CreateOutputFile(iob,PollType,Nation[nation],SceName)
  end

end

if abspath(PROGRAM_FILE) == @__FILE__
  zFsPOCX_DtaControl(DB)
end
