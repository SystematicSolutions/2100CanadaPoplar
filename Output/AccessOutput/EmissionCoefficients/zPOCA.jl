#
# zPOCA.jl
#
using EnergyModel
import ...EnergyModel: ReadDisk,WriteDisk,Select,DT
import ...EnergyModel: ITime,HisTime,MaxTime,Zero,First,Last,Future,Final,Yr
import ...EnergyModel: @finite_math,EnergyModel,finite_inverse,finite_divide,finite_power,finite_exp,finite_log
import ...EnergyModel: DB
using   ..EnergyModel: HDF5DataSetNotFoundException,E2020Folder,OutputFolder,rm_dir_contents

using HDF5,DataFrames,CSV,Printf

const VariableArray{N} = Array{Float32,N} where {N}
const SetArray = Vector{String}

# Residential Sector Data Structure
Base.@kwdef struct RzPOCAData
  db::String

  RefNameDB::String = ReadDisk(db,"MainDB/RefNameDB") # Reference Case Name
  Area::SetArray = ReadDisk(db, "MainDB/AreaKey")
  AreaDS::SetArray = ReadDisk(db, "MainDB/AreaDS")
  Areas::Vector{Int} = collect(Select(Area))
  Nation::SetArray = ReadDisk(db, "MainDB/NationKey")
  NationDS::SetArray = ReadDisk(db, "MainDB/NationDS")
  Nations::Vector{Int} = collect(Select(Nation))
  Year::SetArray = ReadDisk(db, "MainDB/YearKey")
  Poll::SetArray = ReadDisk(db, "MainDB/PollKey")
  PollDS::SetArray = ReadDisk(db, "MainDB/PollDS")
  Polls::Vector{Int} = collect(Select(Poll))
  FuelEP::SetArray = ReadDisk(db, "MainDB/FuelEPKey")
  FuelEPDS::SetArray = ReadDisk(db, "MainDB/FuelEPDS")
  FuelEPs::Vector{Int} = collect(Select(FuelEP))
  
  CalDB::String = "RCalDB"
  Input::String = "RInput"
  Outpt::String = "ROutput"
  Outpt2::String = "ROutput"
  
  EC::SetArray = ReadDisk(db, "$Input/ECKey")
  ECDS::SetArray = ReadDisk(db, "$Input/ECDS")
  ECs::Vector{Int} = collect(Select(EC))
  Enduse::SetArray = ReadDisk(db, "$Input/EnduseKey")
  EnduseDS::SetArray = ReadDisk(db, "$Input/EnduseDS")
  Enduses::Vector{Int} = collect(Select(Enduse))
  
  ANMap::VariableArray{2} = ReadDisk(db, "MainDB/ANMap") # [Area,Nation] Map between Area and Nation
  BaseSw::Float32 = ReadDisk(db, "SInput/BaseSw")[1]
  EndTime::Float32 = ReadDisk(db,"SInput/EndTime")[1] #[tv]  Ending Year for Simulation (Year)
  zPOCA::VariableArray{6} = ReadDisk(db, "$Outpt2/POCA") # [Enduse,FuelEP,EC,Poll,Area,Year] Pollution (Tonnes/Yr)
  zPOCARef::VariableArray{6} = ReadDisk(RefNameDB, "$Outpt2/POCA") # [Enduse,FuelEP,EC,Poll,Area,Year] Pollution (Tonnes/Yr)
  NationOutputMap::VariableArray{1} = ReadDisk(db,"SInput/NationOutputMap") #[Nation] Map for Output Control by Nation (0=No Output)(Map)
  PolConv::VariableArray{1} = ReadDisk(db, "SInput/PolConv") # Pollution Conversion Factor (convert GHGs to eCO2)

  SceName::String = ReadDisk(db,"SInput/SceName") #  Scenario Name
  
  #
  # Scratch Variables
  #
  Conversion::VariableArray{1} = zeros(Float32,length(Poll)) # [Poll] Units Conversion Factor
  UnitsDS::SetArray = fill("",length(Poll)) # [Poll] Units Description

end

# Commercial Sector Data Structure
Base.@kwdef struct CzPOCAData
  db::String

  RefNameDB::String = ReadDisk(db,"MainDB/RefNameDB") # Reference Case Name
  Area::SetArray = ReadDisk(db, "MainDB/AreaKey")
  AreaDS::SetArray = ReadDisk(db, "MainDB/AreaDS")
  Areas::Vector{Int} = collect(Select(Area))
  Nation::SetArray = ReadDisk(db, "MainDB/NationKey")
  NationDS::SetArray = ReadDisk(db, "MainDB/NationDS")
  Nations::Vector{Int} = collect(Select(Nation))
  Year::SetArray = ReadDisk(db, "MainDB/YearKey")
  Poll::SetArray = ReadDisk(db, "MainDB/PollKey")
  PollDS::SetArray = ReadDisk(db, "MainDB/PollDS")
  Polls::Vector{Int} = collect(Select(Poll))
  FuelEP::SetArray = ReadDisk(db, "MainDB/FuelEPKey")
  FuelEPDS::SetArray = ReadDisk(db, "MainDB/FuelEPDS")
  FuelEPs::Vector{Int} = collect(Select(FuelEP))
  
  CalDB::String = "CCalDB"
  Input::String = "CInput"
  Outpt::String = "COutput"
  Outpt2::String = "COutput"
  
  EC::SetArray = ReadDisk(db, "$Input/ECKey")
  ECDS::SetArray = ReadDisk(db, "$Input/ECDS")
  ECs::Vector{Int} = collect(Select(EC))
  Enduse::SetArray = ReadDisk(db, "$Input/EnduseKey")
  EnduseDS::SetArray = ReadDisk(db, "$Input/EnduseDS")
  Enduses::Vector{Int} = collect(Select(Enduse))
  
  ANMap::VariableArray{2} = ReadDisk(db, "MainDB/ANMap") # [Area,Nation] Map between Area and Nation
  BaseSw::Float32 = ReadDisk(db, "SInput/BaseSw")[1]
  EndTime::Float32 = ReadDisk(db,"SInput/EndTime")[1] #[tv]  Ending Year for Simulation (Year)
  zPOCA::VariableArray{6} = ReadDisk(db, "$Outpt2/POCA") # [Enduse,FuelEP,EC,Poll,Area,Year] Pollution (Tonnes/Yr)
  zPOCARef::VariableArray{6} = ReadDisk(RefNameDB, "$Outpt2/POCA") # [Enduse,FuelEP,EC,Poll,Area,Year] Pollution (Tonnes/Yr)
  NationOutputMap::VariableArray{1} = ReadDisk(db,"SInput/NationOutputMap") #[Nation] Map for Output Control by Nation (0=No Output)(Map)
  PolConv::VariableArray{1} = ReadDisk(db, "SInput/PolConv") # Pollution Conversion Factor (convert GHGs to eCO2)

  SceName::String = ReadDisk(db,"SInput/SceName") #  Scenario Name
  
  #
  # Scratch Variables
  #
  Conversion::VariableArray{1} = zeros(Float32,length(Poll)) # [Poll] Units Conversion Factor
  UnitsDS::SetArray = fill("",length(Poll)) # [Poll] Units Description

end

# Industrial Sector Data Structure
Base.@kwdef struct IzPOCAData
  db::String

  RefNameDB::String = ReadDisk(db,"MainDB/RefNameDB") # Reference Case Name
  Area::SetArray = ReadDisk(db, "MainDB/AreaKey")
  AreaDS::SetArray = ReadDisk(db, "MainDB/AreaDS")
  Areas::Vector{Int} = collect(Select(Area))
  Nation::SetArray = ReadDisk(db, "MainDB/NationKey")
  NationDS::SetArray = ReadDisk(db, "MainDB/NationDS")
  Nations::Vector{Int} = collect(Select(Nation))
  Year::SetArray = ReadDisk(db, "MainDB/YearKey")
  Poll::SetArray = ReadDisk(db, "MainDB/PollKey")
  PollDS::SetArray = ReadDisk(db, "MainDB/PollDS")
  Polls::Vector{Int} = collect(Select(Poll))
  FuelEP::SetArray = ReadDisk(db, "MainDB/FuelEPKey")
  FuelEPDS::SetArray = ReadDisk(db, "MainDB/FuelEPDS")
  FuelEPs::Vector{Int} = collect(Select(FuelEP))
  
  CalDB::String = "ICalDB"
  Input::String = "IInput"
  Outpt::String = "IOutput"
  Outpt2::String = "IOutput"
  
  EC::SetArray = ReadDisk(db, "$Input/ECKey")
  ECDS::SetArray = ReadDisk(db, "$Input/ECDS")
  ECs::Vector{Int} = collect(Select(EC))
  Enduse::SetArray = ReadDisk(db, "$Input/EnduseKey")
  EnduseDS::SetArray = ReadDisk(db, "$Input/EnduseDS")
  Enduses::Vector{Int} = collect(Select(Enduse))
  
  ANMap::VariableArray{2} = ReadDisk(db, "MainDB/ANMap") # [Area,Nation] Map between Area and Nation
  BaseSw::Float32 = ReadDisk(db, "SInput/BaseSw")[1]
  EndTime::Float32 = ReadDisk(db,"SInput/EndTime")[1] #[tv]  Ending Year for Simulation (Year)
  zPOCA::VariableArray{6} = ReadDisk(db, "$Outpt2/POCA") # [Enduse,FuelEP,EC,Poll,Area,Year] Pollution (Tonnes/Yr)
  zPOCARef::VariableArray{6} = ReadDisk(RefNameDB, "$Outpt2/POCA") # [Enduse,FuelEP,EC,Poll,Area,Year] Pollution (Tonnes/Yr)
  NationOutputMap::VariableArray{1} = ReadDisk(db,"SInput/NationOutputMap") #[Nation] Map for Output Control by Nation (0=No Output)(Map)
  PolConv::VariableArray{1} = ReadDisk(db, "SInput/PolConv") # Pollution Conversion Factor (convert GHGs to eCO2)

  SceName::String = ReadDisk(db,"SInput/SceName") #  Scenario Name
  
  #
  # Scratch Variables
  #
  Conversion::VariableArray{1} = zeros(Float32,length(Poll)) # [Poll] Units Conversion Factor
  UnitsDS::SetArray = fill("",length(Poll)) # [Poll] Units Description

end

function initialize_conversion_data(data)
  (; Poll, Conversion, UnitsDS) = data
  
  # Initialize conversion factors
  poll_conversions = Dict(
    "SOX" => 0.001, "NOX" => 0.001, "PMT" => 0.001, "VOC" => 0.001,
    "N2O" => 0.001, "COX" => 0.001, "CO2" => 0.001, "CH4" => 0.001,
    "SF6" => 0.001, "PFC" => 0.001, "HFC" => 0.001, "PM25" => 0.001,
    "PM10" => 0.001, "Hg" => 1000.0, "O3" => 0.001, "NH3" => 0.001,
    "H2O" => 0.001, "BC" => 0.001, "NF3" => 0.001
  )
  
  # Initialize units descriptions
  poll_units = Dict(
    "SOX" => "Tonnes/TBtu", "NOX" => "Tonnes/TBtu", "PMT" => "Tonnes/TBtu", 
    "VOC" => "Tonnes/TBtu", "N2O" => "Tonnes/TBtu", "COX" => "Tonnes/TBtu", 
    "CO2" => "Tonnes/TBtu", "CH4" => "Tonnes/TBtu", "SF6" => "Tonnes/TBtu", 
    "PFC" => "Tonnes/TBtu", "HFC" => "Tonnes/TBtu", "PM25" => "Tonnes/TBtu",
    "PM10" => "Tonnes/TBtu", "Hg" => "Grams/TBtu", "O3" => "Tonnes/TBtu", 
    "NH3" => "Tonnes/TBtu", "H2O" => "Tonnes/TBtu", "BC" => "Tonnes/TBtu", 
    "NF3" => "Tonnes/TBtu"
  )
  
  for poll_idx in eachindex(Poll)
    poll_key = Poll[poll_idx]
    if haskey(poll_conversions, poll_key)
      Conversion[poll_idx] = poll_conversions[poll_key]
    else
      Conversion[poll_idx] = 1.0
    end
    
    if haskey(poll_units, poll_key)
      UnitsDS[poll_idx] = poll_units[poll_key]
    else
      UnitsDS[poll_idx] = "Units"
    end
  end
end


function zPOCA_DtaRun_Residential(data, iob, nation, polls)
  (; Area, AreaDS, Areas, Nation, NationDS, Nations) = data
  (; Poll, PollDS, Polls, FuelEP, FuelEPDS, FuelEPs, EC, ECDS, ECs, Year) = data
  (; Enduse, EnduseDS, Enduses) = data
  (; ANMap, BaseSw, Conversion, EndTime, UnitsDS, zPOCA, zPOCARef, NationOutputMap) = data

  initialize_conversion_data(data)

  if BaseSw != 0
    @. zPOCARef = zPOCA
  end

  # Select specific years (2005,2010,2015,2020,2025,2030,2035,2040,2045,2050)
  selected_years = [2005, 2010, 2015, 2020, 2025, 2030, 2035, 2040, 2045, 2050]
  year_indices = [Yr(year) for year in selected_years if Yr(year) <= length(Year)]
  
  areas = findall(ANMap[:,nation] .== 1)

  if NationOutputMap[nation] == 1
    for year_idx in year_indices
      for enduse in Enduses
        for fuelep in FuelEPs
          for ec in ECs
            for poll in polls
              for area in areas
                ZZZ = zPOCA[enduse,fuelep,ec,poll,area,year_idx] * Conversion[poll]
                CCC = zPOCARef[enduse,fuelep,ec,poll,area,year_idx] * Conversion[poll]
                
                if ZZZ > 0.01 || CCC > 0.01
                  println(iob,"zPOCA;",Year[year_idx],";",AreaDS[area],";",PollDS[poll],";",
                    ECDS[ec],";",FuelEPDS[fuelep],";",EnduseDS[enduse],";",UnitsDS[poll],";",
                    @sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
                end
              end
            end
          end
        end
      end
    end
  end

end

function zPOCA_DtaRun_Commercial(data, iob, nation, polls)
  (; Area, AreaDS, Areas, Nation, NationDS, Nations) = data
  (; Poll, PollDS, Polls, FuelEP, FuelEPDS, FuelEPs, EC, ECDS, ECs, Year) = data
  (; Enduse, EnduseDS, Enduses) = data
  (; ANMap, BaseSw, Conversion, EndTime, UnitsDS, zPOCA, zPOCARef, NationOutputMap) = data

  initialize_conversion_data(data)

  if BaseSw != 0
    @. zPOCARef = zPOCA
  end

  # Select specific years (2005,2010,2015,2020,2025,2030,2035,2040,2045,2050)
  selected_years = [2005, 2010, 2015, 2020, 2025, 2030, 2035, 2040, 2045, 2050]
  year_indices = [Yr(year) for year in selected_years if Yr(year) <= length(Year)]
  
  areas = findall(ANMap[:,nation] .== 1)

  if NationOutputMap[nation] == 1
    for year_idx in year_indices
      for enduse in Enduses
        for ec in ECs
          for poll in polls
            for area in areas
              for fuelep in FuelEPs
                ZZZ = zPOCA[enduse,fuelep,ec,poll,area,year_idx] * Conversion[poll]
                CCC = zPOCARef[enduse,fuelep,ec,poll,area,year_idx] * Conversion[poll]
                
                if ZZZ > 0.01 || CCC > 0.01
                  println(iob,"zPOCA;",Year[year_idx],";",AreaDS[area],";",PollDS[poll],";",
                    ECDS[ec],";",FuelEPDS[fuelep],";",EnduseDS[enduse],";",UnitsDS[poll],";",
                    @sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
                end
              end
            end
          end
        end
      end
    end
  end

end

function zPOCA_DtaRun_Industrial(data, iob, nation, polls)
  (; Area, AreaDS, Areas, Nation, NationDS, Nations) = data
  (; Poll, PollDS, Polls, FuelEP, FuelEPDS, FuelEPs, EC, ECDS, ECs, Year) = data
  (; Enduse, EnduseDS, Enduses) = data
  (; ANMap, BaseSw, Conversion, EndTime, UnitsDS, zPOCA, zPOCARef, NationOutputMap) = data

  initialize_conversion_data(data)

  if BaseSw != 0
    @. zPOCARef = zPOCA
  end

  # Select specific years (2005,2010,2015,2020,2025,2030,2035,2040,2045,2050)
  selected_years = [2005, 2010, 2015, 2020, 2025, 2030, 2035, 2040, 2045, 2050]
  year_indices = [Yr(year) for year in selected_years if Yr(year) <= length(Year)]
  
  areas = findall(ANMap[:,nation] .== 1)

  if NationOutputMap[nation] == 1
    for year_idx in year_indices
      for enduse in Enduses
        for ec in ECs
          for poll in polls
            for area in areas
              for fuelep in FuelEPs
                ZZZ = zPOCA[enduse,fuelep,ec,poll,area,year_idx] * Conversion[poll]
                CCC = zPOCARef[enduse,fuelep,ec,poll,area,year_idx] * Conversion[poll]
                
                if ZZZ > 0.01 || CCC > 0.01
                  println(iob,"zPOCA;",Year[year_idx],";",AreaDS[area],";",PollDS[poll],";",
                    ECDS[ec],";",FuelEPDS[fuelep],";",EnduseDS[enduse],";",UnitsDS[poll],";",
                    @sprintf("%.6E",ZZZ),";",@sprintf("%.6E",CCC))
                end
              end
            end
          end
        end
      end
    end
  end

end


function zPOCA_Residential(db,iob,nation,polls)
  data = RzPOCAData(; db)
  zPOCA_DtaRun_Residential(data,iob,nation,polls)
end

function zPOCA_Commercial(db,iob,nation,polls)
  data = CzPOCAData(; db)
  zPOCA_DtaRun_Commercial(data,iob,nation,polls)
end

function zPOCA_Industrial(db,iob,nation,polls)
  data = IzPOCAData(; db)
  zPOCA_DtaRun_Industrial(data,iob,nation,polls)
end

function zPOCA_CreateOutputFile(iob,PollType,nationkey,SceName)
  filename = "zPOCA-$PollType-$nationkey-$SceName.dta"
  open(joinpath(OutputFolder, filename), "w") do filename
    write(filename, String(take!(iob)))
  end
end


function zPOCA_DtaControl(db)
  @info "zPOCA_DtaControl"
  data = RzPOCAData(; db)
  (; SceName) = data
  
  # Select specific pollutants (SOX,COX,NOX,PMT,VOC,PM25,PM10,Hg,NH3,BC)
  polls=Select(data.Poll,["SOX", "COX", "NOX", "PMT", "VOC", "PM25", "PM10", "Hg", "NH3", "BC"])
  PollType = "CAC"

  nations = Select(data.Nation,["CN", "US"])
  
  
  for nation_idx in nations
    if nation_idx > 0
      iob = IOBuffer()
      println(iob,"Variable;Year;Area;Pollutant;Sector;Fuel;Enduse;Units;zData;zInitial")

      zPOCA_Residential(db, iob, nation_idx, polls)
      zPOCA_Commercial(db, iob, nation_idx, polls)
      zPOCA_Industrial(db, iob, nation_idx, polls)

      zPOCA_CreateOutputFile(iob,PollType,data.Nation[nation_idx],SceName)
    end
  end
end

if abspath(PROGRAM_FILE) == @__FILE__
  zPOCA_DtaControl(DB)
end
