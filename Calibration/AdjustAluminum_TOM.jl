#
# AdjustAluminum_TOM.jl
#
using EnergyModel

module AdjustAluminum_TOM
...
function SetPCTC(data, SelectArea, TargetArea, ec)
  ...
  (;Enduses) = data
  (;Techs,Years) = data
  (;PCCN,PCTC,PFPN,PEM,PFTC,POCF,RPCTC,RPFTC) = data
  
  for tech in Techs, enduse in Enduses
    PCCN[enduse,tech,ec,TargetArea] = PCCN[enduse,tech,ec,SelectArea]
  end
  
  for year in Years, tech in Techs, enduse in Enduses
    PCTC[enduse,tech,ec,TargetArea,year] = PCTC[enduse,tech,ec,SelectArea,year]
  end
  
  for enduse in Enduses
    PEM[enduse,ec,TargetArea] = PEM[enduse,ec,SelectArea]
  end
  
  for tech in Techs, enduse in Enduses
    POCF[enduse,tech,ec,TargetArea] = POCF[enduse,tech,ec,SelectArea]
  end
  
  for tech in Techs, enduse in Enduses
    PFPN[enduse,tech,ec,TargetArea] = PFPN[enduse,tech,ec,SelectArea]
  end
    
  for year in Years, tech in Techs, enduse in Enduses
    PFTC[enduse,tech,ec,TargetArea,year] = PFTC[enduse,tech,ec,SelectArea,year]
  end
    
  for year in Years, tech in Techs, enduse in Enduses
    RPCTC[enduse,tech,ec,TargetArea,year] = RPCTC[enduse,tech,ec,SelectArea,year]
  end
    
  for year in Years, tech in Techs, enduse in Enduses
    RPFTC[enduse,tech,ec,TargetArea,year] = RPFTC[enduse,tech,ec,SelectArea,year]
  end  
...
function CalibrationControl(db)
  @info "AdjustAluminum_TOM.jl - CalibrationControl"
