...
Base.@kwdef struct EControl
  ...
  OBAOffsets::VariableArray{2} = zeros(Float32,length(Plant),length(Year)) # [Year] Output-Based Allocations for Electric Generation (Tonnes/GWh)
...
function ElecPolicy(db)
  ...
  (; FBuyFr,GoalPolSw,GPEUSw,GratSw,ISaleSw,OBA,OBAElectric) = data
  (; OBAFraction,OBANaturalGasNew,OBAOffsets,OBARawNG) = data
  (; OffsetConv,OffMktFr,OffNew,PBnkSw,PCost,PCovMarket,PolConv) = data
  (; PolCovRef,PollMarket,xPolTot,RePotential) = data
  (; ReReductionsAB,UnArea,UnCounter,UnCoverage) = data
  ...
  for year in Years, plant in plants
    OBAOffsets[plant,year]=370
  end
  years=collect(Yr(2030):Final)
  for year in years, plant in plants
    OBAOffsets[plant,year]=311
  end
  years = collect(Yr(2023):Yr(2029))
  for year in years, plant in plants
      ...
      for year in years, unit in units
        if UnCoverage[unit,CO2,year] == 1
          ...
          if ECCMarket[ecc,market,year] == 1
            ...
            #
            if (UnPlant[unit] == "OnshoreWind") || (UnPlant[unit] == "SolarPV") || (UnPlant[unit] == "SolarThermal")
              UnOffsets[unit,poll,year]=OBAOffsets[plant,year]
            end
          end
        end
      end
