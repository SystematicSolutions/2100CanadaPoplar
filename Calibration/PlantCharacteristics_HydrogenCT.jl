#
# PlantCharacteristics_HydrogenCT.jl
#
using EnergyModel

module PlantCharacteristics_HydrogenCT
...
Base.@kwdef struct EControl
  ...
  GCExpSw::VariableArray{3} = ReadDisk(db,"EGInput/GCExpSw") # [Plant,Area,Year] Generation Capacity Expansion Switch (Switch)
...
function ECalibration(db)
  data = EControl(; db)
  (;Area,AreaDS,Areas,Fuel,FuelDS,FuelEP,FuelEPDS,FuelEPs,Fuels,GenCo) = data
  (;GenCoDS,GenCos,Month,MonthDS,Months,Node,NodeDS,Nodes,Plant,PlantDS) = data
  (;Plants,Poll,PollDS,Polls,Power,PowerDS,Powers,TimeP,TimePs,Year) = data
  (;YearDS,Years) = data
  ...
  for year in Years, area in areas, month in Months, timep in TimePs
    AvFactor[FuelCell,timep,month,area,year]=AvFactor[OGCT,timep,month,area,year]
  end
  WriteDisk(db,"EGInput/AvFactor",AvFactor)

  for year in Years, area in areas, poll in Polls, fuelep in FuelEPs
    CFStd[fuelep,FuelCell,poll,area,year]=CFStd[fuelep,OGCT,poll,area,year]
  end
  WriteDisk(db,"EGInput/CFStd",CFStd)

  for year in Years, area in areas, poll in Polls
    CoverNew[FuelCell,poll,area,year]=CoverNew[OGCT,poll,area,year]
  end
  WriteDisk(db,"EGInput/CoverNew",CoverNew)

  for year in Years, area in areas
    DInvFr[FuelCell,area,year]=DInvFr[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/DInvFr",DInvFr)

  for year in Years, area in areas
    DIVTC[FuelCell,area,year]=DIVTC[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/DIVTC",DIVTC)

  for year in Years, area in areas
    DRisk[FuelCell,area,year]=DRisk[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/DRisk",DRisk)

  for area in areas
    EmitNew[FuelCell,area]=EmitNew[OGCT,area]
  end
  WriteDisk(db,"EGInput/EmitNew",EmitNew)

  EmitSw[FuelCell]=EmitSw[OGCT]
  WriteDisk(db,"EGInput/EmitSw",EmitSw)

  for area in areas, poll in Polls, fuelep in FuelEPs
    EUPCostN[fuelep,FuelCell,poll,area]=EUPCostN[fuelep,OGCT,poll,area]
  end
  WriteDisk(db,"EGInput/EUPCostN",EUPCostN)

  for area in areas, poll in Polls, fuelep in FuelEPs
    EUPVF[fuelep,FuelCell,poll,area]=EUPVF[fuelep,OGCT,poll,area]
  end
  WriteDisk(db,"EGInput/EUPVF",EUPVF)

  for year in Years, poll in Polls, fuelep in FuelEPs
    EURCstM[fuelep,FuelCell,poll,year]=EURCstM[fuelep,OGCT,poll,year]
  end
  WriteDisk(db,"EGInput/EURCstM",EURCstM)

  for area in areas, poll in Polls, fuelep in FuelEPs
    EUROCF[fuelep,FuelCell,poll,area]=EUROCF[fuelep,OGCT,poll,area]
  end
  WriteDisk(db,"EGInput/EUROCF",EUROCF)

  for year in Years, area in areas, poll in Polls
    EURPCSw[FuelCell,poll,area,year]=EURPCSw[OGCT,poll,area,year]
  end
  WriteDisk(db,"EGInput/EURPCSw",EURPCSw)

  for year in Years, area in areas, poll in Polls, fuelep in FuelEPs
    EUVRP[fuelep,FuelCell,poll,area,year]=EUVRP[fuelep,OGCT,poll,area,year]
  end
  WriteDisk(db,"EGInput/EUVRP",EUVRP)

  for fuelep in FuelEPs
    EUVRRT[fuelep,FuelCell]=EUVRRT[fuelep,OGCT]
  end
  WriteDisk(db,"EGInput/EUVRRT",EUVRRT)

  #
  # F1New[FuelCell,area]="Hydrogen"
  # WriteDisk(db,"EGInput/F1New",F1New)
  #

  for year in Years, area in areas
    FIT[FuelCell,area,year]=FIT[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/FIT",FIT)

  for fuel in Fuels
    FlPlnMap[fuel,FuelCell]=0.0
  end
  fuel=Select(Fuel,"Hydrogen")
  FlPlnMap[fuel,FuelCell]=1.0
  WriteDisk(db,"EGInput/FlPlnMap",FlPlnMap)

  for year in Years, area in areas
    GCCCFlag[FuelCell,area,year]=GCCCFlag[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/GCCCFlag",GCCCFlag)

  # for year in Years, area in areas
  #   GCCCN[FuelCell,area,year]=GCCCN[OGCT,area,year]
  # end
  #  WriteDisk(db,"EGInput/GCCCN",GCCCN)

  for year in Years
    GCDevTime[FuelCell,year]=GCDevTime[OGCT,year]
  end
  WriteDisk(db,"EGInput/GCDevTime",GCDevTime)

  for year in Years, area in areas
    GCExpSw[FuelCell,area,year]=GCExpSw[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/GCExpSw",GCExpSw)

  for year in Years, area in areas
    GCBL[FuelCell,area,year]=GCBL[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/GCBL",GCBL)

  for year in Years, area in areas
    GCTL[FuelCell,area,year]=GCTL[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/GCTL",GCTL)

  for year in Years, area in areas, node in nodes
    GrMSM[FuelCell,node,area,year]=GrMSM[OGCT,node,area,year]
  end
  WriteDisk(db,"EGInput/GrMSM",GrMSM)

  for year in Years, timep in TimePs, genco in GenCos
    HDFCFR[FuelCell,genco,timep,year]=HDFCFR[OGCT,genco,timep,year]
  end
  WriteDisk(db,"EGInput/HDFCFR",HDFCFR)

  for year in Years, month in Months, timep in TimePs, node in nodes, genco in GenCos
    HDGCFR[FuelCell,genco,node,timep,month,year]=HDGCFR[OGCT,genco,node,timep,month,year]
  end
  WriteDisk(db,"EGInput/HDGCFR",HDGCFR)

  for year in Years, month in Months, timep in TimePs, node in nodes, genco in GenCos
    HDVCFR[FuelCell,genco,node,timep,month,year]=HDVCFR[OGCT,genco,node,timep,month,year]
  end
  WriteDisk(db,"EGInput/HDVCFR",HDVCFR)

  for year in Years, area in areas, poll in Polls
    MEPOCX[FuelCell,poll,area,year]=MEPOCX[OGCT,poll,area,year]
  end
  WriteDisk(db,"EGInput/MEPOCX",MEPOCX)

  for area in areas
    MRunNew[FuelCell,area]=MRunNew[OGCT,area]
  end
  WriteDisk(db,"EGInput/MRunNew",MRunNew)

  for year in Years, area in areas, poll in Polls
    OffNew[FuelCell,poll,area,year]=OffNew[OGCT,poll,area,year]
  end
  WriteDisk(db,"EGInput/OffNew",OffNew)

  for year in Years, area in areas, poll in Polls, fuelep in FuelEPs
    PCostM[fuelep,FuelCell,poll,area,year]=PCostM[fuelep,OGCT,poll,area,year]
  end
  WriteDisk(db,"EGInput/PCostM",PCostM)

  for year in Years, area in areas
    for fuelep in FuelEPs
      PFFrac[fuelep,FuelCell,area,year]=0
    end
    fuelep=Select(FuelEP,"Hydrogen")
    PFFrac[fuelep,FuelCell,area,year]=1.0
  end
  WriteDisk(db,"EGInput/PFFrac",PFFrac)

  for area in areas
    PjMax[FuelCell,area]=PjMax[OGCT,area]
  end
  WriteDisk(db,"EGInput/PjMax",PjMax)

  #########JSO Change#########
  for area in areas
    PjMnPS[FuelCell,area]=1
  end
  WriteDisk(db,"EGInput/PjMnPS",PjMnPS)

  PlantSw[FuelCell]=PlantSw[OGCT]
  WriteDisk(db,"EGInput/PlantSw",PlantSw)

  for year in Years, area in areas, poll in Polls, fuelep in FuelEPs
    POCX[fuelep,FuelCell,poll,area,year]=0.0
  end
  WriteDisk(db,"EGInput/POCX",POCX)

  for year in Years, area in areas
    PoTRNewExo[FuelCell,area,year]=PoTRNewExo[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/PoTRNewExo",PoTRNewExo)

  for year in Years, node in nodes
    RAvNew[FuelCell,node,year]=RAvNew[FuelCell,node,year]
  end
  WriteDisk(db,"EGInput/RAvNew",RAvNew)

  for year in Years, area in areas
    RnBuildFr[FuelCell,area,year]=RnBuildFr[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/RnBuildFr",RnBuildFr)

  for year in Years, area in areas, genco in GenCos, node in nodes
    RnMSM[FuelCell,node,genco,area,year]=RnMSM[OGCT,node,genco,area,year]
  end
  WriteDisk(db,"EGInput/RnMSM",RnMSM)

  for area in areas
    RnSwitch[FuelCell,area]=RnSwitch[OGCT,area]
  end
  WriteDisk(db,"EGInput/RnSwitch",RnSwitch)

  for year in Years, node in nodes
    RRqNew[FuelCell,node,year]=RRqNew[OGCT,node,year]
  end
  WriteDisk(db,"EGInput/RRqNew",RRqNew)

  for year in Years, node in nodes
    RSwNew[FuelCell,node,year]=RSwNew[OGCT,node,year]
  end
  WriteDisk(db,"EGInput/RSwNew",RSwNew)

  for year in Years, area in areas
    Subsidy[FuelCell,area,year]=Subsidy[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/Subsidy",Subsidy)

  for year in Years, area in areas
    TDInvFr[FuelCell,area,year]=TDInvFr[OGCT,area,year]
  end
  WriteDisk(db,"EGInput/TDInvFr",TDInvFr)

  for year in Years, area in areas, genco in GenCos, node in nodes
    UnNewNumber[FuelCell,node,genco,area,year]=UnNewNumber[OGCT,node,genco,area,year]
  end
...
function CalibrationControl(db)
  @info "PlantCharacteristics_HydrogenCT.jl - CalibrationControl"
