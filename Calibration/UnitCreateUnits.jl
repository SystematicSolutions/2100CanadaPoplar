#
# UnitCreateUnits.jl - Create Electric Utility units which will be
#                       given capacity endogenously by ENERGY 2100
#
using EnergyModel

module UnitCreateUnits
...
function CommonCharacteristics(data,unit,plant,genco,node,area,nation)
  ...
  (;UnCogen,UnEAF,UnEffStorage,UnEmit,UnFacility,UnF1,xUnFlFr,UnGenCo) = data
  (;UnHRt,UnMECX,UnMustRun,UnNation,UnNode,UnOnLine,UnOR,UnOwner,UnOUREG) = data
  ...
  UnFacility[unit] = "Endogenous Build "*Area[area]
  UnSource[unit]   = 1
...
function UnitWriteDisk(data)
  ...
  (;UnOURGC,UnPlant,UnPOCX,UnPointer,UnRetire,UnSector,UnSource,UnSqFr,UnStorage,UnUFOMC,UnUOMC) = data
  ...
  WriteDisk(db,"EGInput/UnPointer",UnPointer)
...
function CreateUnits(db)
  data = EControl(; db)
  (;Area,Areas,GenCo,GenCos,Nation,Nations,Node,Nodes) = data
  (;Plant,Plants) = data
  (;ANMap,PjNSw,UnCode,UnCounter,UnitKey,UnName,UnPointer) = data
  ...
  for area in Areas
    ...
    nations = findall(ANMap[area,Nations] .== 1)
    ...
    #
    for node in Nodes
      if PjNSw[node,genco,area,Yr(1985)] == 1   
      
        @info " Top $area $genco $node $nation "
      
        #
        # Create units which are expected to be built endogenously
        #
        if Nation[nation] == "CN"
          
          if Area[area] == "ON"
            plants = Select(Plant,["OGCT","OGCC","SmallOGCC","NGCCS","OtherGeneration",
                                   "SmallHydro","BaseHydro","Biomass","BiomassCCS",
                                   "OnshoreWind","Battery","PeakHydro","SolarPV","Nuclear","SMNR"])
          
          elseif Area[area] == "QC"
            plants = Select(Plant,["OtherGeneration","SmallHydro","BaseHydro",
                                   "Biomass","BiomassCCS","OnshoreWind",
                                   "Battery","PeakHydro","SolarPV"])                         
                         
          elseif Area[area] == "BC"
            plants = Select(Plant,["OtherGeneration","SmallHydro","PeakHydro",
                                   "BaseHydro","Biomass","BiomassCCS","OnshoreWind",
                                   "Battery","SolarPV","Geothermal"])                           
                         
                         
          elseif Area[area] == "AB"
            plants = Select(Plant,["OGCT","OGCC","SmallOGCC","NGCCS","OtherGeneration",
                                   "SmallHydro","BaseHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","Battery",
                                   "SolarPV","Nuclear","SMNR","Geothermal"])                         
                         
          elseif Area[area] == "MB"
            plants = Select(Plant,["OGCT","OtherGeneration","SmallHydro","BaseHydro","PeakHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","Battery",
                                   "SolarPV","Nuclear","SMNR"])                          
                         
          elseif Area[area] == "SK"
            plants = Select(Plant,["OGCT","OGCC","SmallOGCC","NGCCS","OtherGeneration",
                                   "SmallHydro","BaseHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","Battery",
                                   "SolarPV","Nuclear","Geothermal"])
                                   
          elseif Area[area] == "NB"
            plants = Select(Plant,["OGCT","OGCC","SmallOGCC","NGCCS","OtherGeneration",
                                   "SmallHydro","BaseHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","OffshoreWind",
                                   "Battery","SolarPV","Nuclear","SMNR"])                         
                                  
          elseif Area[area] ==  "NS"
            plants = Select(Plant,["OGCT","OGCC","SmallOGCC","NGCCS","OtherGeneration",
                                   "SmallHydro","BaseHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","OffshoreWind",
                                   "Battery","SolarPV","Wave"])                             
                         
          elseif Area[area] ==  "NL"
            plants = Select(Plant,["OGCT","OtherGeneration","SmallHydro","BaseHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","OffshoreWind",
                                   "Battery","SolarPV"])                          
                         
          elseif Area[area] == "PE"
            plants = Select(Plant,["OtherGeneration",
                                   "Biomass","BiomassCCS","OnshoreWind","Battery",
                                   "SolarPV","Nuclear","SMNR"])                            
                         
          elseif Area[area] == "YT"
            plants = Select(Plant,["OGCT","SmallOGCC","OtherGeneration","SmallHydro","BaseHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","Battery",
                                   "SolarPV","Nuclear"])                          
          ...
          elseif Area[area] == "NU"
            ...
            plants = Select(Plant,["OGCT","OGCC","SmallOGCC","OtherGeneration","SmallHydro",
                                   "Biomass","BiomassCCS","OnshoreWind","Battery",
                                   "SolarPV","Nuclear","SMNR"])                           
                         
          end
          
        elseif Nation[nation] == "US"
        
          if (Area[area] == "CA") || (Area[area] == "Mtn") || (Area[area] == "Pac") 
            plants = Select(Plant,["OGCT","OGCC","Biomass","OnshoreWind","SolarPV","Geothermal"])           

          else
             plants = Select(Plant,["Geothermal","OGCT","OGCC","Biomass","OnshoreWind",
                                    "OffshoreWind","SolarPV"]) 
          end
      
        elseif Nation[nation] == "MX"
          plants = Select(Plant,["OGCT","OGCC","Biomass","OnshoreWind","SolarPV"])
        end

        for plant in plants
          if UnPointer[plant,node,genco,area,Yr(1985)] == 0
            uncounter = uncounter+1
            UnPointer[plant,node,genco,area,Yr(1985)] = uncounter
            
            UnName[uncounter] = Area[area]*" Endo "*Node[node]*" "*GenCo[genco]*" "*Plant[plant]  
            ...
            GenNum = string(genco)
            if genco < 10
              GenNum = "0"*string(genco)
            end
            PltNum = string(plant)
            if plant < 10
              PltNum = "0"*string(plant)
            end
            NodNum = string(node)
            if node < 10
              NodNum = "0"*string(node)
            end
            UnCode[uncounter] = Area[area]*"_Endo"*NodNum*GenNum*PltNum
            UnitKey[uncounter] = UnCode[uncounter]
  ...
  for year in years, area in Areas, genco in GenCos, node in Nodes, plant in Plants
     UnPointer[plant,node,genco,area,year] = UnPointer[plant,node,genco,area,Yr(1985)]
  end  
...
function Control(db)
  @info "UnitCreateUnits.jl - Control"
  CreateUnits(db)
end
