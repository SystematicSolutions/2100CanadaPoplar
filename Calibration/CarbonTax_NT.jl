...
function ElecPolicy(db)
  ...
  (; PollMarket,xPolTot,UnArea,UnCounter,UnCoverage) = data
  ...
  Enforce[market] = 2019
  #
  # Set YrFinal equal to the final value of xETAPr - Ian 07/24/25
  #
  YrFinal = Int(2024-ITime+1)
  ...
  #
  # Exemption for Heating oil in Buildings for the next 3 years
  #
  ...
  years_temp=collect(Yr(2024):Yr(2026))
  for year in years_temp, area in areas, pcov in pcovs, ecc in eccs
    PCovMap[HFO,ecc,pcov,area,year] = 0
    PCovMap[LFO,ecc,pcov,area,year] = 0
  end
  ...
  years = collect(Yr(2025):YrFinal)
  for year in years
    xETAPr[market,year] = 0.00
  end
