#
# GHG_Feedstocks_MX.jl - Reads in U.S. values for feedstock coefficients
#
using EnergyModel

module GHG_Feedstocks_MX
...
function CCalibration(db)
  data = CControl(; db)
  (;Input) = data
  (;ECs,Enduses,Fuel,Fuels,FuelEP,Nation,Poll,Polls,Years) = data
...
function ICalibration(db)
  data = IControl(; db)
  (;Input) = data
  (;EC,ECs,Enduses,Fuel,Fuels,FuelEP,Nation,Poll,Polls,Years) = data
  (;ANMap,FsPOCX,POCX) = data
...
function TCalibration(db)
  data = TControl(; db)
  (;Input) = data
  (;Nation,Tech,Poll,Polls,Fuel,Fuels,ECs,Techs,Years) = data
  (;ANMap,FsPOCX) = data
...
function CalibrationControl(db)
  @info "GHG_Feedstocks_MX.jl - CalibrationControl"
