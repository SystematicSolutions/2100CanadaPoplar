#
# GHG_Transportation_MX.jl - this file calculates the GHG coefficients
...
module GHG_Transportation_MX
  ...
  function CalcCoefficients(db)
    ...
    for year in Years, area in areas, poll in polls, ec in ECs, tech in Techs
      TrMEPX[tech,ec,poll,area,year] = 0.0
    end
    
    for year in Years, area in areas, poll in polls, ec in ECs, tech in Techs
      sumVDT = sum(VDT[enduse,tech,ec,area,year] for enduse in Enduses)
      @finite_math TrMEPX[tech,ec,poll,area,year] = xTrMEPol[tech,ec,poll,area,year]/sumVDT
    end
  ...
  function CalibrationControl(db)
    @info "GHG_Transportation_MX.jl - CalibrationControl"
