...
Base.@kwdef struct CAC_IndustrialData
  db::String
...
function CalcCoefficients(data,polls,year)
  ...
  for ec in ECs
    ...
    for area in areas, poll in polls
      ...
      #
      # Forestry Process emissions are driven by Land Use
      #
      if ECC[ecc] == "Forestry"
        landuse = Select(ECC,"LandUse")
        @finite_math MEPOCX[ecc,poll,area,year] = (xMEPol[ecc,poll,area,year] + MisPol[ec,poll,area]) / MEDriver[landuse,area,year]
      end
    end
  end
end
...
function UnitCoefficients(data,polls,year)
  ...
  #
  for poll in polls
    units = findall(xUnPolSw[Units,poll,year] .== 3)
    for unit in units
      ec,ecc,area,valid = GetUnitSets(data,unit)
      if valid==true
        for fuelep in FuelEPs
...
function ExtCoefficients(data,polls,years,Yr1,Yr2)
  ...
  for poll in polls, year in years
    units1 = findall(xUnPolSw[Units,poll,year] .== 3)
...
function CAC_IndustrialDataCalibration(db)
  data = CAC_IndustrialData(; db)
  (;Poll,Input,CgPOCX) = data
  (;FlPOCX,MEPOCX,ORMEPOCX,POCX,UnPOCX,VnPOCX,FsPOCX,FuPOCX) = data
...
function CalibrationControl(db)
  @info "CAC_Industrial.jl - CalibrationControl"

  CAC_IndustrialDataCalibration(db)
