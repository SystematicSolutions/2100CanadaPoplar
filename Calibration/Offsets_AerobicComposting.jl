#
# Offsets_AerobicComposting.jl - Aerobic Composting (AC) - Diverting
# organic waste from landfill to compost (instead of methane -
# CO2 is being generated, no electricity generation).  The CO2
# produced is not recorded since is defined as "carbon neutral".
#
using EnergyModel

module Offsets_AerobicComposting
...
function OffsetData(db)
  ...
  #
  # Solid Waste Process Emission Reductions
  # Source: "Aerobic Composting Cost Curve v1.xlsx", Jeff Amlin 4/3/14
  ...
  for area in areas
    ReA0[offset,area] = 14375.75
  end
  
  for area in areas
    ReB0[offset,area] = -2.5671
  end

  ReC0[offset,NL,year] = 1.5631 *1000
  ReC0[offset,PE,year] = 0.0000 *1000
  ReC0[offset,NS,year] = 0.0000 *1000
  ReC0[offset,NB,year] = 7.0781 *1000
  ReC0[offset,QC,year] = 6.9324 *1000
  ReC0[offset,ON,year] = 4.0264 *1000
  ReC0[offset,MB,year] = 3.1455 *1000
  ReC0[offset,SK,year] = 3.1665 *1000
  ReC0[offset,AB,year] = 8.5475 *1000
  ReC0[offset,BC,year] = 2.7256 *1000
...
function Control(db)
  @info "Offsets_AerobicComposting.jl - Control"
  OffsetData(db)
end
