#
# Offsets_AnaerobicWWT.jl - Anaerobic  Wastewater Treatment (WWT)
# Anaerobic treatment uses anaerobic bacteria (biomass) to convert organic
# pollutants (CH4) into biogas in an oxygen free environment.
#
using EnergyModel

module Offsets_AnaerobicWWT
...
function OffsetData(db)
  ...
  #
  # Wastewater Process Emission Reductions
  # Source: "Wastewater Cost Curve v1.xlsx", Randy Levesque 4/18/14
  ...
  for area in areas
    ReA0[offset,area] = 31.1041
  end
  
  for area in areas
    ReB0[offset,area] = -1.2229
  end

  ReC0[offset,NL,year] = 0.3770 *1000
  ReC0[offset,PE,year] = 0.3274 *1000
  ReC0[offset,NS,year] = 0.4084 *1000
  ReC0[offset,NB,year] = 0.3105 *1000
  ReC0[offset,QC,year] = 7.5621 *1000
  ReC0[offset,ON,year] = 6.6725 *1000
  ReC0[offset,MB,year] = 1.1121 *1000
  ReC0[offset,SK,year] = 0.6672 *1000
  ReC0[offset,AB,year] = 5.2411 *1000
  ReC0[offset,BC,year] = 2.4466 *1000
...
function Control(db)
  @info "Offsets_AnaerobicWWT.jl - Control"
  OffsetData(db)
end
