#
# CarbonTax_OBA_NS.jl - WCI Cap-and-Trade for NS
#
using EnergyModel

module CarbonTax_OBA_NS
...
function ElecPolicy(db)
  ...
  (; xPolTot,UnArea,UnCogen,UnCounter,UnCoverage) = data
  (; UnF1,UnOnLine,UnPGratis) = data
  (; UnPlant,UnSector,xDriver,xETAPr) = data
  (; xExchangeRate,xExchangeRateNation,xFSell,xGPNew,xGoalPol) = data
  (; xInflationNation,xISell,xPGratis,xPolCap,xUnEGA) = data
  (; xUnEGA,xUnFlFr,xUnGP,xUnPol) = data
  (; EIBase,OBA,OBACogen,OBAElectric,OBANaturalGasNew) = data
  ...
  plant1=first(Plants)
  fuelep1=first(FuelEPs)
  for year in years
    OBACogen[year] = OBAElectric[plant1,fuelep1,year]
  end
  ...
  years=collect(Yr(2022):Yr(2023))
  for year in years
    OBANaturalGasNew[year] = 288.0
  end
  ...
  for year in years 
    ...
    (OBANaturalGasNew[Yr(2030)]-OBANaturalGasNew[Yr(2023)])/(2030-2023)
  end
  years = collect(Current:YrFinal)
  ...
  for fuelep in FuelEPs, plant in plants
    OBAElectric[plant,fuelep,Yr(2022)] = 925.0
    OBAElectric[plant,fuelep,Yr(2023)] = 925.0
    OBAElectric[plant,fuelep,Yr(2024)] = 925.0
    OBAElectric[plant,fuelep,Yr(2025)] = 925.0
    OBAElectric[plant,fuelep,Yr(2026)] = 925.0
    OBAElectric[plant,fuelep,Yr(2027)] = 900.0
    OBAElectric[plant,fuelep,Yr(2028)] = 850.0
    OBAElectric[plant,fuelep,Yr(2029)] = 650.0
    years = collect(Yr(2030):YrFinal)
    for year in years
      OBAElectric[plant,fuelep,year] = 370.0
    end
  ...
  for year in years
    ETADAP[market,year] =  65.00/xExchangeRateNation[CN,year]/xInflationNation[US,year]
  end
...
function PolicyControl(db)
  @info"CarbonTax_OBA_NS.jl - PolicyControl"
  ElecPolicy(db)
end
